...
async function testPlatformStatus(): Promise<PlatformSnapshot | null> {
  ...
  try {
    ...
    if (snapshot.autonomy.mode === 'BLOCKED') {
      ...
      pass('autonomy_mode', `Autonomy: ${snapshot.autonomy.mode} (confidence: ${snapshot.autonomy.confidenceLevel}%)`);
    }

    // Check incidents
    if (snapshot.incidents.bySeverity.critical > 0) {
      fail('critical_incidents', `${snapshot.incidents.bySeverity.critical} critical incident(s) open`);
    } else {
  ...
  for (const tc of testCases) {
    try {
      ...
      if (tc.expectBlock && result.decision !== 'BLOCK') {
        ...
        pass(`execution_gate_${tc.name}`, `Decision: ${result.decision} (confidence: ${result.confidence}%)`);
      }
    } catch (error) {
      fail(`execution_gate_${tc.name}`, `Error: ${error instanceof Error ? error.message : 'Unknown'}`);
    }
  }
}
...
async function testContradictions(): Promise<ContradictionReport | null> {
  ...
  try {
    ...
    if (report.bySeverity.critical > 0) {
      fail('contradictions_critical', `${report.bySeverity.critical} critical contradiction(s)`, report.contradictions.filter(c => c.severity === 'critical'));
    } else {
      pass('contradictions_critical', 'No critical contradictions');
    }

    if (report.bySeverity.high > 0) {
      fail('contradictions_high', `${report.bySeverity.high} high severity contradiction(s)`, report.contradictions.filter(c => c.severity === 'high'));
    } else {
...
function testDegradedMode(snapshot: PlatformSnapshot | null): void {
  ...
  if (degradedFeatures.length > 0) {
    pass('degraded_mode', `${degradedFeatures.length} feature(s) in degraded mode`, degradedFeatures);
  } else {
  ...
  if (criticalDegraded.length > 0) {
    fail('critical_features_degraded', `Critical features degraded: ${criticalDegraded.join(', ')}`);
  } else {
...
function testFeatureAvailability(snapshot: PlatformSnapshot | null): void {
  ...
  pass('feature_availability_matrix', `ON: ${states.ON}, LIMITED: ${states.LIMITED}, BLOCKED: ${states.BLOCKED}, OFF: ${states.OFF}`);
...
async function runSmokeTests(): Promise<SmokeTestReport> {
  ...
  if (passed) {
    log('✓ המערכת עכשיו יודעת להגיד לעצמה מתי לעצור.');
  }
