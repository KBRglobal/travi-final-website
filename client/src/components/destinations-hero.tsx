...
import { MapPin, ArrowRight, Compass, Building2, Heart, BookOpen } from "lucide-react";
...
export function DestinationsHero({ destinationCount, regionCount }: DestinationsHeroProps) {
  ...
  return (
    <section
      className="relative min-h-[500px] sm:min-h-[600px] lg:min-h-[700px] flex items-center pt-20 sm:pt-24 pb-12 sm:pb-16 px-4 sm:px-6 lg:px-8 overflow-hidden bg-gradient-to-br from-slate-50 via-white to-purple-50/30 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950"
    ...
    >
      ...
      <div className="w-full max-w-7xl mx-auto relative z-10">
        <div className="grid lg:grid-cols-[1fr_1.3fr] gap-12 lg:gap-16 items-center">
          ...
          <div className="text-start order-1 lg:order-1" data-testid="destinations-hero-content">
            ...
            >
              <div className="inline-flex items-center gap-3 px-5 py-2.5 rounded-full bg-white dark:bg-slate-800 shadow-lg shadow-[#6443F4]/10 border border-[#6443F4]/20">
                ...
                <span
                  ...
                  data-testid="badge-destinations-count"
                >
            ...
            {(heroTitle || heroSubtitle) && (
              ...
              >
                {heroTitle && (
                  <span
                    className="block text-3xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl font-semibold text-slate-900 dark:text-white leading-[1.1] tracking-tight mb-1 sm:mb-2"
                ...
                {heroSubtitle && (
                  <span className="relative inline-block">
                    <span
                      className="block text-3xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl font-semibold leading-[1.1] tracking-tight animated-gradient-text"
                      style={{ fontFamily: "'Chillax', var(--font-sans)" }}
                    >
                      {heroSubtitle}
                    </span>
                    <span
                      className="absolute -bottom-1 sm:-bottom-2 left-0 w-full h-0.5 sm:h-1 bg-gradient-to-r from-[#6443F4] via-[#8B5CF6] to-[#F24294] rounded-full opacity-80"
                      aria-hidden="true"
                    />
                  </span>
                )}
            ...
            {heroDescription && (
              <motion.p
                className="text-base sm:text-lg text-slate-600 dark:text-slate-400 mb-6 sm:mb-8 max-w-md leading-relaxed"
                initial={shouldAnimate ? { opacity: 0, y: 20 } : {}}
                animate={{ opacity: 1, y: 0 }}
            ...
            >
              ...
              <Link href={localePath("/travel-guides")}>
                <Button
                  variant="outline"
                  className="rounded-full px-6 py-6 text-base font-semibold border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800"
                >
          ...
          {hasSlides && dest && (
            <div
              className="order-2 lg:order-2 relative px-2 sm:px-4 lg:px-0"
              data-testid="destinations-hero-gallery"
            >
              <motion.div
          ...
          {!hasSlides && (
            <div
              className="order-2 lg:order-2 relative px-2 sm:px-4 lg:px-0"
              data-testid="destinations-hero-placeholder"
            >
              <motion.div
                className="relative w-full max-w-lg mx-auto flex flex-col lg:h-[65vh] lg:min-h-[500px]"
                initial={shouldAnimate ? { opacity: 0, x: 40 } : {}}
