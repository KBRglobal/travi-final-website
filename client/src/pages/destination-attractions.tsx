...
import { 
  MapPin, Star, Clock, Users, X, ChevronRight, ChevronLeft,
  Ticket, Building2, Camera, TreePine, Music, Landmark,
  Sparkles, Lightbulb, Waves, Ship, Mountain,
  Calendar, Train, CreditCard, Eye, ArrowUp, HelpCircle,
  ChevronDown, Loader2, Timer, Globe2
} from "lucide-react";
...
function inferCategory(title: string, venueName: string): string {
  ...
  if (combined.includes("tower") || combined.includes("observation") || combined.includes("view")) return "observation-deck";
  if (combined.includes("aquarium") || combined.includes("sea life")) return "aquarium";
  if (combined.includes("zoo") || combined.includes("safari") || combined.includes("wildlife")) return "zoo";
  ...
  if (combined.includes("show") || combined.includes("theatre") || combined.includes("theater")) return "show";
  if (combined.includes("tour") || combined.includes("walking")) return "tour";
  if (combined.includes("garden") || combined.includes("park") || combined.includes("botanical")) return "garden";
  if (combined.includes("church") || combined.includes("cathedral") || combined.includes("mosque") || combined.includes("temple")) return "church";
  if (combined.includes("palace") || combined.includes("castle") || combined.includes("fort")) return "landmark";
...
function transformApiResponse(
  ...
  const categoryStats = Object.entries(categoryGroups).map(([cat, attractions]) => {
  ...
  const top10 = transformedAttractions.filter(a => a.topRank).sort((a, b) => (a.topRank || 99) - (b.topRank || 99));
  ...
  return {
    ...
    heroImage: DESTINATION_HERO_IMAGES[slug] || "https://images.unsplash.com/photo-1488085061387-422e29b40080?w=1600",
    ...
    faq: DESTINATION_FAQS[slug] || [
      { question: `How many days do I need in ${destinationInfo.name}?`, answer: `Plan 3-5 days to see the major attractions in ${destinationInfo.name}.` },
      { question: `What are the best attractions in ${destinationInfo.name}?`, answer: `The top attractions include museums, landmarks, and unique local experiences.` },
      { question: `Do I need to book tickets in advance?`, answer: `Yes, advance booking is recommended for popular attractions to skip the lines.` }
    ]
  };
}
...
function getCategoryIcon(category: string) {
  switch (category) {
    ...
    case "observation-deck": return Eye;
    case "theme-park": return Sparkles;
    case "water-park": return Waves;
    case "aquarium": return Waves;
    case "zoo": return TreePine;
    case "tour": return Camera;
    case "cruise": return Ship;
    case "show": return Music;
    case "experience": return Sparkles;
    case "desert-safari": return Mountain;
...
function matchesDuration(attractionDuration: string, filter: string): boolean {
  ...
  if (filter === "medium") return hours.includes("1-2") || hours.includes("2-3") || hours.includes("2 hour");
  if (filter === "long") return hours.includes("3-4") || hours.includes("4-6") || hours.includes("full day") || hours.includes("3+");
  return true;
}
              ...
              <div className="container mx-auto px-4 md:px-6 lg:px-8">
                <div className="max-w-2xl mx-auto text-center">
                  ...
                  <p className="text-lg text-slate-600 dark:text-slate-400 mb-8">
                    We don't have attraction data for {cityName} yet. Check back soon or explore other destinations.
                  </p>
    ...
    <>
      ...
      <div className="min-h-screen bg-white dark:bg-slate-950">
        ...
        <main className="relative z-10">
          ...
          <section className="relative pt-24 pb-16 md:pt-32 md:pb-24" data-testid="section-destination-hero">
            <div className="container mx-auto px-4 md:px-6 lg:px-8">
              {/* Breadcrumbs */}
              <motion.nav 
              ...
              >
                ...
                <p className="text-xl md:text-2xl text-slate-600 dark:text-slate-400 mb-8 max-w-3xl">
                  Discover {data.totalAttractions}+ things to do in {data.name} — from iconic landmarks to hidden gems
                </p>
                ...
                <div className="flex flex-wrap items-center gap-4 md:gap-6">
                  <div className="flex items-center gap-2 px-4 py-2 rounded-full bg-[#6443F4]/10 border border-[#6443F4]/20">
                    ...
                    <span className="font-semibold text-slate-900 dark:text-white">{data.totalAttractions}</span>
                    <span className="text-slate-600 dark:text-slate-400">Attractions</span>
                  </div>
                  <div className="flex items-center gap-2 px-4 py-2 rounded-full bg-[#6443F4]/10 border border-[#6443F4]/20">
                    <Globe2 className="w-5 h-5 text-[#6443F4]" />
                    <span className="font-semibold text-slate-900 dark:text-white">{data.categoryStats.length}</span>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="text-left">
                        ...
                        <div className="text-sm text-slate-600 dark:text-slate-400 whitespace-nowrap">{stat.category}</div>
                      </div>
                    </motion.button>
                  );
                })}
            ...
            <div className="container mx-auto px-4 md:px-6 lg:px-8">
              ...
              {filteredAttractions.length === 0 ? (
                <div className="text-center py-16">
                  ...
                  <p className="text-lg text-slate-600 dark:text-slate-400 mb-4">No attractions match your filters</p>
                  <Button 
                    onClick={clearFilters} 
                    variant="outline"
                          ...
                          <CardContent className="p-5">
                            <div className="flex items-center gap-4 mb-3">
                              <div className="flex items-center gap-1">
                                ...
                                <span className="font-semibold text-slate-900 dark:text-white">{attraction.rating}</span>
            ...
            <div className="container mx-auto px-4 md:px-6 lg:px-8 text-center">
              ...
              <div className="flex flex-wrap justify-center gap-4">
                <Button 
                  ...
                  onClick={() => document.getElementById('section-attractions-grid')?.scrollIntoView({ behavior: 'smooth' })}
