  ...
  const debugMutation = useMutation({
    mutationFn: async (query: string) => {
      const response = await apiRequest(
        "GET",
        `/api/admin/search/debug?q=${encodeURIComponent(query)}`
      );
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap gap-1 mt-1">
                          {debugResult.queryAnalysis.queryExpansion.synonymsApplied.length > 0 ? (
                            debugResult.queryAnalysis.queryExpansion.synonymsApplied.map(
                              (syn, i) => (
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          ...
                          <p className="font-mono text-sm">
                            {debugResult.queryAnalysis.spellCheck.corrected}
                          </p>
                        </div>
                        <div>
                          <p className="text-xs text-muted-foreground">Confidence</p>
                          <p className="text-sm">
                            {(debugResult.queryAnalysis.spellCheck.confidence * 100).toFixed(0)}%
                          </p>
                        </div>
                      </div>
                  ...
                  <CardContent className="space-y-4">
                    <div className="flex items-center gap-4">
                      <div>
                        ...
                        <Badge
                          variant={getIntentBadgeColor(debugResult.intentClassification.primary)}
                          className="text-lg"
                        >
                        ...
                        <div className="flex items-center gap-2">
                          <div className="w-32 h-2 bg-muted rounded-full overflow-hidden">
                            <div
                              ...
                              style={{
                                width: `${debugResult.intentClassification.confidence * 100}%`,
                              }}
                            />
                          </div>
                          <span className="text-sm">
                            {(debugResult.intentClassification.confidence * 100).toFixed(0)}%
                          </span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      {Object.keys(debugResult.intentClassification.extractedEntities).length >
                      0 ? (
                        <div className="font-mono text-sm bg-muted p-3 rounded-lg">
                          {JSON.stringify(
                            debugResult.intentClassification.extractedEntities,
                            null,
                            2
                          )}
                        </div>
                    ...
                    <div>
                      ...
                      {Object.keys(debugResult.intentClassification.suggestedFilters).length > 0 ? (
                        <div className="font-mono text-sm bg-muted p-3 rounded-lg">
                          {JSON.stringify(
                            debugResult.intentClassification.suggestedFilters,
                            null,
                            2
                          )}
                        </div>
              ...
              <TabsContent value="results" className="space-y-4">
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>Detailed scoring breakdown for each result</CardDescription>
                  </CardHeader>
                  <CardContent>
                          ...
                          debugResult.results.map((result, index) => (
                            ...
                            >
                              ...
                              <div className="grid grid-cols-2 md:grid-cols-6 gap-3 text-center">
                                <div>
                                  ...
                                  <p className="font-medium">
                                    {result.scoreBreakdown.typeWeight.toFixed(1)}x
                                  </p>
                                </div>
                                <div>
                                  <p className="text-xs text-muted-foreground">Title</p>
                                  <p className="font-medium">
                                    {result.scoreBreakdown.titleMatchBoost.toFixed(1)}x
                                  </p>
                                </div>
                                <div>
                                  <p className="text-xs text-muted-foreground">Recency</p>
                                  <p className="font-medium">
                                    {result.scoreBreakdown.recencyBoost.toFixed(2)}x
                                  </p>
                                </div>
                                <div>
                                  <p className="text-xs text-muted-foreground">Popularity</p>
                                  <p className="font-medium">
                                    {result.scoreBreakdown.popularityBoost.toFixed(2)}x
                                  </p>
                                </div>
                                <div>
                                  <p className="text-xs text-muted-foreground">Intent</p>
                                  <p className="font-medium">
                                    {result.scoreBreakdown.intentBoost.toFixed(1)}x
                                  </p>
                                </div>
                                <div>
                                  <p className="text-xs text-muted-foreground">Total</p>
                                  <p className="font-bold text-primary">
                                    {result.scoreBreakdown.totalMultiplier.toFixed(2)}x
                                  </p>
                                </div>
                              </div>
                              ...
                              <div className="mt-3 flex items-center gap-2 flex-wrap">
                                ...
                                {result.matchDetails.titleContains &&
                                  !result.matchDetails.titleMatch && (
                  ...
                  <CardContent>
                    <div className="space-y-4">
                      {rankingFactors?.factors &&
                        Object.entries(rankingFactors.factors).map(([key, value]) => (
