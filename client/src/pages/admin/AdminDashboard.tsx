...
function ClickableStatsCard({
  ...
  return (
    <Card
      ...
      className={cn(
        "border-[hsl(var(--admin-border-subtle))] transition-all duration-150",
        href && "cursor-pointer hover:border-[hsl(var(--admin-border-hover))] hover:shadow-sm"
      )}
      onClick={href ? handleClick : undefined}
    >
      <CardHeader className="flex flex-row items-center justify-between gap-2 pb-2">
        <CardTitle className="text-[13px] font-medium text-[hsl(var(--admin-text-secondary))]">
          {title}
        </CardTitle>
        <Icon className="h-4 w-4 text-[hsl(var(--admin-text-muted))]" />
      </CardHeader>
      <CardContent>
        <div className="text-xl font-semibold text-[hsl(var(--admin-text))]">{value}</div>
        {description && (
          <p className="text-[11px] text-[hsl(var(--admin-text-muted))] mt-1 line-clamp-1">
            {description}
          </p>
        )}
        {href && (
          <div className="flex items-center gap-1 text-[11px] text-[hsl(var(--admin-text-secondary))] mt-2 hover:text-[hsl(var(--admin-text))]">
            <span>View</span>
...
export default function AdminDashboard() {
  ...
  return (
    <div className="space-y-8" data-testid="admin-dashboard">
      {/* Clean Linear/Notion-style header */}
      <div className="flex items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-semibold text-[hsl(var(--admin-text))]">Dashboard</h1>
          <p className="text-[13px] text-[hsl(var(--admin-text-secondary))] mt-0.5">
            Overview of your content and system health
          </p>
        </div>
        <Button
          variant="outline"
          ...
          className="h-8 px-3 text-[13px] border-[hsl(var(--admin-border))] hover:bg-[hsl(var(--admin-surface-hover))]"
        >
          <RefreshCw
            className={cn(
              "h-3.5 w-3.5 mr-1.5",
              (statsLoading || activityLoading) && "animate-spin"
            )}
          />
          Refresh
        </Button>
      ...
      <div className="space-y-4">
        <h2 className="text-[13px] font-medium tracking-wide text-[hsl(var(--admin-text-muted))] uppercase">
          Overview
        </h2>
        ...
        <div className="lg:col-span-2">
          <Card className="border-[hsl(var(--admin-border-subtle))]">
            <CardHeader className="pb-3">
              <CardTitle className="text-[15px] font-medium text-[hsl(var(--admin-text))] flex items-center gap-2">
                <Activity className="h-4 w-4 opacity-60" />
                Recent Activity
              </CardTitle>
              <CardDescription className="text-[13px] text-[hsl(var(--admin-text-muted))]">
              ...
              <div className="grid grid-cols-1 gap-1">
                {quickActions.map(action => (
                  <Button
                    key={action.href}
                    variant="ghost"
                    className="w-full justify-start gap-2.5 h-9 px-2 text-[13px] text-[hsl(var(--admin-text-secondary))] hover:text-[hsl(var(--admin-text))] hover:bg-[hsl(var(--admin-surface-hover))]"
                    asChild
                    data-testid={action.testId}
                  >
                    <Link href={action.href}>
                      <action.icon className="h-4 w-4 opacity-60" />
