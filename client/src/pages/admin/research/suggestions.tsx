...
const CONTENT_TYPE_CONFIG: Record<string, { label: string; icon: typeof FileText; color: string }> =
  {
    article: {
      label: "Articles",
      icon: FileText,
    ...
    district: {
      ...
      icon: Building2,
      color:
        "bg-[#6443F4]/10 dark:bg-[#6443F4]/20 text-[#6443F4] dark:text-[#6443F4] border-[#6443F4]/30 dark:border-[#6443F4]/30",
    },
    event: {
      label: "Events",
      icon: Calendar,
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {suggestion.category && (
              <Badge
                variant="secondary"
                className="text-xs"
                data-testid={`badge-category-${suggestion.id}`}
              >
                {suggestion.category}
              </Badge>
            )}
      ...
      <CardContent className="space-y-4">
        ...
        {suggestion.priority !== null && (
          <div className="space-y-1.5">
            <div className="flex items-center justify-between text-xs">
              ...
              <span className="font-medium" data-testid={`text-priority-${suggestion.id}`}>
        ...
        {keyPoints.length > 0 && (
          <Collapsible open={keyPointsOpen} onOpenChange={setKeyPointsOpen}>
            ...
            <CollapsibleContent className="pt-2">
              <ul
                className="space-y-1.5 text-sm text-muted-foreground pl-4"
                data-testid={`list-keypoints-${suggestion.id}`}
              >
        ...
        {suggestion.sourceExcerpt && (
          <Collapsible open={excerptOpen} onOpenChange={setExcerptOpen}>
            ...
            <CollapsibleContent className="pt-2">
              <p
                className="text-xs text-muted-foreground italic bg-muted p-3 rounded-md"
                data-testid={`text-excerpt-${suggestion.id}`}
              >
                "{suggestion.sourceExcerpt}"
              </p>
            </CollapsibleContent>
        ...
        {suggestion.rejectionReason && (
          <div
            className="text-xs text-rose-700 dark:text-rose-300 bg-rose-100 dark:bg-rose-900 p-2 rounded-md"
            data-testid={`text-rejection-reason-${suggestion.id}`}
          >
...
export default function SuggestionsReviewPage() {
  ...
  const updateStatusMutation = useMutation({
    mutationFn: async ({
      ...
      const res = await apiRequest("PATCH", `/api/research-suggestions/${suggestionId}/status`, {
        status,
        rejectionReason,
      });
      return res.json();
  ...
  const handleGenerate = (suggestion: ContentSuggestion) => {
    ...
    toast({
      title: "Single generation not yet implemented",
      description: "Use 'Generate All Approved' instead",
    });
  };
  ...
  const filteredSuggestions = activeTab === "all" ? suggestions : contentTypes[activeTab] || [];
      ...
      <div className="max-w-6xl mx-auto space-y-8">
        <header className="space-y-4">
          ...
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
              ...
              <div className="flex flex-wrap items-center gap-3">
                ...
                >
                  ...
                  <span className="sm:hidden">
                    {STATUS_CONFIG[research.status as SuggestionStatus]?.label || research.status}
                  </span>
                </Badge>
              </div>
              <p className="text-muted-foreground">
                Review and approve content suggestions extracted from your research
              </p>
            </div>
            ...
            >
              ...
              Generate All Approved ({stats.approved})
            </Button>
          </div>
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-md bg-emerald-100 dark:bg-emerald-900">
              ...
              <span className="text-sm text-emerald-700 dark:text-emerald-300">Approved:</span>
              <span
            ...
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-md bg-rose-100 dark:bg-rose-900">
              ...
              <span className="text-sm text-rose-700 dark:text-rose-300">Rejected:</span>
              <span
            ...
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-md bg-[#6443F4]/10 dark:bg-[#6443F4]/20">
              ...
              <span className="text-sm text-[#6443F4] dark:text-[#6443F4]">Generated:</span>
              <span
                className="font-semibold text-[#6443F4] dark:text-[#6443F4]"
        ...
        <div className="p-4 bg-muted rounded-lg border">
          ...
          <p className="text-sm text-muted-foreground">
            Review the generated content suggestions. You can approve, reject, or edit each
            suggestion. Once approved, click "Generate All" to create the full content.
          </p>
        </div>

        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
          <TabsList
            className="flex-wrap h-auto gap-1 p-1.5 bg-muted"
            data-testid="tabs-contents-types"
          >
        ...
        <Dialog open={editDialogOpen} onOpenChange={setEditDialogOpen}>
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                Modify the suggestion details before generating content.
              </DialogDescription>
            </DialogHeader>
