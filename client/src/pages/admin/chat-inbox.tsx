...
export default function AdminChatInbox() {
  ...
  const sendMessage = useMutation({
    mutationFn: async (contents: string) => {
      const res = await apiRequest("POST", `/api/admin/live-chat/conversations/${selectedConversation}/messages`, {
        contents,
      });
      return res.json();
    },
  ...
  return (
    <div className="p-6 h-[calc(100vh-100px)]" data-testid="admin-chat-inbox">
      <div className="flex items-center gap-3 mb-6">
        <MessageCircle className="w-8 h-8 text-[#7B4BA4]" />
        <div>
          <h1 className="text-2xl font-bold" data-testid="text-page-title">Live Chat Inbox</h1>
          <p className="text-muted-foreground text-sm">Respond to visitor messages</p>
        </div>
      </div>
                ...
                <div className="flex gap-2">
                  {activeConversation.status === "open" && (
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => updateStatus.mutate({ id: activeConversation.id, status: "closed" })}
                  ...
                  {activeConversation.status === "closed" && (
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => updateStatus.mutate({ id: activeConversation.id, status: "archived" })}
