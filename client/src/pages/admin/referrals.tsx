...
import { Loader2, Users, MousePointer2, DollarSign, TrendingUp, Settings, Trash2, Eye } from "lucide-react";
...
export default function AdminReferrals() {
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ code, commissionRate, isActive }: { code: string; commissionRate: number; isActive: boolean }) => {
      const res = await apiRequest("PATCH", `/api/referrals/admin/${code}`, { commissionRate, isActive });
      return res.json();
    },
    onSuccess: () => {
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">Manage partners, view clicks, and track conversions.</p>
      </div>
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-2xl font-bold" data-testid="stat-partners">{partners?.length || 0}</p>
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-2xl font-bold" data-testid="stat-total-clicks">{totalStats.clicks}</p>
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-2xl font-bold" data-testid="stat-total-conversions">{totalStats.conversions}</p>
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-2xl font-bold" data-testid="stat-total-commission">${totalStats.commission.toFixed(2)}</p>
              ...
              <TableBody>
                {partners.map((partner) => (
                  <TableRow key={partner.id} data-testid={`row-partner-${partner.id}`}>
                    ...
                    <TableCell>
                      <code className="text-sm bg-muted px-2 py-1 rounded" data-testid={`text-code-${partner.id}`}>
                        {partner.code}
                      </code>
                    </TableCell>
      ...
      <Dialog open={editDialogOpen} onOpenChange={setEditDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Update settings for {selectedPartner?.name}
            </DialogDescription>
          </DialogHeader>
              ...
              {clicks && clicks.length > 0 ? (
                <div className="space-y-2">
                  {clicks.slice(0, 10).map((click) => (
                    <div key={click.id} className="text-sm p-2 bg-muted rounded" data-testid={`detail-click-${click.id}`}>
              ...
              {conversions && conversions.length > 0 ? (
                <div className="space-y-2">
                  {conversions.slice(0, 10).map((conv) => (
                    <div key={conv.id} className="text-sm p-2 bg-muted rounded" data-testid={`detail-conversion-${conv.id}`}>
                      <div className="flex items-center justify-between">
