...
function LivePricingWidget({ attraction, onAffiliateClick }: { attraction: TiqetsAttraction; onAffiliateClick: () => void }) {
  const highlights = attraction.highlights || attraction.tiqetsHighlights || [];
  const hasFreeCancellation = attraction.cancellationPolicy?.toLowerCase().includes('free') || 
                    ...
                    <div className="flex items-start justify-between gap-4 mb-4">
                      <div className="flex-1">
                        ...
                        <p className="text-slate-600 dark:text-slate-400 text-sm leading-relaxed">
                          {ticket.description || "Experience this amazing attraction with flexible booking options."}
                        </p>
                      </div>
                    </div>
...
function IntroductionSection({ attraction }: { attraction: TiqetsAttraction }) {
  ...
  const placeholderIntro = description.split('\n')[0] || 
          ...
          <CardContent className="p-6 md:p-8">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-6 md:gap-8">
              {facts.map((fact, index) => {
                ...
                return (
                  <div key={index} className="flex items-start gap-3" data-testid={`quick-fact-${index}`}>
...
function TicketOptionsTable({ attraction, onAffiliateClick }: { attraction: TiqetsAttraction; onAffiliateClick: () => void }) {
  const ticketOptions = [
    {
      type: "Standard Entry",
              ...
              {ticketOptions.map((ticket, index) => (
                <tr 
                  ...
                  className={`border-b border-slate-200 dark:border-slate-700 transition-colors ${
                    ticket.popular ? 'bg-[#6443F4]/5 dark:bg-[#6443F4]/10' : 'hover:bg-slate-50 dark:hover:bg-slate-800/30'
                  }`}
                ...
                >
                  ...
                  <td className="py-4 px-5">
                    {attraction.productUrl ? (
                      ...
                      <span className="text-slate-400 dark:text-slate-500 text-sm">Coming soon</span>
                    )}
                  </td>
                </tr>
              ))}
  ...
  const defaultHighlights: AIContentItem[] = [
    {
      ...
      description: "Experience breathtaking panoramic views that stretch across the city skyline and beyond. Perfect for photography enthusiasts and first-time visitors alike."
    },
    {
      icon: "Star",
      title: "World-Class Experience",
      description: "Immerse yourself in a carefully curated experience designed to engage all your senses. Interactive exhibits and informative displays enhance your visit."
    },
    {
      icon: "Camera",
      title: "Memorable Moments",
      description: "Create lasting memories with unique photo opportunities and exclusive experiences. Every corner offers something new to discover and appreciate."
    }
  ];

  const highlights = (Array.isArray(aiContent?.whatToExpect) && aiContent.whatToExpect.length > 0) 
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="grid md:grid-cols-3 gap-6 mb-12">
          {highlights.map((item, index) => {
            const image = tiqetsImages[index + 1]?.large || tiqetsImages[index + 1]?.medium || "/cards/dubai.webp";
            return (
              <Card 
                key={index}
  ...
  const defaultTips: AIContentItem[] = [
    {
      ...
      description: "Avoid peak sunset hours (5-7 PM). Go for early morning (8-10 AM) for smaller crowds. Weekday mornings are ideal for a relaxed experience."
    },
    {
      icon: "Lightbulb",
      title: "Pro Tips",
      description: "Arrive 15 minutes early for security screening. Wear comfortable shoes for walking. Bring a camera and portable charger."
    },
    {
      icon: "Calendar",
      title: "Book Ahead",
      description: "Book online to skip the ticket line. Consider combo tickets to bundle with nearby attractions. Morning slots typically have shorter waits."
    }
  ];

  const tips = (Array.isArray(aiContent?.visitorTips) && aiContent.visitorTips.length > 0) 
...
function HowToGetThereSection({ attraction }: { attraction: TiqetsAttraction }) {
  ...
  const defaultTransport: AIContentTransport[] = [
    ...
    },
    {
      ...
      details: "15-30 minutes from city center depending on traffic. Drop-off at main entrance. Uber and Careem available."
    },
    {
      mode: "Car",
  ...
  const transportOptions = (Array.isArray(aiContent?.howToGetThere?.transport) && aiContent.howToGetThere.transport.length > 0) 
  ...
  return (
    ...
    >
      <div className="max-w-5xl mx-auto">
        ...
        <div className="aspect-video bg-slate-200 dark:bg-slate-700 rounded-2xl mb-10 overflow-hidden shadow-lg" data-testid="map-placeholder">
                ...
                <Card className="overflow-hidden border-slate-200 dark:border-slate-700 transition-all duration-300 hover:shadow-xl hover:-translate-y-1 hover:border-[#6443F4]/30">
                  <div className="aspect-[4/3] overflow-hidden relative">
                    ...
                    {attraction.primaryCategory && (
                      <Badge 
                        className="absolute top-3 left-3 bg-[#6443F4]/10 backdrop-blur-sm text-[#6443F4] border-[#6443F4]/20"
                      >
                        {attraction.primaryCategory}
                      </Badge>
                    )}
...
function StickyBookingCTA({ attractionName, productUrl, onAffiliateClick }: { attractionName: string; productUrl?: string | null; onAffiliateClick: () => void }) {
...
export default function AttractionDetail() {
  ...
  if (error || !data?.attraction) {
    return (
      <AttractionError 
        message={error instanceof Error ? error.message : "The attraction you're looking for doesn't exist or has been removed."} 
      />
    );
  }
  ...
  const metaDescription = attraction.metaDescription || 
  ...
  const tiqetsImages = (attraction.tiqetsImages as TiqetsImage[]) || [];
  const heroImage = tiqetsImages[0]?.extra_large || 
  ...
  const breadcrumbSchema = {
    ...
    "itemListElement": [
      ...
      { "@type": "ListItem", "position": 3, "name": attraction.cityName, "item": `https://travi.world/destinations/${destination.toLowerCase()}` },
      { "@type": "ListItem", "position": 4, "name": `${attraction.cityName} Attractions`, "item": `https://travi.world/attractions/list/${destination.toLowerCase()}` },
    ...
    <>
      ...
      <main className="min-h-screen pb-24 bg-background">
        ...
        <div className="relative">
          ...
          <FAQSection 
            faqs={attraction.faqs as FAQItem[] | null} 
      ...
      <StickyBookingCTA attractionName={title} productUrl={attraction.productUrl} onAffiliateClick={trackAffiliateClick} />
      <div className="h-20" />
      <PublicFooter />
    </>
  );
