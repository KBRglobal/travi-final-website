...
export const internalLinks = pgTable("internal_links", {
  id: varchar("id")
    .primaryKey()
    .default(sql`gen_random_uuid()`),
  sourceContentId: varchar("source_content_id").references(() => contents.id, {
    onDelete: "cascade",
  }),
  targetContentId: varchar("target_content_id").references(() => contents.id, {
    onDelete: "cascade",
  }),
...
// ============================================================================
// NATIVE CONTENT ARCHITECTURE - 30 Languages Support
// ============================================================================
// Expanded from pilot (en, ar, fr) to full 30-language support
// Native content generation (not translation) with locale purity validation
// ============================================================================

// Entity types supported for native content
...
// Native localized content table - supports all 30 locales
...
// ============================================================================
// SEO Keywords Table - Locale-specific keywords by destination
// ============================================================================
...
export const customReports = pgTable("custom_reports", {
  ...
  visualization: jsonb("visualization")
    .$type<{ type: string; config: Record<string, any> }>()
    .notNull(),
  isPublic: boolean("is_public").default(false),
  createdBy: varchar("created_by").references(() => users.id),
  createdAt: timestamp("created_at").defaultNow(),
...
export const pageSections = pgTable(
  "page_sections",
  {
    ...
    pageId: varchar("page_id")
      .notNull()
      .references(() => editablePages.id, { onDelete: "cascade" }),
...
export const octopusJobsRelations = relations(octopusJobs, ({ one, many }) => ({
  destination: one(destinations, {
    fields: [octopusJobs.destinationId],
    references: [destinations.id],
  }),
...
export const contentHighlightTranslationsRelations = relations(
  contentHighlightTranslations,
  ({ one }) => ({
    highlight: one(contentHighlights, {
      fields: [contentHighlightTranslations.highlightId],
...
export const attractionDistrictsRelations = relations(attractionDistricts, ({ one }) => ({
  attraction: one(attractions, {
    fields: [attractionDistricts.attractionId],
    references: [attractions.id],
  }),
  district: one(districts, {
    fields: [attractionDistricts.districtId],
...
export const travelAdvisoriesRelations = relations(travelAdvisories, ({ one }) => ({
  destination: one(destinations, {
    fields: [travelAdvisories.destinationId],
    references: [destinations.id],
  }),
}));
...
export const healthAlertsRelations = relations(healthAlerts, ({ one }) => ({
  destination: one(destinations, {
    fields: [healthAlerts.destinationId],
    references: [destinations.id],
  }),
}));
...
export const destinationEventsRelations = relations(destinationEvents, ({ one }) => ({
  destination: one(destinations, {
    fields: [destinationEvents.destinationId],
    references: [destinations.id],
  }),
}));
...
// ============================================
// OCTYPO PHASE 2: CONTENT EXPLODER TABLES
// ============================================

// Entity types for content explosion
...
export const explodedArticles = pgTable(
  "exploded_articles",
  {
    ...
    entityId: varchar("entity_id").references(() => contentEntities.id, { onDelete: "set null" }),
    generatedContentId: varchar("generated_content_id").references(() => contents.id, {
      onDelete: "cascade",
    }),
...
// ============================================
// OCTYPO PHASE 3: REAL AUTOPILOT TABLES
// ============================================
...
// Task status for approval workflow
...
// ============================================
// OCTYPO PHASE 4: VAMS (Visual Asset Management) TABLES
// ============================================
...
export const vamsVariantTypeEnum = pgEnum("vams_variant_type", [
  "hero", // Large hero images (1920px)
  "card", // Card thumbnails (400px)
  "og", // Open Graph (1200x630)
  "thumbnail", // Small thumbnails (200px)
  "gallery", // Gallery size (800px)
  "mobile", // Mobile optimized (640px)
]);
...
// ============================================
// OCTYPO PHASE 5: SEO ENHANCEMENT TABLES
// ============================================
