...
export function registerInternalCompetitionRoutes(app: Express): void {
  ...
  app.get("/api/admin/competition/analyze/:contentId", requireAuth, async (req: Request, res: Response) => {
  ...
  // Get cached analysis
  app.get("/api/admin/competition/cached/:contentId", requireAuth, async (req: Request, res: Response) => {
  ...
  app.patch("/api/admin/competition/pair/:pairId/resolve", requireAuth, async (req: Request, res: Response) => {
    if (!isInternalCompetitionEnabled()) {
      return res.json({ enabled: false, message: "Internal competition detection disabled" });
    }
  ...
  console.log("[InternalCompetition] Routes registered");
}
