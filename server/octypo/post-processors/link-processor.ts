...
export async function processContentLinks(
  ...
  try {
    ...
    logger.info(`LinkProcessor completed for ${content.id}`, {
    ...
    logger.error('LinkProcessor failed', { contentId: content.id, error: message });
...
async function saveLinksToDatabase(
  ...
  try {
    ...
    logger.warn('Failed to save links to database', {
...
export async function getContentLinkStats(contentId: string): Promise<{
  ...
  try {
    ...
    logger.error('Failed to get link stats', { contentId, error });
...
export async function batchProcessLinks(
  ...
  for (let i = 0; i < contentIds.length; i += concurrency) {
    ...
    const contentRecords = await db
      ...
      .from(contents)
      .where(sql`${contents.id} = ANY(${batch})`);
