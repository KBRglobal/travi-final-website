...
const VALIDATOR_PERSONAS: AgentPersona[] = [
  ...
  },
  {
    ...
    expertise: ['safety warnings', 'accessibility info', 'health considerations', 'travel advisories'],
...
export class ValidatorAgent extends BaseAgent {
  ...
  private performDataValidation(content: GeneratedAttractionContent): ValidationResult {
    ...
    if (titleLength < BLUEPRINT_REQUIREMENTS.metaTitle.min || titleLength > BLUEPRINT_REQUIREMENTS.metaTitle.max) {
      issues.push({
        ...
        fix: titleLength < BLUEPRINT_REQUIREMENTS.metaTitle.min 
    ...
    if (descLength < BLUEPRINT_REQUIREMENTS.metaDescription.min || descLength > BLUEPRINT_REQUIREMENTS.metaDescription.max) {
      issues.push({
        ...
        fix: descLength < BLUEPRINT_REQUIREMENTS.metaDescription.min 
  ...
  private async performLLMValidation(task: ValidationTask): Promise<ValidationResult> {
    const systemPrompt = this.persona.systemPrompt + `
...
export function initializeValidatorAgents(): void {
  ...
  console.log(`[ValidatorAgents] Initialized ${VALIDATOR_PERSONAS.length} validator agents`);
}
