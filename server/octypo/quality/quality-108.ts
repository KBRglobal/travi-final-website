...
const BANNED_PHRASES = [
  ...
  'once-in-a-lifetime', 'must-see', 'world-class', 'iconic'
];

const CONTRACTIONS = ["you'll", "it's", "don't", "won't", "can't", "we'll", "they'll", "you're", "we're", "that's", "here's", "there's"];
...
function checkTraviAuthenticity(content: GeneratedAttractionContent): QualityCategoryResult {
  const maxPoints = QUALITY_CATEGORIES.travi_authenticity.maxPoints; // 6
  const issues: string[] = [];
  let score = maxPoints;
  ...
  const hasBrandVoice = fullText.includes("here's the thing") || 
  ...
  if (directAddress < 5) { // Reduced from 10
    score -= 1; // Reduced from -2
...
function checkHumanization(content: GeneratedAttractionContent): QualityCategoryResult {
  ...
  if (contractionCount < 3) { // Reduced from 5
    score -= 2;
  ...
  if (bannedFound.length > 2) { // Allow up to 2 banned phrases
  ...
  const conversationalBridges = ["but", "however", "actually", "honestly", "look,", "here's", "though", "still"].filter(
    b => fullText.includes(b)
  ).length;
  if (conversationalBridges < 2) { // Reduced from 3
    score -= 1; // Reduced from -2
...
function checkSensoryImmersion(content: GeneratedAttractionContent): QualityCategoryResult {
  const maxPoints = QUALITY_CATEGORIES.sensory_immersion.maxPoints; // 12
  const issues: string[] = [];
  let score = maxPoints;
  ...
  const sightWords = ['see', 'view', 'watch', 'gaze', 'glimpse', 'colors', 'light', 'golden', 'glitter'].filter(
    w => fullText.includes(w)
  ).length;
  
  const soundWords = ['hear', 'sound', 'echo', 'music', 'silence', 'murmur', 'buzz'].filter(
    w => fullText.includes(w)
  ).length;
  
  const feelWords = ['feel', 'touch', 'breeze', 'cool', 'warm', 'humid', 'texture'].filter(
    w => fullText.includes(w)
  ).length;
  
  const smellWords = ['smell', 'aroma', 'scent', 'fragrance', 'spice', 'incense'].filter(
    w => fullText.includes(w)
  ).length;
  
  const totalSensory = sightWords + soundWords + feelWords + smellWords;
  
  if (totalSensory < 6) { // Reduced from 10
...
function checkQuotesSources(content: GeneratedAttractionContent): QualityCategoryResult {
  const maxPoints = QUALITY_CATEGORIES.quotes_sources.maxPoints; // 10
  const issues: string[] = [];
  let score = maxPoints;
  ...
  const hasSpecificNumbers = (fullText.match(/\d+\s*(minute|hour|meter|floor|year|percent|%|km|miles?|feet)/gi) || []).length;
  if (hasSpecificNumbers < 3) { // Reduced from 5
    score -= 2; // Reduced from -3
    issues.push(`Low specific numbers: ${hasSpecificNumbers}/5 target`);
  }
  
  return {
    category: 'quotes_sources',
    score: Math.max(0, score),
    maxPoints,
    passed: score >= maxPoints * 0.5, // Relaxed from 0.6
    issues
...
function checkCulturalDepth(content: GeneratedAttractionContent): QualityCategoryResult {
  ...
  const culturalTerms = ['tradition', 'culture', 'heritage', 'history', 'local', 'authentic', 
                         'custom', 'ritual', 'ceremony', 'architecture', 'art', 'cuisine'].filter(
  ...
  const hasRespectfulTone = !fullText.includes('weird') && 
                            !fullText.includes('strange') && 
                            !fullText.includes('exotic');
...
function checkEngagement(content: GeneratedAttractionContent): QualityCategoryResult {
  ...
  const hasCallToAction = fullText.toLowerCase().includes('book') || 
  ...
  const hasTips = fullText.toLowerCase().includes('tip:') || 
...
function checkVoiceTone(content: GeneratedAttractionContent): QualityCategoryResult {
  ...
  const hasActiveVoice = fullText.includes("you'll see") || 
  ...
  const hasWarning = fullText.includes('skip this if') || 
  ...
  const toneConsistency = !fullText.includes('must-visit') && 
...
function checkTechnicalSEO(content: GeneratedAttractionContent): QualityCategoryResult {
  ...
  const hasSchema = content.schemaPayload && 
                    content.schemaPayload['@context'] === 'https://schema.org';
  if (!hasSchema) {
    score -= 4;
...
function checkAEO(content: GeneratedAttractionContent): QualityCategoryResult {
  ...
  if (capsuleWords < 15 || capsuleWords > 80) { // Relaxed from 35-70
    score -= 2; // Reduced from -3
  ...
  const hasFAQSchema = content.schemaPayload?.['@type'] === 'FAQPage' || 
...
function checkPAA(content: GeneratedAttractionContent): QualityCategoryResult {
  ...
  const paaPatterns = ['how much', 'is it worth', 'best time', 'how long', 'do i need', 
                       'what should', 'can i', 'is there'];
  
  const matchedPatterns = paaPatterns.filter(p => 
    faqQuestions.some(q => q.includes(p))
  ).length;
  ...
  const avgAnswerLength = content.faqs.reduce((sum, f) => sum + countWords(f.answer), 0) / 
                          Math.max(1, content.faqs.length);
  
  if (avgAnswerLength < 25 || avgAnswerLength > 90) { // Relaxed from 35-80
    score -= 2; // Reduced from -3
...
function checkCompleteness(content: GeneratedAttractionContent): QualityCategoryResult {
  ...
  const totalWords = countWords(content.introduction) +
...
export function calculateQuality108Score(content: GeneratedAttractionContent): Quality108Score {
  ...
  const criticalIssues = allIssues.filter(i => 
    i.includes('CRITICAL') || i.includes('banned phrases')
  );
  
  const majorIssues = allIssues.filter(i => 
    i.includes('minimum') || i.includes('target') || i.includes('Missing')
  ).filter(i => !criticalIssues.includes(i));
  
  const minorIssues = allIssues.filter(i => 
    !criticalIssues.includes(i) && !majorIssues.includes(i)
  );
