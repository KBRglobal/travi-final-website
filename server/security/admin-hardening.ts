...
function ipMatchesCidr(ip: string, cidr: string): boolean {
  ...
  const normalizedIp =
    ip === "::1" || ip === "::ffff:127.0.0.1" ? "127.0.0.1" : ip.replace("::ffff:", "");
...
export function mandatory2FAMiddleware(req: Request, res: Response, next: NextFunction): void {
  ...
  const userRecord = user.claims?.sub
    ? user
    : { id: user.id, totpEnabled: user.totpEnabled, totpSecret: user.totpSecret };
 ...
 */
export function createAdminHardeningStack(): Array<
  (req: Request, res: Response, next: NextFunction) => void
> {
  return [emergencyKillSwitch, ipAllowlistMiddleware];
}
