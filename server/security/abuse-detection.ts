...
function cleanupStaleEntries(): void {
  ...
  if (notFoundCounts.size > MAX_NOT_FOUND_COUNTS) {
    const keys = Array.from(notFoundCounts.keys()).slice(0, notFoundCounts.size - MAX_NOT_FOUND_COUNTS);
...
function getIpReputation(ip: string): IpReputation {
  ...
  if (reputation.isBlocked && reputation.blockExpiresAt && reputation.blockExpiresAt < new Date()) {
    ...
    console.log(`[AbuseDetection] Block expired for IP ${maskIp(ip)}`);
  }
...
export function recordAbuseEvent(event: Omit<AbuseEvent, 'id'>): AbuseEvent {
  ...
  if (reputation.score <= ABUSE_CONFIG.blockScoreThreshold && !reputation.isBlocked) {
    ...
    console.warn(`[AbuseDetection] Auto-blocked IP ${maskIp(event.ip)} (score: ${reputation.score})`);
  }
  
  // Log the event
  console.log(`[AbuseDetection] ${event.severity.toUpperCase()}: ${event.patternType} from ${maskIp(event.ip)} on ${event.path}`);
...
export function blockIp(ip: string, durationMs: number = ABUSE_CONFIG.blockDuration): void {
  ...
  reputation.score = 0;
  console.log(`[AbuseDetection] Manually blocked IP ${maskIp(ip)} for ${durationMs / 60000} minutes`);
}
...
export function unblockIp(ip: string): void {
  ...
  if (reputation) {
    ...
    console.log(`[AbuseDetection] Unblocked IP ${maskIp(ip)}`);
  }
}
