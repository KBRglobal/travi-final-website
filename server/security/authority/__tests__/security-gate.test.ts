...
describe("SecurityGate", () => {
  ...
  const originalEnv = process.env.ENABLE_SECURITY_AUTHORITY;

  // Reset state before each test
  beforeEach(async () => {
    // Enable security authority for tests
    process.env.ENABLE_SECURITY_AUTHORITY = "true";
  ...
  describe("Red Team: Privilege Escalation", () => {
    it("should block privilege escalation via role spoofing", async () => {
      ...
      // In enforce mode, user_management requires approval
      const decision = await SecurityGate.assertAllowed(request);

      // Even with claimed super_admin, enforce mode requires approval
  ...
  describe("Red Team: Override Abuse", () => {
    ...
    it("should limit override usage count", async () => {
      ...
      if (result.success && result.override) {
        ...
        expect(thirdUse.error).toContain("max uses");
      }
    });
  });
