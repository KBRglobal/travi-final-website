...
function signToken(payload: string): string {
  ...
  return crypto.createHmac(PRE_AUTH_CONFIG.hmacAlgorithm, secret).update(payload).digest("hex");
}
...
export async function consumePreAuthToken(token: string): Promise<boolean> {
  ...
  try {
    const result = await db.delete(preAuthTokens).where(eq(preAuthTokens.tokenHash, tokenHash));
