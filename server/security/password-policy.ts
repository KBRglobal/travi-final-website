...
export function recordDualLockoutFailure(username: string, ip: string): void {
  ...
  if (ipEntry.failedAttempts >= DUAL_LOCKOUT_CONFIG.ip.maxAttempts) {
    ...
    console.log(`[Security] IP lockout triggered for ${ip.substring(0, ip.lastIndexOf('.') + 1)}*** (${ipEntry.failedAttempts} failures)`);
  }
  ...
  if (highFailureCount || (multiIpAttack && userEntry.failedAttempts >= 10)) {
    ...
    console.log(`[Security] Username lockout triggered for ${normalizedUsername.substring(0, 3)}*** (${userEntry.failedAttempts} failures from ${userEntry.failingIps.size} IPs)`);
  }
 ...
 * @param ip - The IP address of the request
 */
export function checkDualLockout(username: string, ip: string): {
  locked: boolean;
