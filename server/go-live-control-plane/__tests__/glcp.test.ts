...
describe('Capability Registry', () => {
  beforeEach(async () => {
    const { clearCapabilityStore, discoverCapabilities, registerCapabilities } = await import('../capabilities/registry');
...
describe('Conflict Detector', () => {
  ...
  it('should check for blocking conflicts', async () => {
    ...
    const noBlockers = hasBlockingConflicts([
      { type: 'dependency_missing', severity: 'warning', affectedCapabilities: [], message: 'test' },
    ]);
...
describe('Rollback', () => {
  ...
  it('should check if can rollback', async () => {
    ...
    const result = canRollback({
      ...
      durationMs: 100,
      steps: [
        { id: 's1', order: 0, capabilityId: 'c1', capabilityName: 'Cap 1', action: 'enable', status: 'success', rollbackable: true },
        { id: 's2', order: 1, capabilityId: 'c2', capabilityName: 'Cap 2', action: 'enable', status: 'success', rollbackable: true },
      ],
      success: true,
      errors: [],
      warnings: [],
