...
export async function getIndexingSummary(): Promise<IndexingSummary> {
  ...
  const withCanonical = published.filter(c => c.canonicalContentId || (c as any).canonicalUrl).length;
  ...
  const recentlyIndexed = published.filter(c =>
    c.updatedAt && new Date(c.updatedAt) > sevenDaysAgo
  ).length;
...
export async function setNoindex(
  ...
  const rollbackToken = Buffer.from(JSON.stringify({
 ...
 */
export async function removeNoindex(contentId: string, dryRun = false): Promise<{ success: boolean }> {
  if (dryRun) {
    return { success: true };
  }
 ...
 */
export async function getPendingReindex(): Promise<{ contentId: string; queuedAt: Date; priority: string }[]> {
 ...
 */
export async function rollbackIndexingAction(rollbackToken: string): Promise<{ success: boolean; message: string }> {
  try {
...
console.log('[SEO Engine] Indexing adapter loaded');
