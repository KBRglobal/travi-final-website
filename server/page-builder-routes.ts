...
router.get("/pages", isAuthenticated, async (_req: Request, res: Response) => {
  try {
    const pages = await db.select()
      .from(editablePages)
      .orderBy(desc(editablePages.updatedAt));
    ...
    res.status(500).json({ 
      ...
      heGuide: "איך זה עובד: השתמש בעמוד זה כדי לנהל את כל העמודים הניתנים לעריכה"
    });
  }
});
  ...
  try {
    ...
    if (!page) {
      return res.status(404).json({ 
        ...
        heGuide: "איך זה עובד: העמוד המבוקש לא נמצא במערכת"
      });
    }
    ...
    res.status(500).json({ 
      ...
      heGuide: "איך זה עובד: אירעה שגיאה בטעינת העמוד"
    });
  }
});
  ...
  try {
    ...
    if (existingPage.length > 0) {
      return res.status(400).json({ 
        ...
        heGuide: "איך זה עובד: כתובת ה-URL כבר קיימת, אנא בחר כתובת ייחודית"
      });
    }
    ...
    if (error instanceof z.ZodError) {
      return res.status(400).json({ 
        ...
        heGuide: "איך זה עובד: הנתונים שהוזנו אינם תקינים"
      });
    }
    console.error("Error creating page:", error);
    res.status(500).json({ 
      error: "Failed to create page",
      heGuide: "איך זה עובד: אירעה שגיאה ביצירת העמוד"
    });
  }
});
...
router.put("/pages/:id", isAuthenticated, async (req: Request, res: Response) => {
  try {
    ...
    const [page] = await db.update(editablePages)
      .set({ 
        ...req.body, 
        lastEditedBy: userId,
        updatedAt: new Date() 
    ...
    if (!page) {
      return res.status(404).json({ 
        error: "Page not found",
        heGuide: "איך זה עובד: העמוד המבוקש לא נמצא"
      });
    }
    ...
    res.status(500).json({ 
      ...
      heGuide: "איך זה עובד: אירעה שגיאה בעדכון העמוד"
    });
  }
});
...
router.delete("/pages/:id", isAuthenticated, async (req: Request, res: Response) => {
  try {
    const [deletedPage] = await db.delete(editablePages)
      .where(eq(editablePages.id, req.params.id))
      .returning();
    ...
    res.json({ 
      success: true, 
      deletedPage,
      heGuide: "איך זה עובד: העמוד נמחק בהצלחה יחד עם כל הסקציות שלו"
    });
  } catch (error) {
    console.error("Error deleting page:", error);
    res.status(500).json({ 
      error: "Failed to delete page",
      heGuide: "איך זה עובד: אירעה שגיאה במחיקת העמוד"
    });
  }
});
...
router.get("/sections/:pageId", isAuthenticated, async (req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({ 
      ...
      heGuide: "איך זה עובד: אירעה שגיאה בטעינת הסקציות"
    });
  }
});
...
router.post("/sections", isAuthenticated, async (req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({ 
      ...
      heGuide: "איך זה עובד: אירעה שגיאה ביצירת הסקציה"
    });
  }
});
  ...
  try {
    ...
    if (!existingSection) {
      return res.status(404).json({ 
        ...
        heGuide: "איך זה עובד: הסקציה המבוקשת לא נמצאה"
      });
    }
    ...
    res.json({ 
      ...section, 
      versionNumber: newVersionNumber,
      heGuide: "איך זה עובד: הסקציה עודכנה וגרסה חדשה נשמרה להיסטוריה"
    });
  } catch (error) {
...
router.delete("/sections/:id", isAuthenticated, async (req: Request, res: Response) => {
  try {
    const [deletedSection] = await db.delete(pageSections)
      .where(eq(pageSections.id, req.params.id))
      .returning();
    ...
    res.json({ 
      success: true, 
      deletedSection,
      heGuide: "איך זה עובד: הסקציה נמחקה בהצלחה"
    });
  } catch (error) {
    console.error("Error deleting section:", error);
    res.status(500).json({ 
      error: "Failed to delete section",
      heGuide: "איך זה עובד: אירעה שגיאה במחיקת הסקציה"
    });
  }
});
...
router.put("/sections/reorder", isAuthenticated, async (req: Request, res: Response) => {
  try {
    const { sections: sectionUpdates } = req.body as { sections: { id: string; sortOrder: number }[] };
    
    if (!Array.isArray(sectionUpdates) || sectionUpdates.length === 0) {
      return res.status(400).json({ 
        error: "Invalid sections array",
        heGuide: "איך זה עובד: יש לספק רשימת סקציות עם סדר מעודכן"
      });
    }
    ...
    res.json({ 
      success: true,
      heGuide: "איך זה עובד: סדר הסקציות עודכן בהצלחה"
    });
  } catch (error) {
    console.error("Error reordering sections:", error);
    res.status(500).json({ 
      error: "Failed to reorder sections",
      heGuide: "איך זה עובד: אירעה שגיאה בעדכון סדר הסקציות"
    });
  }
});
...
router.get("/sections/:id/versions", isAuthenticated, async (req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({ 
      ...
      heGuide: "איך זה עובד: אירעה שגיאה בטעינת היסטוריית הגרסאות"
    });
  }
});

// POST /api/page-builder/sections/:id/restore/:versionNumber - Restore a version
router.post("/sections/:id/restore/:versionNumber", isAuthenticated, async (req: Request, res: Response) => {
  try {
    ...
    if (!version) {
      return res.status(404).json({ 
        ...
        heGuide: "איך זה עובד: הגרסה המבוקשת לא נמצאה"
      });
    }
    ...
    const [restoredSection] = await db.update(pageSections)
      .set({
...
router.get("/public/:slug", async (req: Request, res: Response) => {
  try {
    const [page] = await db.select()
      .from(editablePages)
      .where(and(
        eq(editablePages.slug, req.params.slug),
        eq(editablePages.isPublished, true)
      ));
    
    if (!page) {
      return res.status(404).json({ 
        error: "Page not found or not published",
        heGuide: "איך זה עובד: העמוד המבוקש לא נמצא או אינו מפורסם"
      });
    }
...
export function registerPageBuilderRoutes(app: Router) {
  ...
  console.log("[PageBuilder] Routes registered at /api/page-builder/*");
}
