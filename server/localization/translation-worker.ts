...
type TranslationProvider = 'openai' | 'anthropic' | 'gemini' | 'mistral' | 'groq' | 'openrouter' | 'perplexity' | 'helicone';
...
export async function processTranslationJob(job: TranslationJob): Promise<void> {
    ...
    switch (field) {
      ...
      case 'metaDescription':
        if (sourceContent.metaDescription) {
          const result = await translateText(sourceContent.metaDescription, sourceLocale, targetLocale);
      ...
      case 'answerCapsule':
        if (sourceContent.answerCapsule) {
          const result = await translateText(sourceContent.answerCapsule, sourceLocale, targetLocale);
  ...
  const [existingTranslation] = await db
    ...
    .where(and(
      ...
      eq(translations.locale, targetLocale as any)
    ));
...
async function translateContentHighlights(
  ...
  for (const highlight of highlights) {
    ...
    if (existing) {
      ...
      await db
        .insert(contentHighlightTranslations)
        .values({
