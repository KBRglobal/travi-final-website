...
import { SEO_REQUIREMENTS, CONTENT_TYPE_REQUIREMENTS, type ContentType } from "../lib/seo-standards";
import { DUBAI_KEYWORDS, getAllTopics } from "@shared/dubai-keywords";
import { db } from "../db";
...
const DESTINATION_LINKS: LinkableContent[] = [
  { id: "dubai", type: "destination", slug: "/destinations/dubai", title: "Dubai Travel Guide", destination: "Dubai", keywords: ["dubai", "uae", "emirates", "burj khalifa", "palm jumeirah"] },
  { id: "paris", type: "destination", slug: "/destinations/paris", title: "Paris Travel Guide", destination: "Paris", keywords: ["paris", "france", "eiffel tower", "louvre", "champs elysees"] },
  { id: "bangkok", type: "destination", slug: "/destinations/bangkok", title: "Bangkok Travel Guide", destination: "Bangkok", keywords: ["bangkok", "thailand", "temples", "street food", "grand palace"] },
  { id: "istanbul", type: "destination", slug: "/destinations/istanbul", title: "Istanbul Travel Guide", destination: "Istanbul", keywords: ["istanbul", "turkey", "bosphorus", "hagia sophia", "blue mosque"] },
  { id: "london", type: "destination", slug: "/destinations/london", title: "London Travel Guide", destination: "London", keywords: ["london", "uk", "england", "big ben", "buckingham palace"] },
  { id: "new-york", type: "destination", slug: "/destinations/new-york", title: "New York Travel Guide", destination: "New York", keywords: ["new york", "nyc", "manhattan", "times square", "central park"] },
  { id: "singapore", type: "destination", slug: "/destinations/singapore", title: "Singapore Travel Guide", destination: "Singapore", keywords: ["singapore", "marina bay", "sentosa", "gardens by the bay"] },
];

// Dubai-specific category pages (from dubai-keywords.ts)
const DUBAI_CATEGORY_LINKS: LinkableContent[] = Object.entries(DUBAI_KEYWORDS).map(([key, category]) => ({
  id: `dubai-${key}`,
  type: "landing" as const,
  slug: `/${key === "neighborhoodsNew" ? "districts" : key === "neighborhoodsOld" ? "old-dubai" : key}`,
  title: category.name,
  destination: "Dubai",
  keywords: category.topics.flatMap(t => t.keywords).slice(0, 10),
  category: category.name,
}));

// Category pages
const CATEGORY_LINKS: LinkableContent[] = [
  { id: "hotels", type: "landing", slug: "/hotels", title: "Find Hotels", keywords: ["hotel", "hotels", "accommodation", "stay", "resort"] },
  { id: "attractions", type: "landing", slug: "/attractions", title: "Top Attractions", keywords: ["attraction", "attractions", "things to do", "sightseeing", "landmark"] },
  { id: "restaurants", type: "landing", slug: "/dining", title: "Dining Guide", keywords: ["restaurant", "restaurants", "dining", "food", "cuisine", "eat"] },
  { id: "transport", type: "landing", slug: "/transport", title: "Transport Guide", keywords: ["transport", "transportation", "metro", "taxi", "bus", "getting around"] },
];
...
export function insertInternalLinks(
  ...
  // Include Dubai-specific category links for enhanced internal linking
  const allTargets = [...DESTINATION_LINKS, ...CATEGORY_LINKS, ...DUBAI_CATEGORY_LINKS]
    .filter(t => t.id !== options.currentId);
...
export function analyzeExistingLinks(content: string): {
  ...
  // Find missing opportunities (including Dubai-specific links)
  const missingOpportunities: string[] = [];
  const contentLower = content.toLowerCase();
  const allTargets = [...DESTINATION_LINKS, ...DUBAI_CATEGORY_LINKS];
...
function calculateRelevance(keyword: string, target: LinkableContent, content: string): number {
  ...
  const mentions = (content.toLowerCase().match(new RegExp(keyword.toLowerCase(), "g")) || []).length;
...
function findBestAnchorText(content: string, keyword: string, position: number): string {
  ...
  const after = content.substring(position + keywordLength, Math.min(content.length, position + keywordLength + 20));
...
function insertLink(content: string, opportunity: LinkOpportunity): { inserted: boolean; content: string } {
...
export function validateInternalLinking(
  ...
  if (analysis.count < requirements.internalLinks.min) {
    issues.push(`Not enough internal links (${analysis.count}). Minimum: ${requirements.internalLinks.min}`);
  }
  
  if (analysis.count > requirements.internalLinks.max + 2) {
    issues.push(`Too many internal links (${analysis.count}). Recommended maximum: ${requirements.internalLinks.max}`);
  }
...
export async function getAllLinkTargets(excludeId?: string): Promise<LinkableContent[]> {
  ...
  const hardcodedTargets = [...DESTINATION_LINKS, ...CATEGORY_LINKS, ...DUBAI_CATEGORY_LINKS];
