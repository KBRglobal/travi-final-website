...
export function registerWriterRoutes(app: Express) {
  ...
  app.post("/api/writers/:id/generate", requireAuth, async (req: Request, res: Response) => {
    try {
      const { id } = req.params;
  ...
  app.post("/api/writers/optimal", requireAuth, async (req: Request, res: Response) => {
    try {
  ...
  app.post("/api/writers/assign", requireAuth, async (req: Request, res: Response) => {
    try {
  ...
  app.post("/api/writers/:id/rewrite", requireAuth, async (req: Request, res: Response) => {
    try {
      const { id } = req.params;
  ...
  app.post("/api/writers/:id/titles", requireAuth, async (req: Request, res: Response) => {
    try {
      const { id } = req.params;
  ...
  app.post("/api/writers/collaborate", requireAuth, async (req: Request, res: Response) => {
    try {
      const { writerIds, topic } = z
        .object({
      ...
      const assignment = await assignmentSystem.createCollaboration(writerIds, topic);
  ...
  app.get("/api/writers/recommendations", async (req: Request, res: Response) => {
    try {
      const { topic, contentType } = z
        .object({
          topic: z.string(),
      ...
      const recommendations = await assignmentSystem.getWriterRecommendations(topic, contentType);
