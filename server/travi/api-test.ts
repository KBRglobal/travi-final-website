  ...
  try {
    ...
    const result = await model.generateContent({
      contents: [
        {
          role: "user",
          parts: [{ text: "Write a 30-word description of Burj Khalifa in Dubai." }],
        },
      ],
      generationConfig: {
        temperature: 0.7,
...
export async function testOpenAIAPI(): Promise<APITestResult> {
  ...
  try {
    ...
    const response = await openai.chat.completions.create({
      model: "gpt-4o-mini",
      messages: [
        { role: "user", content: "Write a 30-word description of Burj Khalifa in Dubai." },
      ],
      temperature: 0.7,
      max_tokens: 100,
    });
...
export async function testAnthropicAPI(): Promise<APITestResult> {
  ...
  if (!apiKey) {
    return {
      ...
      success: false,
      error:
        "No API key configured (checked AI_INTEGRATIONS_ANTHROPIC_API_KEY and ANTHROPIC_API_KEY)",
      duration: Date.now() - startTime,
    };
  }
  ...
  try {
    switch (serviceName) {
      case "gemini": {
        ...
        const result = await model.generateContent({
          contents: [
            { role: "user", parts: [{ text: 'Say "API test successful" in 5 words or less.' }] },
          ],
      ...
      case "freepik": {
        ...
        return {
          service: serviceName,
          success: true,
          response: `Found ${data.meta?.total || 0} results`,
          duration: Date.now() - startTime,
        };
      }
