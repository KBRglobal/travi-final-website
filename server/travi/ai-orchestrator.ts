...
import { canMakeRequest, recordRequestStart, recordRequestResult, delay, getThrottleMultiplier, ServiceType } from './budget-manager';
...
export const AI_MODELS = {
  ...
  gpt: {
    ...
    costPerOutputToken: 0.60 / 1_000_000,  // Per token: $0.0000006
    maxTokens: 16384,
    rateLimit: 10000,
  },
  claude: {
    ...
    costPerInputToken: 0.80 / 1_000_000,   // Per token: $0.0000008
    costPerOutputToken: 4.00 / 1_000_000,  // Per token: $0.000004
    maxTokens: 4096,
    rateLimit: 5000,
  },
} as const;
...
export async function getNextAvailableModel(
  excludeModels: Set<AIModelKey> = new Set()
): Promise<{
  ...
  for (let i = 0; i < modelOrder.length; i++) {
    ...
    if (!check.allowed) {
      console.log(`[AIOrchestrator] ${model.name} unavailable: ${check.reason}`);
      continue;
    }
    ...
    if (usage >= model.rateLimit) {
      console.log(`[AIOrchestrator] ${model.name} daily limit reached: ${usage}/${model.rateLimit}`);
      continue;
    }
  ...
  console.error('[AIOrchestrator] All AI models exhausted for today');
  return null;
}
...
export async function callAI(
  ...
  let modelInfo = options.preferredModel && !failedModels.has(options.preferredModel)
  ...
  if (!modelInfo) {
    console.error('[AIOrchestrator] No AI models available');
    return null;
  }
  ...
  if (!requestCheck.allowed) {
    console.error(`[AIOrchestrator] Request blocked: ${requestCheck.reason}`);
    return null;
  }
  
  // Apply throttle delay if specified
  if (requestCheck.throttled && requestCheck.delayMs) {
    console.log(`[AIOrchestrator] Throttled - waiting ${requestCheck.delayMs}ms before ${model.name} call`);
  ...
  const result = await withRetry(
    ...
    },
    {
      ...
      onRetry: (attempt, error) => {
        console.log(`[AIOrchestrator] Retry ${attempt} for ${model.name}: ${error.message}`);
      },
    }
  );
  ...
  if (failedModels.size < modelOrder.length) {
    console.log(`[AIOrchestrator] ${model.name} failed, trying next model...`);
  ...
  console.error(`[AIOrchestrator] All models failed`);
  return null;
}
...
export function getModelStats(): { [key in AIModelKey]: { used: number; limit: number; available: boolean } } {
  return {
    gemini: {
...
export function getAIStatus(): { available: boolean; models: { [key in AIModelKey]: { available: boolean; rateLimit: number; used: number } } } {
  const stats = getModelStats();
  return {
    available: Object.values(stats).some(m => m.available),
    models: {
      gemini: { available: stats.gemini.available, rateLimit: stats.gemini.limit, used: stats.gemini.used },
      gpt: { available: stats.gpt.available, rateLimit: stats.gpt.limit, used: stats.gpt.used },
      claude: { available: stats.claude.available, rateLimit: stats.claude.limit, used: stats.claude.used },
    },
  };
}
