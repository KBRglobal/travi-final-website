...
const TOKEN_COSTS = {
  gemini: { input: 0.000, output: 0.000 },         // Free tier
  gpt: { input: 0.00015, output: 0.0006 },         // GPT-4o Mini
  claude: { input: 0.0008, output: 0.004 },        // Claude Haiku
};

export type ServiceType = 'gemini' | 'gpt' | 'claude' | 'google_places' | 'freepik' | 'tripadvisor' | 'wikipedia' | 'osm';
...
async function getOrCreateDailyUsage(service: ServiceType): Promise<UsageStats | null> {
  ...
  try {
    ...
    console.error(`[BudgetManager] Failed to get/create usage for ${service}:`, error);
    return null;
  }
}
...
export async function canMakeRequest(service: ServiceType): Promise<{ 
  ...
  if (budgetStatus.status === 'critical') {
    ...
    console.warn(`[BudgetManager] CRITICAL: $${budgetStatus.totalSpent.toFixed(2)} spent - Throttling processing 3x`);
  ...
  if (budgetStatus.status === 'warning') {
    console.log(`[BudgetManager] Warning: $${budgetStatus.totalSpent.toFixed(2)} spent of $${BUDGET_THRESHOLDS.stop} limit`);
  }
...
export function pauseProcessing(reason: string = 'Manual pause'): void {
  processingPaused = true;
  console.warn(`[BudgetManager] Processing PAUSED: ${reason}`);
}
...
export function resumeProcessing(): void {
  ...
  processingThrottled = false;
  console.log(`[BudgetManager] Processing RESUMED`);
}
...
export async function recordRequestStart(service: ServiceType): Promise<void> {
  ...
  try {
    ...
    console.error(`[BudgetManager] Failed to record request start:`, error);
  }
}
...
export async function recordRequestResult(
  ...
  try {
    ...
    console.error(`[BudgetManager] Failed to record request result:`, error);
  }
}
...
async function checkAndCreateAlerts(): Promise<void> {
  try {
    ...
    const severity = budgetStatus.status === 'stopped' ? 'budget_stop' 
                   : budgetStatus.status === 'critical' ? 'critical' 
                   : 'warning';
    
    const threshold = budgetStatus.status === 'stopped' ? BUDGET_THRESHOLDS.stop
                    : budgetStatus.status === 'critical' ? BUDGET_THRESHOLDS.critical
    ...
    if (existingAlert.rows.length === 0) {
      ...
      console.warn(`[BudgetManager] ALERT: ${budgetStatus.alertMessage}`);
    }
  } catch (error) {
    console.error(`[BudgetManager] Failed to check/create alerts:`, error);
  }
}
...
export async function getTodayUsageStats(): Promise<UsageStats[]> {
  ...
  try {
    ...
    console.error(`[BudgetManager] Failed to get today's stats:`, error);
    return [];
  }
}
...
export async function getActiveAlerts(): Promise<any[]> {
  try {
    ...
    console.error(`[BudgetManager] Failed to get active alerts:`, error);
    return [];
  }
}

// Acknowledge an alert
export async function acknowledgeAlert(alertId: string, acknowledgedBy: string = 'system'): Promise<boolean> {
  try {
    await db.execute(sql`
    ...
    console.error(`[BudgetManager] Failed to acknowledge alert:`, error);
    return false;
  }
}
