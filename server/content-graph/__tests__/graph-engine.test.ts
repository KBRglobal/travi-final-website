...
describe("Graph Engine", () => {
  ...
  describe("findOrphans", () => {
    it("should find isolated nodes", () => {
      ...
      // Create bidirectional links so both nodes have incoming edges
      upsertEdge("connected-1", "connected-2", "internal_link");
      upsertEdge("connected-2", "connected-1", "internal_link");

      const orphans = findOrphans();

      // orphan-1 is truly isolated (no edges)
      expect(orphans.some(o => o.nodeId === "orphan-1")).toBe(true);
      // connected-1 has both incoming and outgoing edges, so not an orphan
  ...
  describe("buildFromContent", () => {
    it("should build graph from content object", () => {
      const content: ContentWithDependencies = {
        ...
        locale: "en",
        url: "/test-article",
        internalLinks: [{ targetId: "article-2", anchor: "Related Article" }],
        entityMentions: [{ entityId: "entity-1", entityType: "hotel" }],
