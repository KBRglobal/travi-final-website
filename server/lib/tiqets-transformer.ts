...
export class TiqetsTransformer {
  ...
  /**
   * Transform rating to Hebrew label
   */
  static getRatingLabel(rating: number | null | undefined): string | null {
    if (!rating) return null;
    if (rating >= 4.5) return 'מדורג מצוין';
    if (rating >= 3.5) return 'מדורג היטב';
   ...
   */
  static transformProduct(product: TiqetsProduct, cityName: string): Omit<InsertTiqetsAttraction, 'id' | 'createdAt' | 'updatedAt'> {
    ...
    return {
      ...
      tiqetsWhatsIncluded: Array.isArray(product.whats_included) 
        ...
        : (typeof product.whats_included === 'string' ? product.whats_included : null),
      tiqetsWhatsExcluded: Array.isArray(product.whats_excluded) 
        ? product.whats_excluded.join('\n') 
        : (typeof product.whats_excluded === 'string' ? product.whats_excluded : null),
   ...
   */
  static transformProducts(products: TiqetsProduct[], cityName: string): Omit<InsertTiqetsAttraction, 'id' | 'createdAt' | 'updatedAt'>[] {
