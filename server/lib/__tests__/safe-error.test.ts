...
describe("Safe Error Handling", () => {
  ...
  describe("sanitizeErrorMessage", () => {
    ...
    it("should replace unsafe messages with fallback", () => {
      expect(sanitizeErrorMessage("Error at /app/server/routes.ts:123")).toBe("An error occurred");
    });

    it("should use custom fallback when provided", () => {
      expect(sanitizeErrorMessage("Some internal error", "Something went wrong")).toBe(
  ...
  describe("Security: No Information Disclosure", () => {
    it("should never leak database connection strings", () => {
      const sensitiveError = new Error(
        "ECONNREFUSED postgresql://user:password@db.internal:5432/production"
      );
    ...
    it("should never leak file system paths", () => {
      const sensitiveError = new Error(
        "ENOENT: no such file or directory /var/app/config/secrets.json"
      );
