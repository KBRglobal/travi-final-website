...
export function validateContent(content: GeneratedContent): QualityCheckResult {
  ...
  logger.info(`Quality gate result for "${content.title}": ${passed ? 'PASSED' : 'FAILED'} (score: ${score})`);
...
export function checkLinkQuotas(html: string): QualityCheck {
  ...
  if (internalCount < LINK_QUOTAS.internal.min) {
    issues.push(`Not enough internal links: ${internalCount} (minimum: ${LINK_QUOTAS.internal.min})`);
    score -= 15;
  ...
  if (externalCount < LINK_QUOTAS.external.min) {
    issues.push(`Not enough external links: ${externalCount} (minimum: ${LINK_QUOTAS.external.min})`);
    score -= 10;
...
export function checkImageRequirements(html: string, images?: ImageInfo[]): QualityCheck {
  ...
  if (imageCount < requiredImages) {
    ...
    issues.push(`Not enough images: ${imageCount} (need ${requiredImages}: 1 hero + ${h2Count} for sections)`);
    score -= missing * 10;
  }
...
export function checkSEOCompliance(meta: MetaInfo): QualityCheck {
  ...
  if (descLength < META_LIMITS.description.min) {
    issues.push(`Meta description too short: ${descLength} chars (minimum: ${META_LIMITS.description.min})`);
    score -= 25;
  } else if (descLength > META_LIMITS.description.max) {
    issues.push(`Meta description too long: ${descLength} chars (maximum: ${META_LIMITS.description.max})`);
    score -= 15;
  }
...
export function checkProfessionalTone(html: string): QualityCheck {
  ...
  if (allCapsWords.length > 0) {
    issues.push(`All-caps words detected: ${allCapsWords.slice(0, 3).join(', ')}${allCapsWords.length > 3 ? '...' : ''}`);
    score -= 15;
  }
...
export function checkAltText(images: ImageInfo[]): QualityCheck {
  ...
  const bannedAltWords = [
    'stunning', 'amazing', 'beautiful', 'breathtaking', 'gorgeous',
    'spectacular', 'incredible', 'magnificent', 'wonderful', 'fabulous',
  ];
  ...
  const passed = issues.length === 0;
  const details = passed 
    ? `All ${images.length} images have valid alt text`
    : issues.join('. ');
...
function calculateWeightedScore(checks: QualityCheck[], blockers: QualityBlocker[]): number {
  ...
  const structureChecks = checks.filter(c => 
    ['Heading Structure', 'Link Quotas'].includes(c.name)
  );
    ...
    if (!check.passed) {
      switch (check.name) {
        ...
        case 'Heading Structure':
          ...
          break;
        case 'Link Quotas':
          recommendations.push('Add 5-8 internal links and 2-3 external links to authoritative sources');
          break;
        case 'Image Requirements':
          recommendations.push('Include 1 hero image plus 1 image per H2 section');
          break;
        case 'SEO Compliance':
          recommendations.push('Adjust meta title to 50-60 characters and meta description to 150-160 characters');
          break;
        case 'Banned Phrases':
          recommendations.push('Remove all clickbait phrases and clichés. Use professional alternatives from the phrase guide');
          break;
        case 'Professional Tone':
          recommendations.push('Reduce exclamation marks, avoid all-caps words, and use measured language');
          break;
        case 'Schema Validity':
          recommendations.push('Ensure JSON-LD schema includes @context, @type, and required fields for the schema type');
          break;
        case 'Alt Text':
          recommendations.push('Add descriptive alt text (5-15 words) to all images without marketing language');
          break;
        case 'Readability':
...
export interface QualityGateResult {
  ...
  validation: { isValid: boolean; score: number; errors: unknown[]; warnings: unknown[]; suggestions: string[] };
  blockingIssues: { code: string; message: string; field: string; severity: "critical" | "blocking" }[];
  recommendations: string[];
...
export function validateThroughGateway(content: ContentForValidation): QualityGateResult {
  ...
  return {
    ...
    validation: {
      ...
      score: result.score,
      errors: result.blockers.map(b => ({ code: b.type.toUpperCase(), message: b.message, field: b.type, severity: b.severity })),
      warnings: [],
...
export function canPublish(content: ContentForValidation): { allowed: boolean; reason?: string } {
  ...
  return { 
    allowed: false, 
    reason: result.blockingIssues.map(i => i.message).join("; ") || `Score ${result.score} below threshold ${QUALITY_THRESHOLDS.publish}`
  };
}
...
export function validateForSave(content: ContentForValidation): { valid: boolean; errors: string[] } {
  const result = validateThroughGateway(content);
  const errors: string[] = [];
