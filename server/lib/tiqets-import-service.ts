...
export class TiqetsImportService {
  ...
  async findAllCityIds(
    ...
    found: number;
    total: number;
    details: Array<{ name: string; tiqetsId: string | null }>;
  }> {
    ...
    while (found < ourCities.length && page <= maxPages) {
      ...
      try {
        ...
        for (const tiqetsCity of response.cities || []) {
          ...
          const match = ourCities.find(c => c.name.toLowerCase() === tiqetsCity.name.toLowerCase());
  ...
  async importAllCities(progressCallback?: ProgressCallback): Promise<{
    ...
    for (const city of cities) {
      ...
      const result = await this.importCity(city.tiqetsCityId, city.name, progressCallback);
