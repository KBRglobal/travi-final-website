  ...
  hasMetaTitle: {
    ...
    check: (content: ContentData): CheckResult => {
      ...
      if (content.metaTitle.length > 60) {
        return {
          status: "warning",
          message: `Meta title too long (${content.metaTitle.length}/60 chars)`,
        };
      }
      if (content.metaTitle.length < 30) {
        return {
          status: "warning",
          message: `Meta title short (${content.metaTitle.length}/60 chars)`,
        };
      }
  ...
  hasMetaDescription: {
    ...
    check: (content: ContentData): CheckResult => {
      ...
      if (content.metaDescription.length > 160) {
        return {
          status: "warning",
          message: `Description too long (${content.metaDescription.length}/160 chars)`,
        };
      }
      if (content.metaDescription.length < 70) {
        return {
          status: "warning",
          message: `Description short (${content.metaDescription.length}/160 chars)`,
        };
      }
  ...
  seoScoreCheck: {
    ...
    check: (content: ContentData): CheckResult => {
      ...
      if (content.seoScore < 50) {
        return {
          status: "warning",
          message: `SEO score needs improvement (${content.seoScore}/100)`,
        };
      }
  ...
  hasAnswerCapsule: {
    ...
    check: (content: ContentData): CheckResult => {
      ...
      if (words < 30) {
        return {
          status: "warning",
          message: `Answer capsule short (${words} words, aim for 40-60)`,
        };
      }
      if (words > 80) {
        return {
          status: "warning",
          message: `Answer capsule long (${words} words, aim for 40-60)`,
        };
      }
...
export async function evaluateChecklist(contentId: string): Promise<ChecklistResult> {
  ...
  try {
    ...
    const warningItems = items.filter(
      i => i.status === "warning" || (i.status === "fail" && i.priority !== "required")
    );
