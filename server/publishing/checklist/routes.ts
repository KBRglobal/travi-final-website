...
export function registerChecklistRoutes(app: Express): void {
   ...
   */
  app.get("/api/admin/content/:id/publish-checklist", requireAuth, async (req: Request, res: Response) => {
    const { id } = req.params;
   ...
   */
  app.get("/api/admin/content/:id/can-publish", requireAuth, async (req: Request, res: Response) => {
    const { id } = req.params;
    ...
    try {
      ...
      console.error('[Checklist] Error checking publish:', error);
      res.status(500).json({
        contentId: id,
        canPublish: false,
        error: 'Check failed',
