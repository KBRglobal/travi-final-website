...
router.get('/status', async (_req: Request, res: Response) => {
  try {
    ...
    console.error('[Ingestion] Failed to get status:', error);
    res.status(500).json({
      success: false,
...
router.get('/:source/status', async (req: Request, res: Response) => {
  try {
    ...
    console.error('[Ingestion] Failed to get source status:', error);
    res.status(500).json({
      success: false,
...
router.post('/:source/run', async (req: Request, res: Response) => {
  try {
    ...
    console.log(`[Ingestion] Manual ingestion triggered for: ${source}`);
    
    // Run in background
    ingestionScheduler.runIngestion(source).catch(error => {
      console.error(`[Ingestion] Background ingestion failed for ${source}:`, error);
    });
    ...
    console.error('[Ingestion] Failed to trigger ingestion:', error);
    res.status(500).json({
      success: false,
...
router.get('/:source/history', async (req: Request, res: Response) => {
  try {
    ...
    console.error('[Ingestion] Failed to get history:', error);
    res.status(500).json({
      success: false,
