...
import { getSystemPrompt, describeContext, type ChatContext, type PageContext } from './chat-prompts';
...
import { recordChatIntent, getDominantIntent, getRecentIntents, type IntentType } from '../session/intent-memory';
import { 
...
export function getChatMetrics(): {
  ...
  const fallbackRate = chatMetrics.totalChatRequests > 0
...
const logger = {
  info: (msg: string, data?: Record<string, unknown>) =>
    log.info(`[ChatHandler] ${msg}`, data),
  warn: (msg: string, data?: Record<string, unknown>) =>
    log.warn(`[ChatHandler] ${msg}`, data),
...
const FALLBACK_MESSAGES = {
  timeout: "This is taking longer than expected. For faster results, try searching for your question.",
  provider_unavailable: "I'm having trouble connecting right now. You can try using the search feature instead.",
  orchestrator_rejected: "I'm experiencing high demand right now. Please try again in a few moments, or use the search feature.",
  credits_low: "I'm having trouble connecting right now. You can try using the search feature instead.",
  provider_error: "I'm having trouble connecting right now. You can try using the search feature instead.",
  unknown_error: "I apologize, but I'm currently unable to process your request. Please try using the search feature instead.",
} as const;
...
const DEFAULT_DESTINATION_SLUGS = [
  'dubai', 'paris', 'tokyo', 'london', 'new-york', 'singapore',
  'barcelona', 'rome', 'amsterdam', 'istanbul', 'bangkok', 'hong-kong',
];
...
async function resolveEntities(
  ...
  const results = await Promise.all(
    entities.map(e => resolveEntity(e.type, e.slug, e.name))
  );
...
async function getSearchEnhancedSuggestions(
  ...
  try {
    ...
    const contextDescription = describeSearchContextForChat(searchResults, intentSignals);
...
async function convertSearchResultsToSuggestions(
  ...
  for (const result of searchResults) {
    if (result.type === 'destination' || result.type === 'article' || result.type === 'attraction') {
...
export async function handleChatRequest(request: ChatRequest): Promise<ChatResponse> {
  ...
  try {
    ...
    recordLoopEntry('chat', chatEntryPoint);
  ...
  logger.info('Processing chat request', {
    ...
    dominantIntent,
    recentIntentsCount: intentMemoryData.intents.length,
    recentEntitiesCount: intentMemoryData.entities.length,
  });
  ...
  try {
    ...
    if (!submitResponse.accepted) {
      ...
      const isCreditsIssue = submitResponse.reason?.toLowerCase().includes('credit') || 
                            submitResponse.reason?.toLowerCase().includes('limit');
      const fallbackReason: FallbackReason = isCreditsIssue ? 'credits_low' : 'orchestrator_rejected';
    ...
    if (suggestions.length > 0 || suggestedDestinations.length > 0) {
      recordLoopStep('chat', 'chat_deep_dive');
    }
 ...
 */
export function validateChatRequest(body: unknown): { valid: boolean; error?: string; request?: ChatRequest } {
