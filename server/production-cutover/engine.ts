  ...
  try {
    ...
    if (gm.isPlatformGovernorEnabled()) {
      ...
      governor = {
        ...
        activeRules: [...new Set(restrictions.map(r => r.ruleId))],
      };
    }
  } catch {
    /* not available */
...
function extractBlockers(snapshot: SystemSnapshot): {
  hard: CutoverBlocker[];
  soft: CutoverBlocker[];
} {
  ...
  if (snapshot.readiness.status === "BLOCKED") {
    hard.push({
      ...
      severity: "hard",
      title: "Platform Readiness Blocked",
      description: `Readiness score: ${snapshot.readiness.score}`,
      category: "readiness",
      detectedAt: now,
    });
    ...
    soft.push({
      ...
      description: `Score ${snapshot.readiness.score} below threshold ${CUTOVER_CONFIG.minReadinessScore}`,
      category: "readiness",
      detectedAt: now,
    });
  }
  ...
  if (snapshot.incidents.critical > 0) {
    hard.push({
      ...
      description: `${snapshot.incidents.critical} critical incident(s)`,
      category: "ops",
      detectedAt: now,
    });
    ...
    soft.push({
      ...
      description: `${snapshot.incidents.open} open incident(s)`,
      category: "ops",
      detectedAt: now,
    });
  }
  ...
  if (snapshot.governor.restrictions > 0) {
    soft.push({
      ...
      source: "platform-governor",
      severity: "soft",
      title: "Governor Restrictions Active",
      description: `${snapshot.governor.restrictions} restriction(s)`,
      category: "governor",
      detectedAt: now,
    });
  }
  ...
  if (snapshot.backpressure.memoryPercent > CUTOVER_CONFIG.backpressureMemoryMax) {
    soft.push({
      ...
      description: `Memory at ${snapshot.backpressure.memoryPercent}%`,
      category: "infra",
      detectedAt: now,
    });
  }
