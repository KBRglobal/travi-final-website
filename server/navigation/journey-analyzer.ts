...
const JOURNEY_STAGES: JourneyStageConfig[] = [
  ...
  {
    stage: "deep_dive",
    patterns: [/^\/hotels\/[^/]+\/?$/, /^\/attractions\/[^/]+\/?$/, /^\/articles\/[^/]+\/?$/],
    nextStages: ["retention", "deep_dive", "exploration"],
  },
  {
    stage: "retention",
    patterns: [/^\/saved\/?$/, /^\/newsletter\/?$/, /^\/compare\/?$/],
    nextStages: ["exploration", "deep_dive"],
  },
];
...
export async function getSuggestedLinks(
  ...
  try {
    ...
    for (const suggestion of suggestions) {
      const url = await resolveEntityLink(suggestion.type as EntityType, suggestion.slug);
      if (url) {
...
async function addDestinationSuggestions(
  ...
  const siblingDestinations = await db
    ...
    .from(destinations)
    .where(and(eq(destinations.isActive, true), ne(destinations.id, destinationId)))
    .limit(3);
...
async function addHotelSuggestions(contentId: string, suggestions: SuggestedLink[]): Promise<void> {
  ...
  const relatedHotels = hotel.relatedHotels as Array<{
    id?: string;
    name?: string;
    slug?: string;
  }> | null;
...
async function addAttractionSuggestions(
  ...
  const relatedAttractions = attraction.relatedAttractions as Array<{
    id?: string;
    name?: string;
    slug?: string;
  }> | null;
