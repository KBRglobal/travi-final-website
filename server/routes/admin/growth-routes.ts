...
export function registerGrowthRoutes(app: Express) {
  ...
  app.get("/api/admin/growth/overview", requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      const translationCoverage = expectedTranslations > 0 
      ...
      res.json({
        totalDestinations,
        contentCoverage: totalDestinations > 0 ? Math.round((withContent / totalDestinations) * 100) : 0,
        imageCoverage: totalDestinations > 0 ? Math.round((withImages / totalDestinations) * 100) : 0,
        translationCoverage,
        activeTasks,
        updatedAt: new Date().toISOString(),
      });
    } catch (error) {
      console.error("[Growth] Error getting overview:", error);
  ...
  app.get("/api/admin/growth/content-pipeline", requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      res.json({
        ...
        completionRate: allDestinations.length > 0 
      ...
      console.error("[Growth] Error getting content pipeline:", error);
  ...
  app.get("/api/admin/growth/image-pipeline", requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      const imagesNeeded = needingImages.length + withHeroOnly.reduce((acc, d) => {
        const images = d.images as any[] || [];
      ...
      console.error("[Growth] Error getting image pipeline:", error);
  ...
  app.get("/api/admin/growth/translation-pipeline", requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      const coverageMatrix: Record<string, { translated: number; total: number; percentage: number }> = {};
      ...
      for (const locale of locales) {
        ...
        coverageMatrix[locale.code] = {
          ...
          percentage: publishedContent.length > 0 
      ...
      const contentNeedingTranslation = publishedContent.map(content => {
      ...
      res.json({
        ...
        stats: {
          ...
          overallCoverage: publishedContent.length > 0
            ? Math.round((existingTranslations.length / (publishedContent.length * locales.length)) * 100)
            : 0,
        },
      });
    } catch (error) {
      console.error("[Growth] Error getting translation pipeline:", error);
  ...
  app.get("/api/admin/growth/activity-log", requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      console.error("[Growth] Error getting activity log:", error);
    ...
    try {
      ...
      const [weeklyContent] = await db
        ...
        .where(and(
          ...
          gte(contents.publishedAt, weekAgo)
        ));
      ...
      const [monthlyContent] = await db
        ...
        .where(and(
          eq(contents.status, "published"),
          gte(contents.publishedAt, monthAgo)
        ));
      ...
      const weeklyRate = (weeklyContent?.count || 0) + (weeklyImages?.count || 0) + (weeklyTranslations?.count || 0);
      ...
      console.error("[Growth] Error getting metrics:", error);
  ...
  app.post("/api/admin/growth/trigger-images", requirePermission("canManageSettings"), async (req: Request, res: Response) => {
    try {
      ...
      console.error("[Growth] Error triggering images:", error);
      res.status(500).json({ error: "Failed to trigger image generation" });
    }
  });

  app.post("/api/admin/growth/trigger-translations", requirePermission("canManageSettings"), async (req: Request, res: Response) => {
    try {
      ...
      console.error("[Growth] Error triggering translations:", error);
  ...
  app.get("/api/admin/growth/freshness", requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      console.error("[Growth] Error getting freshness status:", error);
  ...
  app.get("/api/admin/intelligence-stats", requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      res.json({
        ...
        entityLinkedPercent: totalPublished > 0 ? Math.round((entityLinked / totalPublished) * 100) : 0,
      });
    } catch (error) {
      console.error("[Growth] Error getting intelligence stats:", error);
      res.status(500).json({ error: "Failed to get intelligence stats" });
    }
  });

  app.post("/api/admin/growth/refresh/:destinationId", requirePermission("canManageSettings"), async (req: Request, res: Response) => {
    try {
      ...
      console.log(`[Growth] Manual content refresh triggered for destination: ${destinationId}`);
      ...
      console.error("[Growth] Error refreshing content:", error);
