...
router.get("/recent", async (req: Request, res: Response) => {
  try {
    ...
    const [recentJobs, failedCount, pendingCount, processingCount, completedStats] =
      await Promise.all([
        db.select().from(octopusJobs).orderBy(desc(octopusJobs.createdAt)).limit(limit),

        db
          .select({ count: sql<number>`count(*)::int` })
          .from(octopusJobs)
          .where(
            and(eq(octopusJobs.status, "failed"), gte(octopusJobs.createdAt, twentyFourHoursAgo))
          )
          .then(r => r[0]?.count ?? 0),
