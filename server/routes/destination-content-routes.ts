  ...
  for (const [type, mappings] of Object.entries(SECTION_TYPE_MAPPINGS)) {
    ...
    for (const section of sections) {
      ...
      for (const mapping of mappings) {
        ...
        const isStartsWithAtLevel2 =
          section.level === 2 && headingLower.startsWith(mapping.toLowerCase());
...
router.get(
  "/api/public/destination-content/:slug/section/:sectionType",
...
router.get("/api/public/holidays/:countryCode", async (req: Request, res: Response) => {
  try {
    ...
    const holidays = await query.orderBy(update9987PublicHolidays.date).limit(limit);
...
router.get(
  "/api/public/destination-content/:slug/essentials",
  async (req: Request, res: Response) => {
    try {
      const { slug } = req.params;
      const locale = (req.query.locale as string) || "en";
...
router.get(
  "/api/destination-content/guides/rewrite-status",
  async (req: Request, res: Response) => {
    try {
...
router.post(
  "/api/destination-content/guides/:slug/rewrite",
  async (req: Request, res: Response) => {
    try {
      const { slug } = req.params;
...
router.post("/api/destination-content/guides/rewrite-all", async (req: Request, res: Response) => {
  try {
    ...
    res.json({
      success: true,
      message:
        "Batch rewrite started. Check /api/destination-content/guides/rewrite-status for progress.",
    });
...
router.post(
  "/api/destination-content/guides/force-rewrite-all",
  async (req: Request, res: Response) => {
    try {
      // Return immediately and process in background
      res.json({
        success: true,
        message:
          "Force rewrite started. All existing rewrites cleared. Check /api/destination-content/guides/rewrite-status for progress.",
      });
...
router.get(
  "/api/destination-content/guides/:slug/structured",
  async (req: Request, res: Response) => {
    try {
      const { slug } = req.params;
