...
router.get("/api/localized-assets/:entityType/:entityId", async (req: Request, res: Response) => {
  try {
    ...
    console.error("[LocalizedAssets] Error fetching assets:", error);
    res.status(500).json({ error: "Failed to fetch localized assets" });
  }
});

router.get("/api/localized-assets/:entityType/:entityId/:locale/:usage", async (req: Request, res: Response) => {
  try {
    ...
    if (!entityTypeResult.success) {
      return res.status(400).json({ error: "Invalid entityType", valid: localizedAssetEntityTypes });
    }
    ...
    if (!asset) {
      // Fallback to English
      const [fallbackAsset] = await db.select().from(localizedAssets)
        .where(and(
          eq(localizedAssets.entityType, entityTypeResult.data),
          eq(localizedAssets.entityId, entityId),
    ...
    console.error("[LocalizedAssets] Error fetching asset:", error);
...
router.post("/api/admin/localized-assets", requirePermission("canEdit"), async (req: Request, res: Response) => {
  try {
    ...
    if (existing) {
      const [updated] = await db.update(localizedAssets)
        .set({
          ...data,
          updatedAt: new Date(),
        } as any)
    ...
    const [created] = await db.insert(localizedAssets)
      .values(data as any)
      .returning();

    res.status(201).json(created);
  } catch (error) {
    console.error("[LocalizedAssets] Error creating/updating asset:", error);
...
router.delete("/api/admin/localized-assets/:id", requirePermission("canEdit"), async (req: Request, res: Response) => {
  try {
    const { id } = req.params;

    const [deleted] = await db.delete(localizedAssets)
    ...
    console.error("[LocalizedAssets] Error deleting asset:", error);
...
router.get("/api/admin/localized-assets", requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    console.error("[LocalizedAssets] Error listing assets:", error);
