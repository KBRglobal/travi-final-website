...
export function registerContentOwnershipRoutes(app: Express): void {
  app.get("/api/admin/ownership/content/:contentId", requireAuth, async (req: Request, res: Response) => {
  ...
  app.post("/api/admin/ownership/transfer", requireAuth, async (req: Request, res: Response) => {
    ...
    const transfer = transferOwnership(contentId, fromUserId, toUserId, role as OwnershipRole, transferredBy, reason);
    res.json({ success: true, transfer });
  });

  app.delete("/api/admin/ownership/:contentId/:userId/:role", requireAuth, async (req: Request, res: Response) => {
    if (!isContentOwnershipEnabled()) {
      return res.status(403).json({ error: "Feature disabled" });
    }
  ...
  app.get("/api/admin/ownership/history/:contentId", requireAuth, async (req: Request, res: Response) => {
    const { contentId } = req.params;
  ...
  console.log("[ContentOwnership] Routes registered");
}
