...
function requireExportsEnabled(req: Request, res: Response, next: NextFunction): void {
  if (!isExportsEnabled()) {
    res.status(404).json({
 ...
 */
router.get("/contents.csv", requireExportsEnabled, async (req: Request, res: Response) => {
  try {
    const { limit, offset, status, type, locale } = req.query;

    const result = await exportContentsAsCSV({
      format: "csv",
      includeHeaders: true,
 ...
 */
router.get("/contents.json", requireExportsEnabled, async (req: Request, res: Response) => {
  try {
    const { limit, offset, status, type, locale } = req.query;

    const result = await exportContentsAsJSON({
      format: "json",
      limit: limit ? parseInt(limit as string, 10) : undefined,
      offset: offset ? parseInt(offset as string, 10) : undefined,
      filters: {
 ...
 */
router.get("/entities/:type.csv", requireExportsEnabled, async (req: Request, res: Response) => {
  try {
    ...
    const result = await exportEntitiesAsCSV(type, {
      format: "csv",
      includeHeaders: true,
      limit: limit ? parseInt(limit as string, 10) : undefined,
      offset: offset ? parseInt(offset as string, 10) : undefined,
 ...
 */
router.get("/entities/:type.json", requireExportsEnabled, async (req: Request, res: Response) => {
  try {
    const { type } = req.params;
    const { limit, offset, destinationId } = req.query;
    ...
    const result = await exportEntitiesAsJSON(type, {
      format: "json",
      limit: limit ? parseInt(limit as string, 10) : undefined,
      offset: offset ? parseInt(offset as string, 10) : undefined,
      filters: {
