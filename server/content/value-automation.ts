...
} from "../jobs/background-scheduler";
import { getLoadTierManager } from "../system/load-tiers";

const logger = {
  info: (msg: string, data?: Record<string, unknown>) => log.info(`[ValueAutomation] ${msg}`, data),
  warn: (msg: string, data?: Record<string, unknown>) => log.warn(`[ValueAutomation] ${msg}`, data),
...
export function queueLowCostWins(maxItems: number = 10): AutomationQueueResult {
  ...
  for (const win of wins.slice(0, maxItems)) {
    ...
    try {
      const jobId = scheduleBackgroundJob("seo-improvement", win.entityId, "low");
      jobIds.push(jobId);
 ...
 */
export function runAutomationCycle(options?: { maxHighValue?: number; maxLowCost?: number }): {
 ...
 */
export function getContentByRecommendation(
  recommendation: ContentRecommendation,
  limit: number = 50
): ContentValueResult[] {
  const allPerformance = getAllPerformance();
