...
import { getPerformance, getPerformanceScore, HIGH_PERFORMING_SCORE_THRESHOLD } from './metrics/content-performance';
import { getCostAnalytics } from '../ai-orchestrator/cost-analytics';

const logger = {
  info: (msg: string, data?: Record<string, unknown>) =>
    log.info(`[ValueScorer] ${msg}`, data),
  warn: (msg: string, data?: Record<string, unknown>) =>
    log.warn(`[ValueScorer] ${msg}`, data),
...
export function getContentValueSummary(): ContentValueSummary {
  ...
  return {
    ...
    protectedCount,
    insufficientDataCount,
    averageValue: allResults.length > 0 
      ? Math.round((totalValue / allResults.length) * 100) / 100 
      : 0,
    topCandidates,
  };
}
...
export function canImproveContent(entityId: string): { 
  ...
  return {
    allowed: true,
    reason: valueResult.recommendation === 'improve' 
