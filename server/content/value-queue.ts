...
const logger = {
  info: (msg: string, data?: Record<string, unknown>) => log.info(`[ValueQueue] ${msg}`, data),
  warn: (msg: string, data?: Record<string, unknown>) => log.warn(`[ValueQueue] ${msg}`, data),
...
function calculateEstimatedImpact(valueResult: ContentValueResult): number {
  ...
  const costEfficiency = valueResult.aiCost > 0 ? Math.min(1, 0.1 / valueResult.aiCost) : 1;
...
export function queueLowCostWins(maxItems: number = MAX_QUEUE_SIZE): QueueResult {
  ...
  const lowCostCandidates = allPerformance
    .filter(perf => {
      ...
      return (
        valueResult.aiCost < LOW_COST_THRESHOLD && perf.score < HIGH_IMPACT_PERFORMANCE_THRESHOLD
      );
    })
...
export function shouldQueueImprovement(entityId: string): {
  ...
  return {
    should: false,
    reason:
      valueResult.recommendation === "maintain"
