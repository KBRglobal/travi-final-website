...
export function computeMetrics(
  ...
  let overrides = 0, degradedRecoveries = 0, degradedTotal = 0;
  ...
  const metrics: LearningMetrics = {
    ...
    falsePositives: fp,
    falseNegatives: fn,
    accuracy: isNaN(accuracy) ? 1 : accuracy,
    precision: isNaN(precision) ? 1 : precision,
    recall: isNaN(recall) ? 1 : recall,
    f1Score: isNaN(f1Score) ? 1 : f1Score,
...
export function detectPatterns(
  ...
  for (const pattern of patterns.slice(0, config.maxPatternsPerFeature)) {
    ...
    if (patternStore.size >= MAX_PATTERNS) {
      const oldest = Array.from(patternStore.entries())
...
export function generateRecommendations(
  ...
  for (const rec of recommendations.slice(0, config.maxRecommendationsPerFeature)) {
    if (recommendationStore.size >= MAX_RECOMMENDATIONS) {
      const oldest = Array.from(recommendationStore.entries())
