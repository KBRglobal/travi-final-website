...
export function simulateDecision(
  ...
  // Approaching limits - warn
    ...
    state.actions + 1 > effectiveMaxActions * 0.8 ||
    state.spend + estimatedCost > effectiveMaxSpend * 0.8
  ) {
...
interface FeatureStatsAccumulator {
  total: number;
  changed: number;
...
interface HourlyStatsAccumulator {
  changed: number;
  blocksChange: number;
  costChange: number;
}
...
function updateFeatureStats(
  featureStats: Map<GuardedFeature, FeatureStatsAccumulator>,
  decision: HistoricalDecision,
...
function updateHourlyStats(
  hourlyStats: Map<number, HourlyStatsAccumulator>,
  decision: HistoricalDecision,
  simulated: SimulatedDecision
): void {
  ...
  const blockChange =
    (simulated.simulatedDecision === "BLOCK" ? 1 : 0) -
...
function buildFeatureBreakdown(
  featureStats: Map<GuardedFeature, FeatureStatsAccumulator>
): Map<GuardedFeature, FeatureSimulationResult> {
  const breakdown = new Map<GuardedFeature, FeatureSimulationResult>();
  for (const [feature, stats] of featureStats) {
    breakdown.set(feature, {
      feature,
...
function buildHourlyImpact(hourlyStats: Map<number, HourlyStatsAccumulator>): HourlyImpact[] {
...
function computeComparison(
  ...
  simulatedDecisions: SimulatedDecision[]
  ...
  const predictedIncidents = Math.round(
    originalIncidents + (simulatedAllows - originalAllows) * incidentRate
  );
  ...
  const predictedOverrides = Math.round(
    Math.max(0, originalOverrides + (simulatedBlocks - originalBlocks) * overrideRate)
  );
...
export async function runSimulation(request: SimulationRequest): Promise<SimulationResult> {
  ...
  try {
    ...
    const featureStats = new Map<GuardedFeature, FeatureStatsAccumulator>();
    const hourlyStats = new Map<number, HourlyStatsAccumulator>();

    decisions.sort((a, b) => a.timestamp.getTime() - b.timestamp.getTime());

    for (const decision of decisions) {
      const hourKey = decision.timestamp.toISOString().slice(0, 13);
      if (!budgetStateByHour.has(hourKey)) budgetStateByHour.set(hourKey, new Map());

      const simulated = simulateDecision(decision, request.policy, budgetStateByHour.get(hourKey)!);
      simulatedDecisions.push(simulated);
      updateFeatureStats(featureStats, decision, simulated);
      updateHourlyStats(hourlyStats, decision, simulated);
    }

    const featureBreakdown = buildFeatureBreakdown(featureStats);
    const comparison = computeComparison(decisions, simulatedDecisions);
    const hourlyImpact = buildHourlyImpact(hourlyStats);
    const findings = generateFindings(
      comparison.decisionsChanged,
      comparison.totalDecisions,
      comparison.blocksChange,
      comparison.costChange,
      comparison.incidentChange,
      featureBreakdown
    );
    ...
    resultCache.set(simulationId, { result, expiresAt: Date.now() + config.cacheResultsMs });
 ...
 */
function findChangeRateFindings(changeRate: number): SimulationFinding[] {
...
function findCostFindings(costChange: number): SimulationFinding[] {
  if (costChange < -100) {
    return [
      {
        type: "opportunity",
...
function findIncidentFindings(incidentChange: number): SimulationFinding[] {
  if (incidentChange > 0) {
    return [
      {
        type: "risk",
...
function findFeatureRiskFindings(
...
function generateFindings(
  ...
  return [
    ...findChangeRateFindings(changeRate),
    ...findCostFindings(costChange),
    ...findIncidentFindings(incidentChange),
    ...findFeatureRiskFindings(featureBreakdown),
  ];
}
