...
export function explainDecision(
  ...
  const technicalDetails = (audience === 'developer' || audience === 'operator')
...
export function generateActivitySummary(
  ...
  const filteredOutcomes = features
    ? outcomes.filter(o => features.includes(o.feature))
    : outcomes;
...
function buildDetails(
  ...
  if (decision.budgetState) {
    ...
    if (audience === 'executive') {
      ...
      details.push(`Budget: ${decision.budgetState.used}/${decision.budgetState.limit} (${utilization}%) for ${decision.budgetState.period} period`);
    }
  }
...
function buildFeatureSummaries(
  outcomes: Array<{ feature: GuardedFeature; decision: string; metadata?: Record<string, unknown> }>,
  audience: ExplanationAudience
): FeatureSummary[] {
  const byFeature = new Map<GuardedFeature, {
    total: number;
    blocked: number;
    cost: number;
  }>();
  ...
  for (const [feature, stats] of byFeature) {
    ...
    const status: FeatureSummary['status'] =
      blockRate > 0.3 ? 'critical' :
      blockRate > 0.1 ? 'attention_needed' : 'healthy';
    ...
    const issues: string[] = [];
    if (blockRate > 0.2) {
      issues.push(audience === 'executive' ? 'High block rate' : `Block rate: ${(blockRate * 100).toFixed(1)}%`);
    }
...
function buildActionItems(
  signals: Array<{ severity: string; feature?: GuardedFeature; type: string; recommendation?: { reason: string } }>,
  ...
  for (const signal of signals.slice(0, 5)) {
    ...
    const category = signal.type.includes('budget') ? 'budget' :
      signal.type.includes('incident') ? 'incident' : 'policy';
  ...
  if (metrics.accuracy < 0.85) {
    items.push({
      ...
      description: audience === 'executive'
...
function buildHealthAssessment(
  ...
  if (criticalSignals > 0 || metrics.accuracy < 0.7) {
    status = 'critical';
    explanation = audience === 'executive'
