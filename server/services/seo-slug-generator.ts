...
export function generateSeoSlug(title: string, cityName: string): string {
  ...
  const noiseWords = [
    "entry ticket", "entry tickets", "admission ticket", "admission tickets",
    "skip the line", "skip-the-line", "fast track", "fast-track",
    "guided tour", "walking tour", "bus tour", "boat tour",
    "hop on hop off", "hop-on hop-off", "hop-on-hop-off",
    "tickets", "ticket", "admission", "entrance",
    "with transfer", "with transfers", "with transport",
    "from", "to", "and", "the", "a", "an",
    "private", "shared", "group", "vip",
...
export async function backfillSeoSlugs(options?: {
  ...
  if (total === 0) {
    console.log("[SEO Slugs] All attractions already have SEO slugs");
    return { updated: 0, errors: 0 };
  }
  
  console.log(`[SEO Slugs] Backfilling ${total} attractions...`);
  ...
  while (true) {
    ...
    for (const attraction of batch) {
      try {
        ...
        console.error(`[SEO Slugs] Error for ${attraction.id}:`, error);
        errors++;
      }
    }
    ...
    console.log(`[SEO Slugs] Progress: ${updated}/${total} (${errors} errors)`);
  }
  
  console.log(`[SEO Slugs] Backfill complete: ${updated} updated, ${errors} errors`);
 ...
 */
export async function previewSeoSlugs(limit: number = 10): Promise<Array<{
  id: string;
  title: string;
  cityName: string;
  currentSlug: string;
