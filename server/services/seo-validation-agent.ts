...
const REQUIRED_ELEMENTS: Record<PageType, {
  minFaq: number;
  maxFaq: number;
  ...
  attraction: { minFaq: 6, maxFaq: 10, minInternalLinks: 5, maxInternalLinks: 8, minTips: 3, minHighlights: 3, maxHighlights: 5 },
  hotel: { minFaq: 6, maxFaq: 8, minInternalLinks: 5, maxInternalLinks: 8, minTips: 3, minImages: 5, maxImages: 8 },
...
export class SEOValidationAgent {
  ...
  private validateTier1Critical(content: ContentData, pageType: PageType): SEOCheck[] {
    ...
    const titleHasKeyword = primaryKeyword ? metaTitle.toLowerCase().includes(primaryKeyword.toLowerCase()) : true;
    ...
    const h1HasKeyword = primaryKeyword ? h1.toLowerCase().includes(primaryKeyword.toLowerCase()) : false;
    checks.push({
    ...
    const keywordInIntro = primaryKeyword ? first150Words.toLowerCase().includes(primaryKeyword.toLowerCase()) : true;
    checks.push({
  ...
  private validateTier2Essential(content: ContentData, pageType: PageType): SEOCheck[] {
    ...
    const linksValid = internalLinks >= reqs.minInternalLinks && internalLinks <= reqs.maxInternalLinks;
  ...
  private validateTier3Technical(content: ContentData): SEOCheck[] {
    ...
    const ogValid = !!(ogTags["og:title"] || ogTags.ogTitle) &&
  ...
  private validateTier4Quality(content: ContentData): SEOCheck[] {
    ...
    checks.push({
      ...
      tier: "tier4_quality",
      passed: noCliches,
      message: noCliches ? "No cliches found" : `Cliches found: ${clichesFound.slice(0, 3).join(", ")}`,
      autoFixable: false
    });
    ...
    checks.push({
      ...
      message: hasSpecificPrices ? "Specific prices included" : "Use specific prices instead of vague terms",
      autoFixable: false
    });

    // 3. Honest Cons (not just pros)
    const hasCons = !!content.cons?.length ||
    ...
    const hasCurrentYear = fullText.includes(String(currentYear)) || fullText.includes(String(currentYear + 1));
    ...
    if (paragraphs.length > 0) {
      const avgSentences = paragraphs.reduce((sum, p) =>
        sum + p.split(/[.!?]+/).filter(s => s.trim()).length, 0
      ) / paragraphs.length;
  ...
  private getFullText(content: ContentData): string {
    ...
    if (content.tips) {
      parts.push(content.tips.map((t: any) => typeof t === "string" ? t : t.text || "").join(" "));
    }
  ...
  private countH2Sections(content: ContentData): number {
    ...
    // Count from blocks
    if (content.blocks) {
      count += content.blocks.filter((b: any) =>
        b.type === "heading" || b.type === "section" || b.type === "h2"
      ).length;
    }
  ...
  private countCTAs(content: ContentData): number {
    ...
    const ctaPhrases = [
      "book now", "plan your", "get tickets", "reserve", "learn more",
      "discover", "explore", "read more", "check out", "see more",
      "click here", "visit", "sign up", "get started", "try now"
    ];
  ...
  private generateMetaTitle(content: ContentData): string {
    ...
    let base = title.replace(/Complete Guide to\s*/i, "").replace(/\s*Guide$/i, "").trim();
  ...
  private extractPrimaryKeyword(content: ContentData): string {
    ...
    for (const word of words) {
      if (word.length > 4 && !["guide", "complete", "best", "dubai", "ultimate"].includes(word.toLowerCase())) {
        return word;
      }
    }
  ...
  private extractSecondaryKeywords(content: ContentData): string[] {
    ...
    const potential = [
      "tickets", "prices", "hours", "location", "tips",
      "restaurants nearby", "hotels nearby", "attractions", "things to do",
      "best time to visit", "how to get there", "what to wear", "booking",
      "entry fee", "opening hours", "sunset view", "photography"
    ];
  ...
  private generateAltText(content: ContentData): string {
    const title = content.title || "Dubai attraction";
    const category = content.type || content.category || "tourism";

    return `${title.replace(/ - Complete Guide.*$/i, "")} - ${category} photography in Dubai`.substring(0, 125);
  }
}
...
export function validateSEO(content: ContentData, pageType: PageType = "attraction"): SEOValidationResult {
