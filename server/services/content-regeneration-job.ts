...
async function saveContentToDatabase(
  ...
  if (qualityScore.overallScore < QUALITY_THRESHOLD) {
    console.warn(`[Regeneration] BLOCKED: Cannot save content for ${attractionId} - score ${qualityScore.overallScore} below threshold ${QUALITY_THRESHOLD}`);
    return false;
  }
  ...
  try {
    ...
    console.error(`[Regeneration] Failed to save content for ${attractionId}:`, error);
    return false;
  }
}

export async function regenerateAllAttractions(options: {
  batchSize?: number;
  ...
  if (regenerationStats.inProgress) {
    console.log("[Regeneration] Job already in progress");
    return;
  }

  console.log("[Regeneration] Starting content regeneration job...");
  ...
  if (cityFilter) {
    filtered = filtered.filter(a => 
      a.cityName?.toLowerCase() === cityFilter.toLowerCase()
    );
  }
  
  console.log(`[Regeneration] Skipped ${attractions.length - filtered.length} attractions that already passed (score >= ${QUALITY_THRESHOLD})`);
  console.log(`[Regeneration] Remaining to process: ${filtered.length}`);
  
  // If nothing left to process, we're done
  if (filtered.length === 0) {
    console.log("[Regeneration] All attractions already processed!");
  ...
  console.log(`[Regeneration] Processing ${total} attractions in ${totalBatches} batches`);
  ...
  for (let batchIndex = 0; batchIndex < totalBatches; batchIndex++) {
    ...
    console.log(`[Regeneration] Processing batch ${batchIndex + 1}/${totalBatches} (${batch.length} attractions, ${PARALLEL_LIMIT} parallel)`);
    const chunks = [];
    ...
    for (const chunk of chunks) {
        ...
        try {
          ...
          if (result && result.qualityScore.passed) {
            ...
            if (saved) {
              ...
              console.log(`[Regeneration] ✅ ${attraction.title} - Score: ${result.qualityScore.overallScore}`);
            } else {
              regenerationStats.errors++;
            }
          } else if (result) {
            regenerationStats.failed++;
            console.log(`[Regeneration] ⚠️ ${attraction.title} - Score: ${result.qualityScore.overallScore} (below ${QUALITY_THRESHOLD}, NOT SAVED)`);
          } else {
            regenerationStats.errors++;
            console.log(`[Regeneration] ❌ ${attraction.title} - Generation failed`);
          }
          
          regenerationStats.processed++;
          
        } catch (error) {
          console.error(`[Regeneration] Error processing ${attraction.title}:`, error);
          regenerationStats.errors++;
          regenerationStats.processed++;
        }
      });
      ...
      regenerationStats.estimatedCompletion = new Date(
        Date.now() + (avgTimePerItem * remaining)
      );
    }

    // Delay between batches to avoid rate limiting
    if (batchIndex < totalBatches - 1) {
      console.log(`[Regeneration] Waiting ${delayBetweenBatches}ms before next batch...`);
  ...
  console.log(`[Regeneration] Job completed!`);
  console.log(`[Regeneration] Results: ${regenerationStats.passed} passed, ${regenerationStats.failed} failed, ${regenerationStats.errors} errors`);
}
...
export async function regenerateSingleAttraction(attractionId: string): Promise<{
  ...
  try {
    ...
    return {
      ...
      error: result.qualityScore.passed ? undefined : "Quality score below 90 threshold - content NOT saved",
    };
  } catch (error) {
    console.error(`[Regeneration] Error regenerating ${attractionId}:`, error);
    return { 
      success: false, 
