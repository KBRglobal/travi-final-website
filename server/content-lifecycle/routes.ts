...
export function registerLifecycleRoutes(app: Express): void {
  ...
  app.post(
    "/api/admin/lifecycle/events/query",
    requireAuth,
  ...
  // Get recent events
  app.get(
    "/api/admin/lifecycle/events/recent",
    requireAuth,
    async (req: Request, res: Response) => {
      if (!isLifecycleTimelineEnabled()) {
        return res.json({ enabled: false, message: "Lifecycle timeline disabled" });
  ...
  app.post(
    "/api/admin/lifecycle/:contentId/event",
    requireAuth,
    async (req: Request, res: Response) => {
      if (!isLifecycleTimelineEnabled()) {
        return res.json({ enabled: false, message: "Lifecycle timeline disabled" });
  ...
  // Export audit trail
  app.get(
    "/api/admin/lifecycle/:contentId/export",
    requireAuth,
    async (req: Request, res: Response) => {
      if (!isLifecycleTimelineEnabled()) {
        return res.json({ enabled: false, message: "Lifecycle timeline disabled" });
      ...
      try {
        ...
        if (format === "csv") {
          ...
          res.setHeader(
            "Content-Disposition",
            `attachment; filename="audit-${req.params.contentId}.csv"`
          );
          res.send(audit.data);
        } else {
          res.json(audit);
  ...
  app.get(
    "/api/admin/lifecycle/:contentId/compare",
    requireAuth,
    async (req: Request, res: Response) => {
      if (!isLifecycleTimelineEnabled()) {
        return res.json({ enabled: false, message: "Lifecycle timeline disabled" });
