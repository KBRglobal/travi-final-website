...
import { createPreAuthToken, verifyPreAuthToken, consumePreAuthToken } from "./security/pre-auth-token";
...
import { autonomyPolicyRoutes, initAutonomyPolicy, shutdownAutonomyPolicy } from "./autonomy/policy";
...
import { registerPublishingRoutes, guardManualPublish, isPublishGuardsEnabled, registerChecklistRoutes } from "./publishing";
...
export async function registerRoutes(
    ...
    try {
      ...
      const transformedGuides = guides.map((guide) => {
        ...
        return {
          ...
          categoryLabel: article?.category ? article.category.charAt(0).toUpperCase() + article.category.slice(1) : "Tips",
          readTime: guide.wordCount ? `${Math.ceil(guide.wordCount / 200)} min read` : "5 min read",
          image: guide.cardImage || guide.heroImage || "https://images.unsplash.com/photo-1512453979798-5ea266f8880c?w=1200&q=80",
          author: (guide as any).author?.name || "TRAVI Editorial Team",
          date: guide.publishedAt ? new Date(guide.publishedAt).toLocaleDateString("en-US", { month: 'short', day: 'numeric', year: 'numeric' }) : "Dec 28, 2025"
        };
      });
  ...
  app.get("/api/guides/featured", async (_req, res) => {
    try {
      ...
      res.json({
        ...
        readTime: featured.wordCount ? `${Math.ceil(featured.wordCount / 200)} min read` : "25 min read",
        image: featured.heroImage || featured.cardImage || "https://images.unsplash.com/photo-1488646953014-85cb44e25828?w=1200&h=800&fit=crop&q=90",
  ...
  const sampleHotels = [
    {
      ...
      location: { name: "Downtown", city: "Dubai" },
    ...
    },
    {
      ...
      location: { name: "Beach District", city: "Dubai" },
    ...
    },
    {
      ...
      location: { name: "Arts District", city: "Dubai" },
      amenities: ["wifi", "restaurant", "gym"]
    },
    {
      id: "sample-4",
      name: "Luxury Tower Suites",
      stars: 5,
      rating: 9.5,
      reviews: 2103,
      price: 425,
      ...
      location: { name: "Financial Center", city: "Dubai" },
  ...
  app.get("/api/hotels/search", async (req: Request, res: Response) => {
    try {
      ...
      const amenitiesFilter = amenities ? String(amenities).split(',').map(a => a.trim().toLowerCase()) : [];
      ...
      // 16 whitelisted destinations only (matching Tiqets coverage)
      ...
      let hotels = allHotels
        .filter((hotel: any) => {
          ...
          if (!["hotel", "resort", "boutique hotel", "luxury hotel"].some(t => accommodationType.includes(t) || accommodationType === "")) {
            return false;
          }
          const rating = hotel.review_summary?.rating || 0;
          // Filter by rating if star filter is applied (TripAdvisor uses 1-5 scale)
        ...
        .map((hotel: any) => {
          ...
          return {
            ...
            rating: hotel.review_summary?.rating ? (hotel.review_summary.rating * 2).toFixed(1) : null,
            reviews: hotel.review_summary?.count || 0,
            price: hotel.price_ranges?.minimum || hotel.price_ranges?.maximum || null,
            currency: String(currency).toUpperCase(),
            image: hotel.image || "https://images.unsplash.com/photo-1566073771259-6a8506099945?w=800&h=600&fit=crop&q=80",
            location: {
              name: cityInfo.name,
              city: cityInfo.name
            },
      ...
      if (minRatingNum !== null) {
        hotels = hotels.filter((h: any) => h.rating !== null && parseFloat(h.rating) >= minRatingNum);
      }
      ...
      res.json({ 
        hotels: paginatedHotels.length > 0 ? paginatedHotels : (pageNum === 1 ? sampleHotels.slice(0, pageSizeNum) : []),
  ...
  app.get("/api/hotels/:hotelId", async (req: Request, res: Response) => {
    try {
      ...
      const tpoToken = process.env.TPO_API_TOKEN;
      if (!tpoToken) {
        const sampleHotel = sampleHotels.find(h => String(h.id) === String(hotelId)) || sampleHotels[0];
        if (!sampleHotel) {
          return res.status(404).json({ error: "Hotel not found" });
        }
        
        let description = "Experience luxury and comfort at this stunning property. Featuring world-class amenities, exceptional service, and breathtaking views, this hotel offers an unforgettable stay for discerning travelers.";
        ...
        return res.json({ 
          hotel: {
            ...
            seoDescription,
            highlights,
            gallery: [sampleHotel?.image || "https://images.unsplash.com/photo-1566073771259-6a8506099945?w=800&h=600&fit=crop&q=80"],
            facilities: ["WiFi", "Pool", "Spa", "Restaurant", "Fitness Center", "Parking", "Room Service", "Concierge"]
          },
          source: "sample",
          aiGenerated
        });
      ...
      const hotel: any = {
        ...
        currency: "USD",
        image: hotelData.photos?.[0]?.url || (hotelData.id 
        ...
        facilities: hotelData.facilities || hotelData.amenities || ["WiFi", "Pool", "Spa", "Restaurant"],
        description: hotelData.description || hotelData.desc || "Experience luxury and comfort at this stunning property. Featuring world-class amenities and exceptional service.",
        gallery: (hotelData.photos || []).map((p: any) => p.url || `https://photo.hotellook.com/image_v2/limit/${hotelData.id}/${p.id || 1}/800x600.jpg`),
      ...
      if (hotel.gallery.length === 0) {
        ...
        for (let i = 2; i <= 5; i++) {
          hotel.gallery.push(`https://photo.hotellook.com/image_v2/limit/${hotelData.id}/${i}/800x600.jpg`);
        }
      }
  ...
  // ============================================================================
  // ATTRACTIONS API - Tiqets Integration with Affiliate Link
  // ============================================================================
  ...
  const tiqetsDestinations: Record<string, { name: string; slug: string; country: string }> = {
  ...
  // Sample attraction data for each destination (curated based on Tiqets offerings)
  // Expanded to 12-15 attractions per destination for comprehensive coverage
  const attractionsByDestination: Record<string, any[]> = {
    "dubai": [
      { id: "burj-khalifa", name: "Burj Khalifa At The Top", category: "Observation Deck", price: 149, currency: "AED", duration: "2-3 hours", image: "/destinations-hero/dubai/dubai/dubai-hero-burj-khalifa-palms-sunset.webp.webp", rating: 4.8, reviews: 45000 },
      { id: "dubai-frame", name: "Dubai Frame", category: "Landmark", price: 50, currency: "AED", duration: "1-2 hours", image: "/destinations-hero/dubai/dubai/dubai-hero-dubai-frame-skyline-aerial.webp.webp", rating: 4.6, reviews: 18000 },
      { id: "dubai-aquarium", name: "Dubai Aquarium & Underwater Zoo", category: "Aquarium", price: 125, currency: "AED", duration: "2-3 hours", image: "/cards/dubai.webp", rating: 4.7, reviews: 28000 },
      { id: "museum-future", name: "Museum of the Future", category: "Museum", price: 149, currency: "AED", duration: "2-3 hours", image: "/cards/dubai.webp", rating: 4.9, reviews: 12000 },
      { id: "desert-safari", name: "Dubai Desert Safari", category: "Tour", price: 180, currency: "AED", duration: "6 hours", image: "/destinations-hero/dubai/dubai/dubai-hero-habibi-welcome-desert-safari.webp.webp", rating: 4.7, reviews: 35000 },
      { id: "desert-safari-premium", name: "Premium Desert Safari with BBQ Dinner", category: "Tour", price: 350, currency: "AED", duration: "7 hours", image: "/cards/dubai.webp", rating: 4.8, reviews: 22000 },
      { id: "palm-jumeirah-tour", name: "Palm Jumeirah Boat Tour", category: "Tour", price: 199, currency: "AED", duration: "2 hours", image: "/destinations-hero/dubai/dubai/dubai-hero-atlantis-palm-jumeirah-beach.webp.webp", rating: 4.5, reviews: 15000 },
      { id: "atlantis-aquaventure", name: "Atlantis Aquaventure Waterpark", category: "Theme Park", price: 299, currency: "AED", duration: "6-8 hours", image: "/cards/dubai.webp", rating: 4.7, reviews: 42000 },
      { id: "img-worlds", name: "IMG Worlds of Adventure", category: "Theme Park", price: 299, currency: "AED", duration: "6-8 hours", image: "/cards/dubai.webp", rating: 4.5, reviews: 25000 },
      { id: "dubai-marina-cruise", name: "Dubai Marina Dinner Cruise", category: "Tour", price: 250, currency: "AED", duration: "2-3 hours", image: "/destinations-hero/dubai/dubai/dubai-hero-marina-abra-boat-night.webp.webp", rating: 4.6, reviews: 18000 },
      { id: "ski-dubai", name: "Ski Dubai Snow Park", category: "Theme Park", price: 210, currency: "AED", duration: "2-3 hours", image: "/cards/dubai.webp", rating: 4.4, reviews: 32000 },
      { id: "miracle-garden", name: "Dubai Miracle Garden", category: "Garden", price: 55, currency: "AED", duration: "2-3 hours", image: "/cards/dubai.webp", rating: 4.6, reviews: 28000 },
      { id: "global-village", name: "Global Village", category: "Theme Park", price: 25, currency: "AED", duration: "4-6 hours", image: "/cards/dubai.webp", rating: 4.5, reviews: 45000 },
      { id: "ain-dubai", name: "Ain Dubai Observation Wheel", category: "Observation Deck", price: 130, currency: "AED", duration: "1 hour", image: "/cards/dubai.webp", rating: 4.6, reviews: 12000 }
    ],
    "abu-dhabi": [
      { id: "louvre-abu-dhabi", name: "Louvre Abu Dhabi", category: "Museum", price: 63, currency: "AED", duration: "3-4 hours", image: "/cards/abu-dhabi.webp", rating: 4.8, reviews: 22000 },
      { id: "ferrari-world", name: "Ferrari World Abu Dhabi", category: "Theme Park", price: 310, currency: "AED", duration: "5-6 hours", image: "/cards/abu-dhabi.webp", rating: 4.6, reviews: 18000 },
      { id: "sheikh-zayed-mosque", name: "Sheikh Zayed Grand Mosque", category: "Religious Site", price: 0, currency: "AED", duration: "2-3 hours", image: "/cards/abu-dhabi.webp", rating: 4.9, reviews: 45000 },
      { id: "yas-waterworld", name: "Yas Waterworld", category: "Theme Park", price: 280, currency: "AED", duration: "5-6 hours", image: "/cards/abu-dhabi.webp", rating: 4.7, reviews: 24000 },
      { id: "warner-bros-world", name: "Warner Bros. World Abu Dhabi", category: "Theme Park", price: 295, currency: "AED", duration: "6-8 hours", image: "/cards/abu-dhabi.webp", rating: 4.8, reviews: 28000 },
      { id: "qasr-al-watan", name: "Qasr Al Watan Presidential Palace", category: "Historic Site", price: 60, currency: "AED", duration: "2-3 hours", image: "/cards/abu-dhabi.webp", rating: 4.7, reviews: 15000 },
      { id: "emirates-park-zoo", name: "Emirates Park Zoo", category: "Zoo", price: 35, currency: "AED", duration: "3-4 hours", image: "/cards/abu-dhabi.webp", rating: 4.4, reviews: 12000 },
      { id: "seaworld-abu-dhabi", name: "SeaWorld Abu Dhabi", category: "Theme Park", price: 375, currency: "AED", duration: "6-8 hours", image: "/cards/abu-dhabi.webp", rating: 4.7, reviews: 8000 },
      { id: "yas-island-combo", name: "Yas Island 2-Park Pass", category: "Theme Park", price: 495, currency: "AED", duration: "2 days", image: "/cards/abu-dhabi.webp", rating: 4.8, reviews: 16000 },
      { id: "mangrove-kayaking", name: "Mangrove Kayaking Adventure", category: "Tour", price: 150, currency: "AED", duration: "3 hours", image: "/cards/abu-dhabi.webp", rating: 4.6, reviews: 9000 },
      { id: "desert-safari-abu-dhabi", name: "Abu Dhabi Desert Safari", category: "Tour", price: 220, currency: "AED", duration: "6 hours", image: "/cards/abu-dhabi.webp", rating: 4.6, reviews: 18000 },
      { id: "observation-deck-300", name: "Observation Deck at 300", category: "Observation Deck", price: 95, currency: "AED", duration: "1-2 hours", image: "/cards/abu-dhabi.webp", rating: 4.5, reviews: 11000 },
      { id: "heritage-village", name: "Heritage Village", category: "Historic Site", price: 0, currency: "AED", duration: "1-2 hours", image: "/cards/abu-dhabi.webp", rating: 4.3, reviews: 8000 }
    ],
    "london": [
      { id: "tower-of-london", name: "Tower of London", category: "Historic Site", price: 34, currency: "GBP", duration: "3-4 hours", image: "/cards/london.webp", rating: 4.7, reviews: 67000 },
      { id: "london-eye", name: "London Eye", category: "Observation Deck", price: 32, currency: "GBP", duration: "1 hour", image: "/cards/london.webp", rating: 4.5, reviews: 89000 },
      { id: "westminster-abbey", name: "Westminster Abbey", category: "Historic Site", price: 27, currency: "GBP", duration: "2-3 hours", image: "/cards/london.webp", rating: 4.8, reviews: 34000 },
      { id: "buckingham-palace", name: "Buckingham Palace", category: "Historic Site", price: 30, currency: "GBP", duration: "2-3 hours", image: "/cards/london.webp", rating: 4.6, reviews: 28000 },
      { id: "british-museum", name: "British Museum Guided Tour", category: "Museum", price: 20, currency: "GBP", duration: "2-3 hours", image: "/cards/london.webp", rating: 4.8, reviews: 52000 },
      { id: "st-pauls-cathedral", name: "St Paul's Cathedral", category: "Historic Site", price: 23, currency: "GBP", duration: "2-3 hours", image: "/cards/london.webp", rating: 4.7, reviews: 28000 },
      { id: "tower-bridge", name: "Tower Bridge Exhibition", category: "Historic Site", price: 12, currency: "GBP", duration: "1-2 hours", image: "/destinations-hero/london/london-hero-tower-bridge-thames-sunset.webp.webp", rating: 4.6, reviews: 35000 },
      { id: "madame-tussauds-london", name: "Madame Tussauds London", category: "Museum", price: 38, currency: "GBP", duration: "2-3 hours", image: "/cards/london.webp", rating: 4.4, reviews: 45000 },
      { id: "natural-history-museum", name: "Natural History Museum Tour", category: "Museum", price: 15, currency: "GBP", duration: "2-3 hours", image: "/cards/london.webp", rating: 4.7, reviews: 42000 },
      { id: "kensington-palace", name: "Kensington Palace", category: "Historic Site", price: 22, currency: "GBP", duration: "2-3 hours", image: "/cards/london.webp", rating: 4.5, reviews: 18000 },
      { id: "hampton-court", name: "Hampton Court Palace", category: "Historic Site", price: 28, currency: "GBP", duration: "4-5 hours", image: "/cards/london.webp", rating: 4.7, reviews: 22000 },
      { id: "thames-river-cruise", name: "Thames River Cruise", category: "Tour", price: 18, currency: "GBP", duration: "1-2 hours", image: "/cards/london.webp", rating: 4.5, reviews: 38000 },
      { id: "harry-potter-studio", name: "Warner Bros. Harry Potter Studio", category: "Tour", price: 55, currency: "GBP", duration: "4-5 hours", image: "/cards/london.webp", rating: 4.9, reviews: 85000 },
      { id: "the-shard", name: "The View from The Shard", category: "Observation Deck", price: 32, currency: "GBP", duration: "1-2 hours", image: "/cards/london.webp", rating: 4.6, reviews: 32000 }
    ],
    "paris": [
      // Top 10 Paris Attractions - Real Tiqets Data (January 2026)
      { id: "eiffel-tower", name: "Eiffel Tower Summit Access", category: "Landmark", price: 36, currency: "EUR", duration: "2-3 hours", image: "/cards/paris.webp", rating: 4.8, reviews: 156000, highlights: ["Priority lift access to summit", "360° panoramic city views", "Champagne bar at top level"], bookingNote: "Book 1-3 months ahead for peak season" },
      { id: "louvre", name: "Louvre Museum Timed Entry", category: "Museum", price: 22, currency: "EUR", duration: "3-4 hours", image: "/cards/paris.webp", rating: 4.9, reviews: 198000, highlights: ["Home to Mona Lisa & Venus de Milo", "Over 35,000 artworks", "Skip ticket-buying line"], bookingNote: "Free for EU residents under 26" },
      { id: "versailles", name: "Palace of Versailles Full Estate", category: "Historic Site", price: 27, currency: "EUR", duration: "4-5 hours", image: "/cards/paris.webp", rating: 4.8, reviews: 112000, highlights: ["Hall of Mirrors", "Marie Antoinette's Estate", "Musical Fountains on select days"], bookingNote: "Closed Mondays; 45 min from Paris" },
      { id: "musee-dorsay", name: "Musée d'Orsay Priority Entry", category: "Museum", price: 16, currency: "EUR", duration: "2-3 hours", image: "/cards/paris.webp", rating: 4.8, reviews: 78000, highlights: ["4,000+ Impressionist masterpieces", "Van Gogh, Monet, Renoir", "Former train station architecture"], bookingNote: "Save 20% when booking online" },
      { id: "arc-de-triomphe", name: "Arc de Triomphe Rooftop Access", category: "Landmark", price: 16, currency: "EUR", duration: "1-2 hours", image: "/cards/paris.webp", rating: 4.7, reviews: 52000, highlights: ["284 steps to rooftop", "Panoramic Champs-Élysées view", "Tomb of Unknown Soldier"], bookingNote: "Combo with Musée d'Orsay available" },
      { id: "paris-catacombs", name: "Paris Catacombs with Audio Guide", category: "Historic Site", price: 31, currency: "EUR", duration: "1.5 hours", image: "/cards/paris.webp", rating: 4.7, reviews: 48000, highlights: ["1.5km underground ossuary", "Remains of 6 million Parisians", "Only 200 visitors at once"], bookingNote: "BOOK WEEKS IN ADVANCE - sells out fast" },
      { id: "sainte-chapelle", name: "Sainte-Chapelle Gothic Chapel", category: "Historic Site", price: 13, currency: "EUR", duration: "1 hour", image: "/cards/paris.webp", rating: 4.9, reviews: 42000, highlights: ["1,113 stained glass panels", "13th century Gothic masterpiece", "Part of Conciergerie complex"], bookingNote: "Best visited on sunny days" },
      { id: "seine-river-cruise", name: "Seine River Cruise", category: "Tour", price: 18, currency: "EUR", duration: "1 hour", image: "/cards/paris.webp", rating: 4.6, reviews: 86000, highlights: ["Pass Eiffel Tower, Notre-Dame, Louvre", "Audio guide in 12 languages", "Dinner cruise options available"], bookingNote: "Sunset cruises most popular" },
      { id: "centre-pompidou", name: "Centre Pompidou Modern Art Museum", category: "Museum", price: 15, currency: "EUR", duration: "2-3 hours", image: "/cards/paris.webp", rating: 4.6, reviews: 38000, highlights: ["Largest modern art collection in Europe", "Picasso, Kandinsky, Matisse", "Iconic inside-out architecture"], bookingNote: "Rooftop has great city views" },
      { id: "moulin-rouge", name: "Moulin Rouge Cabaret Show", category: "Entertainment", price: 87, currency: "EUR", duration: "2 hours", image: "/cards/paris.webp", rating: 4.7, reviews: 28000, highlights: ["World-famous Féerie show", "60 Doriss Girl dancers", "Optional champagne dinner"], bookingNote: "Smart dress code required" },
      // Additional Popular Attractions
      { id: "montmartre-tour", name: "Montmartre & Sacré-Cœur Walking Tour", category: "Tour", price: 25, currency: "EUR", duration: "2 hours", image: "/cards/paris.webp", rating: 4.7, reviews: 32000, highlights: ["Bohemian artist quarter", "Place du Tertre painters", "Sacré-Cœur Basilica visit"], bookingNote: "Steep stairs to summit" },
      { id: "orangerie", name: "Musée de l'Orangerie", category: "Museum", price: 12, currency: "EUR", duration: "1.5 hours", image: "/cards/paris.webp", rating: 4.8, reviews: 31000, highlights: ["Monet's Water Lilies murals", "Tuileries Garden location", "Intimate art experience"], bookingNote: "Less crowded than Louvre" },
      { id: "conciergerie", name: "Conciergerie Prison Museum", category: "Historic Site", price: 12, currency: "EUR", duration: "1-2 hours", image: "/cards/paris.webp", rating: 4.5, reviews: 18000, highlights: ["Marie Antoinette's prison cell", "Medieval royal palace", "French Revolution history"], bookingNote: "Combo with Sainte-Chapelle saves money" },
      { id: "pantheon", name: "Panthéon & Crypt", category: "Historic Site", price: 13, currency: "EUR", duration: "1-2 hours", image: "/cards/paris.webp", rating: 4.6, reviews: 22000, highlights: ["Foucault's Pendulum", "Voltaire & Victor Hugo tombs", "Stunning dome views"], bookingNote: "Latin Quarter location" },
      { id: "disneyland-paris", name: "Disneyland Paris 1-Day Ticket", category: "Theme Park", price: 105, currency: "EUR", duration: "10-12 hours", image: "/cards/paris.webp", rating: 4.6, reviews: 92000, highlights: ["2 parks: Disneyland & Walt Disney Studios", "Over 50 attractions", "Character meet & greets"], bookingNote: "35 min from city center by train" },
      { id: "paris-versailles-bundle", name: "Paris & Versailles Bundle", category: "Combo", price: 57, currency: "EUR", duration: "Full day", image: "/cards/paris.webp", rating: 4.7, reviews: 15000, highlights: ["Louvre entry", "Versailles palace", "Seine River cruise"], bookingNote: "Save €4 vs buying separately" },
      { id: "opera-garnier", name: "Palais Garnier Opera House", category: "Historic Site", price: 14, currency: "EUR", duration: "1-2 hours", image: "/cards/paris.webp", rating: 4.7, reviews: 26000, highlights: ["Phantom of the Opera inspiration", "Marc Chagall ceiling", "Grand Staircase"], bookingNote: "Self-guided or guided tours" },
      { id: "rodin-museum", name: "Rodin Museum & Gardens", category: "Museum", price: 14, currency: "EUR", duration: "2 hours", image: "/cards/paris.webp", rating: 4.7, reviews: 24000, highlights: ["The Thinker & The Kiss", "Beautiful sculpture gardens", "Artist's former home"], bookingNote: "Near Invalides" },
      { id: "paris-food-tour", name: "Paris Food & Pastry Tour", category: "Tour", price: 95, currency: "EUR", duration: "3 hours", image: "/cards/paris.webp", rating: 4.8, reviews: 12000, highlights: ["8-10 tastings included", "Cheese, wine, macarons", "Local neighborhood gems"], bookingNote: "Small group max 12 people" },
      { id: "grevin-museum", name: "Grévin Wax Museum", category: "Museum", price: 26, currency: "EUR", duration: "1.5 hours", image: "/cards/paris.webp", rating: 4.4, reviews: 18000, highlights: ["300+ wax figures", "French celebrities", "Interactive experiences"], bookingNote: "Family-friendly attraction" }
    ],
    "new-york": [
      { id: "empire-state", name: "Empire State Building", category: "Observation Deck", price: 44, currency: "USD", duration: "1-2 hours", image: "/cards/new-york.webp", rating: 4.7, reviews: 78000 },
      { id: "statue-liberty", name: "Statue of Liberty & Ellis Island", category: "Landmark", price: 25, currency: "USD", duration: "4-5 hours", image: "/cards/new-york.webp", rating: 4.8, reviews: 95000 },
      { id: "met-museum", name: "Metropolitan Museum of Art", category: "Museum", price: 30, currency: "USD", duration: "3-4 hours", image: "/cards/new-york.webp", rating: 4.9, reviews: 120000 },
      { id: "top-of-rock", name: "Top of the Rock", category: "Observation Deck", price: 40, currency: "USD", duration: "1-2 hours", image: "/cards/new-york.webp", rating: 4.6, reviews: 55000 },
      { id: "911-memorial", name: "9/11 Memorial & Museum", category: "Museum", price: 33, currency: "USD", duration: "2-3 hours", image: "/cards/new-york.webp", rating: 4.8, reviews: 65000 },
      { id: "moma", name: "Museum of Modern Art (MoMA)", category: "Museum", price: 30, currency: "USD", duration: "2-3 hours", image: "/cards/new-york.webp", rating: 4.7, reviews: 52000 },
      { id: "central-park-tour", name: "Central Park Walking Tour", category: "Tour", price: 35, currency: "USD", duration: "2 hours", image: "/cards/new-york.webp", rating: 4.5, reviews: 28000 },
      { id: "one-world-observatory", name: "One World Observatory", category: "Observation Deck", price: 43, currency: "USD", duration: "1-2 hours", image: "/cards/new-york.webp", rating: 4.7, reviews: 48000 },
      { id: "edge-hudson-yards", name: "Edge at Hudson Yards", category: "Observation Deck", price: 38, currency: "USD", duration: "1-2 hours", image: "/cards/new-york.webp", rating: 4.6, reviews: 22000 },
      { id: "guggenheim", name: "Guggenheim Museum", category: "Museum", price: 30, currency: "USD", duration: "2-3 hours", image: "/cards/new-york.webp", rating: 4.6, reviews: 35000 },
      { id: "broadway-show", name: "Broadway Show Tickets", category: "Entertainment", price: 99, currency: "USD", duration: "2-3 hours", image: "/cards/new-york.webp", rating: 4.8, reviews: 85000 },
      { id: "intrepid-museum", name: "Intrepid Sea, Air & Space Museum", category: "Museum", price: 36, currency: "USD", duration: "3-4 hours", image: "/cards/new-york.webp", rating: 4.6, reviews: 28000 },
      { id: "nyc-food-tour", name: "NYC Food & Culture Tour", category: "Tour", price: 75, currency: "USD", duration: "3 hours", image: "/cards/new-york.webp", rating: 4.7, reviews: 18000 }
    ],
    "tokyo": [
      { id: "tokyo-skytree", name: "Tokyo Skytree", category: "Observation Deck", price: 3400, currency: "JPY", duration: "2-3 hours", image: "/cards/tokyo.webp", rating: 4.6, reviews: 45000 },
      { id: "teamlab-planets", name: "teamLab Planets", category: "Museum", price: 3800, currency: "JPY", duration: "2-3 hours", image: "/cards/tokyo.webp", rating: 4.8, reviews: 68000 },
      { id: "senso-ji", name: "Senso-ji Temple", category: "Religious Site", price: 0, currency: "JPY", duration: "1-2 hours", image: "/cards/tokyo.webp", rating: 4.7, reviews: 89000 },
      { id: "tokyo-tower", name: "Tokyo Tower", category: "Observation Deck", price: 2800, currency: "JPY", duration: "1-2 hours", image: "/cards/tokyo.webp", rating: 4.5, reviews: 42000 },
      { id: "teamlab-borderless", name: "teamLab Borderless", category: "Museum", price: 3800, currency: "JPY", duration: "2-3 hours", image: "/cards/tokyo.webp", rating: 4.9, reviews: 55000 },
      { id: "shibuya-crossing-tour", name: "Shibuya Crossing Walking Tour", category: "Tour", price: 5000, currency: "JPY", duration: "2 hours", image: "/cards/tokyo.webp", rating: 4.5, reviews: 18000 },
      { id: "meiji-shrine", name: "Meiji Shrine", category: "Religious Site", price: 0, currency: "JPY", duration: "1-2 hours", image: "/cards/tokyo.webp", rating: 4.6, reviews: 52000 },
      { id: "tsukiji-food-tour", name: "Tsukiji Outer Market Food Tour", category: "Tour", price: 12000, currency: "JPY", duration: "3 hours", image: "/cards/tokyo.webp", rating: 4.7, reviews: 22000 },
      { id: "edo-tokyo-museum", name: "Edo-Tokyo Museum", category: "Museum", price: 600, currency: "JPY", duration: "2-3 hours", image: "/cards/tokyo.webp", rating: 4.5, reviews: 28000 },
      { id: "tokyo-disneyland", name: "Tokyo Disneyland", category: "Theme Park", price: 8400, currency: "JPY", duration: "10-12 hours", image: "/cards/tokyo.webp", rating: 4.8, reviews: 125000 },
      { id: "tokyo-disneysea", name: "Tokyo DisneySea", category: "Theme Park", price: 8400, currency: "JPY", duration: "10-12 hours", image: "/cards/tokyo.webp", rating: 4.9, reviews: 98000 },
      { id: "robot-restaurant", name: "Robot Restaurant Show", category: "Entertainment", price: 8500, currency: "JPY", duration: "1.5 hours", image: "/cards/tokyo.webp", rating: 4.4, reviews: 32000 },
      { id: "sumo-tournament", name: "Sumo Tournament Tickets", category: "Entertainment", price: 15000, currency: "JPY", duration: "5-6 hours", image: "/cards/tokyo.webp", rating: 4.8, reviews: 12000 }
    ],
    "singapore": [
      { id: "gardens-bay", name: "Gardens by the Bay", category: "Garden", price: 53, currency: "SGD", duration: "3-4 hours", image: "/cards/singapore.webp", rating: 4.8, reviews: 78000 },
      { id: "marina-bay-sands", name: "Marina Bay Sands SkyPark", category: "Observation Deck", price: 32, currency: "SGD", duration: "1-2 hours", image: "/cards/singapore.webp", rating: 4.5, reviews: 42000 },
      { id: "universal-singapore", name: "Universal Studios Singapore", category: "Theme Park", price: 82, currency: "SGD", duration: "6-8 hours", image: "/cards/singapore.webp", rating: 4.7, reviews: 55000 },
      { id: "sea-aquarium", name: "S.E.A. Aquarium", category: "Aquarium", price: 43, currency: "SGD", duration: "2-3 hours", image: "/cards/singapore.webp", rating: 4.6, reviews: 38000 },
      { id: "singapore-flyer", name: "Singapore Flyer", category: "Observation Deck", price: 33, currency: "SGD", duration: "1 hour", image: "/cards/singapore.webp", rating: 4.5, reviews: 28000 },
      { id: "sentosa-cable-car", name: "Sentosa Cable Car", category: "Cable Car", price: 35, currency: "SGD", duration: "1-2 hours", image: "/cards/singapore.webp", rating: 4.5, reviews: 22000 },
      { id: "night-safari", name: "Night Safari", category: "Zoo", price: 55, currency: "SGD", duration: "4-5 hours", image: "/cards/singapore.webp", rating: 4.7, reviews: 48000 },
      { id: "singapore-zoo", name: "Singapore Zoo", category: "Zoo", price: 50, currency: "SGD", duration: "4-5 hours", image: "/cards/singapore.webp", rating: 4.7, reviews: 52000 },
      { id: "river-wonders", name: "River Wonders", category: "Zoo", price: 42, currency: "SGD", duration: "3-4 hours", image: "/cards/singapore.webp", rating: 4.5, reviews: 25000 },
      { id: "adventure-cove", name: "Adventure Cove Waterpark", category: "Theme Park", price: 38, currency: "SGD", duration: "4-6 hours", image: "/cards/singapore.webp", rating: 4.5, reviews: 18000 },
      { id: "artscience-museum", name: "ArtScience Museum", category: "Museum", price: 22, currency: "SGD", duration: "2-3 hours", image: "/cards/singapore.webp", rating: 4.6, reviews: 32000 },
      { id: "chinatown-food-tour", name: "Chinatown Food Tour", category: "Tour", price: 85, currency: "SGD", duration: "3 hours", image: "/cards/singapore.webp", rating: 4.6, reviews: 12000 },
      { id: "singapore-river-cruise", name: "Singapore River Cruise", category: "Tour", price: 28, currency: "SGD", duration: "40 min", image: "/cards/singapore.webp", rating: 4.4, reviews: 22000 }
    ],
    "bangkok": [
      { id: "grand-palace", name: "Grand Palace", category: "Historic Site", price: 500, currency: "THB", duration: "2-3 hours", image: "/cards/bangkok.webp", rating: 4.7, reviews: 67000 },
      { id: "wat-arun", name: "Wat Arun", category: "Religious Site", price: 100, currency: "THB", duration: "1-2 hours", image: "/destinations-hero/bangkok/bangkok-hero-wat-arun-temple-chao-phraya-river-dusk.webp.webp", rating: 4.6, reviews: 45000 },
      { id: "floating-market", name: "Damnoen Saduak Floating Market", category: "Tour", price: 1200, currency: "THB", duration: "4-5 hours", image: "/destinations-hero/bangkok/bangkok-hero-floating-market-boats-traditional-culture.webp.webp", rating: 4.4, reviews: 28000 },
      { id: "safari-world", name: "Safari World Bangkok", category: "Zoo", price: 1500, currency: "THB", duration: "5-6 hours", image: "/cards/bangkok.webp", rating: 4.5, reviews: 32000 },
      { id: "wat-pho", name: "Wat Pho (Reclining Buddha)", category: "Religious Site", price: 200, currency: "THB", duration: "1-2 hours", image: "/cards/bangkok.webp", rating: 4.7, reviews: 55000 },
      { id: "chatuchak-market", name: "Chatuchak Weekend Market Tour", category: "Tour", price: 800, currency: "THB", duration: "3-4 hours", image: "/cards/bangkok.webp", rating: 4.5, reviews: 28000 },
      { id: "ayutthaya-day-trip", name: "Ayutthaya Day Trip", category: "Tour", price: 1800, currency: "THB", duration: "10-12 hours", image: "/cards/bangkok.webp", rating: 4.7, reviews: 25000 },
      { id: "thai-cooking-class", name: "Thai Cooking Class", category: "Tour", price: 1500, currency: "THB", duration: "4 hours", image: "/cards/bangkok.webp", rating: 4.8, reviews: 18000 },
      { id: "chao-phraya-cruise", name: "Chao Phraya Dinner Cruise", category: "Tour", price: 2200, currency: "THB", duration: "2-3 hours", image: "/cards/bangkok.webp", rating: 4.5, reviews: 22000 },
      { id: "dream-world", name: "Dream World Bangkok", category: "Theme Park", price: 1400, currency: "THB", duration: "6-8 hours", image: "/cards/bangkok.webp", rating: 4.3, reviews: 15000 },
      { id: "siam-ocean-world", name: "SEA LIFE Bangkok Ocean World", category: "Aquarium", price: 1200, currency: "THB", duration: "2-3 hours", image: "/cards/bangkok.webp", rating: 4.5, reviews: 28000 },
      { id: "muay-thai-show", name: "Muay Thai Live Show", category: "Entertainment", price: 1800, currency: "THB", duration: "2 hours", image: "/cards/bangkok.webp", rating: 4.6, reviews: 12000 },
      { id: "rooftop-bar-tour", name: "Rooftop Bar Hopping Tour", category: "Tour", price: 2500, currency: "THB", duration: "4 hours", image: "/cards/bangkok.webp", rating: 4.5, reviews: 8000 }
    ],
    "barcelona": [
      { id: "sagrada-familia", name: "Sagrada Familia", category: "Landmark", price: 26, currency: "EUR", duration: "2-3 hours", image: "/destinations-hero/barcelona/barcelona/barcelona-hero-sagrada-familia-aerial.webp.webp", rating: 4.8, reviews: 145000 },
      { id: "park-guell", name: "Park Güell", category: "Park", price: 10, currency: "EUR", duration: "2-3 hours", image: "/destinations-hero/barcelona/barcelona/barcelona-hero-park-guell-gaudi-mosaic.webp.webp", rating: 4.6, reviews: 95000 },
      { id: "casa-batllo", name: "Casa Batlló", category: "Landmark", price: 35, currency: "EUR", duration: "1-2 hours", image: "/cards/barcelona.webp", rating: 4.7, reviews: 58000 },
      { id: "picasso-museum", name: "Picasso Museum", category: "Museum", price: 12, currency: "EUR", duration: "2-3 hours", image: "/cards/barcelona.webp", rating: 4.6, reviews: 42000 },
      { id: "la-pedrera", name: "La Pedrera (Casa Milà)", category: "Landmark", price: 28, currency: "EUR", duration: "1-2 hours", image: "/cards/barcelona.webp", rating: 4.6, reviews: 38000 },
      { id: "camp-nou", name: "Camp Nou Stadium Tour", category: "Tour", price: 28, currency: "EUR", duration: "2-3 hours", image: "/destinations-hero/barcelona/barcelona/barcelona-hero-camp-nou-stadium-match.webp.webp", rating: 4.7, reviews: 55000 },
      { id: "gothic-quarter-tour", name: "Gothic Quarter Walking Tour", category: "Tour", price: 20, currency: "EUR", duration: "2-3 hours", image: "/cards/barcelona.webp", rating: 4.5, reviews: 28000 },
      { id: "montserrat-day-trip", name: "Montserrat Day Trip", category: "Tour", price: 55, currency: "EUR", duration: "5-6 hours", image: "/cards/barcelona.webp", rating: 4.7, reviews: 35000 },
      { id: "flamenco-show", name: "Flamenco Show", category: "Entertainment", price: 45, currency: "EUR", duration: "1-2 hours", image: "/cards/barcelona.webp", rating: 4.6, reviews: 22000 },
      { id: "barcelona-aquarium", name: "Barcelona Aquarium", category: "Aquarium", price: 25, currency: "EUR", duration: "2-3 hours", image: "/cards/barcelona.webp", rating: 4.4, reviews: 32000 },
      { id: "portaventura", name: "PortAventura World", category: "Theme Park", price: 55, currency: "EUR", duration: "10-12 hours", image: "/cards/barcelona.webp", rating: 4.6, reviews: 48000 },
      { id: "barcelona-food-tour", name: "Barcelona Tapas & Wine Tour", category: "Tour", price: 85, currency: "EUR", duration: "3-4 hours", image: "/cards/barcelona.webp", rating: 4.7, reviews: 18000 },
      { id: "casa-vicens", name: "Casa Vicens (Gaudí)", category: "Landmark", price: 18, currency: "EUR", duration: "1-2 hours", image: "/cards/barcelona.webp", rating: 4.5, reviews: 15000 }
    ],
    "rome": [
      { id: "colosseum", name: "Colosseum", category: "Historic Site", price: 18, currency: "EUR", duration: "2-3 hours", image: "/cards/rome.webp", rating: 4.8, reviews: 180000 },
      { id: "vatican", name: "Vatican Museums & Sistine Chapel", category: "Museum", price: 28, currency: "EUR", duration: "3-4 hours", image: "/cards/rome.webp", rating: 4.7, reviews: 165000 },
      { id: "st-peters", name: "St. Peter's Basilica", category: "Religious Site", price: 0, currency: "EUR", duration: "2-3 hours", image: "/cards/rome.webp", rating: 4.9, reviews: 120000 },
      { id: "pantheon", name: "Pantheon Guided Tour", category: "Historic Site", price: 15, currency: "EUR", duration: "1-2 hours", image: "/cards/rome.webp", rating: 4.7, reviews: 75000 },
      { id: "roman-forum", name: "Roman Forum & Palatine Hill", category: "Historic Site", price: 18, currency: "EUR", duration: "2-3 hours", image: "/cards/rome.webp", rating: 4.6, reviews: 68000 },
      { id: "trevi-fountain-tour", name: "Trevi Fountain Walking Tour", category: "Tour", price: 25, currency: "EUR", duration: "2 hours", image: "/cards/rome.webp", rating: 4.5, reviews: 28000 },
      { id: "borghese-gallery", name: "Borghese Gallery", category: "Museum", price: 15, currency: "EUR", duration: "2-3 hours", image: "/cards/rome.webp", rating: 4.8, reviews: 42000 },
      { id: "castel-sant-angelo", name: "Castel Sant'Angelo", category: "Historic Site", price: 18, currency: "EUR", duration: "2-3 hours", image: "/cards/rome.webp", rating: 4.6, reviews: 35000 },
      { id: "trastevere-food-tour", name: "Trastevere Food Tour", category: "Tour", price: 79, currency: "EUR", duration: "3-4 hours", image: "/cards/rome.webp", rating: 4.7, reviews: 18000 },
      { id: "catacombs-rome", name: "Rome Catacombs Tour", category: "Historic Site", price: 32, currency: "EUR", duration: "2-3 hours", image: "/cards/rome.webp", rating: 4.6, reviews: 25000 },
      { id: "colosseum-underground", name: "Colosseum Underground Tour", category: "Tour", price: 55, currency: "EUR", duration: "3 hours", image: "/cards/rome.webp", rating: 4.8, reviews: 22000 },
      { id: "pompeii-day-trip", name: "Pompeii Day Trip from Rome", category: "Tour", price: 125, currency: "EUR", duration: "12-14 hours", image: "/cards/rome.webp", rating: 4.7, reviews: 32000 },
      { id: "vatican-night-tour", name: "Vatican Museums Night Tour", category: "Tour", price: 55, currency: "EUR", duration: "3 hours", image: "/cards/rome.webp", rating: 4.8, reviews: 15000 }
    ],
    "amsterdam": [
      { id: "rijksmuseum", name: "Rijksmuseum", category: "Museum", price: 22, currency: "EUR", duration: "3-4 hours", image: "/cards/amsterdam.webp", rating: 4.8, reviews: 95000 },
      { id: "anne-frank", name: "Anne Frank House", category: "Museum", price: 16, currency: "EUR", duration: "1-2 hours", image: "/cards/amsterdam.webp", rating: 4.7, reviews: 78000 },
      { id: "van-gogh", name: "Van Gogh Museum", category: "Museum", price: 22, currency: "EUR", duration: "2-3 hours", image: "/destinations-hero/Amsterdam/amsterdam/amsterdam-hero-van-gogh-museum-art.webp.webp", rating: 4.8, reviews: 88000 },
      { id: "canal-cruise", name: "Amsterdam Canal Cruise", category: "Tour", price: 18, currency: "EUR", duration: "1-1.5 hours", image: "/destinations-hero/Amsterdam/amsterdam/amsterdam-hero-canal-sunset-wine.webp.webp", rating: 4.5, reviews: 65000 },
      { id: "heineken-experience", name: "Heineken Experience", category: "Tour", price: 23, currency: "EUR", duration: "1.5-2 hours", image: "/cards/amsterdam.webp", rating: 4.5, reviews: 42000 },
      { id: "keukenhof", name: "Keukenhof Gardens", category: "Garden", price: 20, currency: "EUR", duration: "4-5 hours", image: "/cards/amsterdam.webp", rating: 4.7, reviews: 55000 },
      { id: "royal-palace", name: "Royal Palace Amsterdam", category: "Historic Site", price: 12, currency: "EUR", duration: "1-2 hours", image: "/cards/amsterdam.webp", rating: 4.4, reviews: 25000 },
      { id: "body-worlds", name: "Body Worlds Amsterdam", category: "Museum", price: 23, currency: "EUR", duration: "1.5-2 hours", image: "/cards/amsterdam.webp", rating: 4.5, reviews: 18000 },
      { id: "madame-tussauds-amsterdam", name: "Madame Tussauds Amsterdam", category: "Museum", price: 27, currency: "EUR", duration: "1.5-2 hours", image: "/cards/amsterdam.webp", rating: 4.3, reviews: 22000 },
      { id: "moco-museum", name: "Moco Museum", category: "Museum", price: 22, currency: "EUR", duration: "1.5-2 hours", image: "/cards/amsterdam.webp", rating: 4.6, reviews: 28000 },
      { id: "amsterdam-food-tour", name: "Amsterdam Food Walking Tour", category: "Tour", price: 79, currency: "EUR", duration: "3 hours", image: "/cards/amsterdam.webp", rating: 4.6, reviews: 12000 },
      { id: "windmill-tour", name: "Zaanse Schans Windmills Tour", category: "Tour", price: 45, currency: "EUR", duration: "4-5 hours", image: "/cards/amsterdam.webp", rating: 4.6, reviews: 35000 },
      { id: "artis-zoo", name: "ARTIS Amsterdam Royal Zoo", category: "Zoo", price: 27, currency: "EUR", duration: "3-4 hours", image: "/cards/amsterdam.webp", rating: 4.5, reviews: 22000 }
    ],
    "hong-kong": [
      { id: "victoria-peak", name: "Victoria Peak Tram", category: "Observation Deck", price: 88, currency: "HKD", duration: "2-3 hours", image: "/destinations-hero/hongkong/hong-kong-hero-victoria-harbour-skyline-sunset.webp.webp", rating: 4.6, reviews: 56000 },
      { id: "ocean-park", name: "Ocean Park Hong Kong", category: "Theme Park", price: 498, currency: "HKD", duration: "6-8 hours", image: "/cards/hong-kong.webp", rating: 4.5, reviews: 42000 },
      { id: "ngong-ping", name: "Ngong Ping 360", category: "Cable Car", price: 235, currency: "HKD", duration: "3-4 hours", image: "/cards/hong-kong.webp", rating: 4.7, reviews: 38000 },
      { id: "hong-kong-disneyland", name: "Hong Kong Disneyland", category: "Theme Park", price: 639, currency: "HKD", duration: "8-10 hours", image: "/cards/hong-kong.webp", rating: 4.6, reviews: 65000 },
      { id: "big-buddha", name: "Tian Tan Buddha (Big Buddha)", category: "Religious Site", price: 0, currency: "HKD", duration: "2-3 hours", image: "/destinations-hero/hongkong/hong-kong-hero-tian-tan-buddha-stairs.webp.webp", rating: 4.7, reviews: 48000 },
      { id: "symphony-lights", name: "Symphony of Lights Cruise", category: "Tour", price: 280, currency: "HKD", duration: "1-2 hours", image: "/cards/hong-kong.webp", rating: 4.5, reviews: 25000 },
      { id: "madame-tussauds-hk", name: "Madame Tussauds Hong Kong", category: "Museum", price: 290, currency: "HKD", duration: "1.5-2 hours", image: "/cards/hong-kong.webp", rating: 4.3, reviews: 18000 },
      { id: "lantau-day-trip", name: "Lantau Island Day Trip", category: "Tour", price: 550, currency: "HKD", duration: "6-8 hours", image: "/cards/hong-kong.webp", rating: 4.6, reviews: 22000 },
      { id: "sky100", name: "Sky100 Observation Deck", category: "Observation Deck", price: 198, currency: "HKD", duration: "1-2 hours", image: "/cards/hong-kong.webp", rating: 4.4, reviews: 28000 },
      { id: "chi-lin-nunnery", name: "Chi Lin Nunnery & Nan Lian Garden", category: "Religious Site", price: 0, currency: "HKD", duration: "2-3 hours", image: "/destinations-hero/hongkong/hong-kong-hero-nan-lian-garden-pavilion.webp.webp", rating: 4.6, reviews: 32000 },
      { id: "hk-food-tour", name: "Hong Kong Street Food Tour", category: "Tour", price: 650, currency: "HKD", duration: "3 hours", image: "/cards/hong-kong.webp", rating: 4.7, reviews: 15000 },
      { id: "tai-o-fishing-village", name: "Tai O Fishing Village Tour", category: "Tour", price: 450, currency: "HKD", duration: "4-5 hours", image: "/cards/hong-kong.webp", rating: 4.5, reviews: 12000 },
      { id: "macau-day-trip", name: "Macau Day Trip", category: "Tour", price: 890, currency: "HKD", duration: "10-12 hours", image: "/cards/hong-kong.webp", rating: 4.6, reviews: 25000 }
    ],
    "istanbul": [
      { id: "hagia-sophia", name: "Hagia Sophia", category: "Historic Site", price: 25, currency: "EUR", duration: "2-3 hours", image: "/cards/istanbul.webp", rating: 4.8, reviews: 95000 },
      { id: "topkapi", name: "Topkapi Palace", category: "Historic Site", price: 20, currency: "EUR", duration: "3-4 hours", image: "/cards/istanbul.webp", rating: 4.7, reviews: 78000 },
      { id: "blue-mosque", name: "Blue Mosque", category: "Religious Site", price: 0, currency: "EUR", duration: "1-2 hours", image: "/destinations-hero/istanbul/istanbul-hero-blue-mosque-aerial-sunset.webp.webp", rating: 4.6, reviews: 67000 },
      { id: "grand-bazaar-tour", name: "Grand Bazaar Guided Tour", category: "Tour", price: 35, currency: "EUR", duration: "2-3 hours", image: "/destinations-hero/istanbul/istanbul-hero-grand-bazaar-lanterns-market.webp.webp", rating: 4.5, reviews: 32000 },
      { id: "basilica-cistern", name: "Basilica Cistern", category: "Historic Site", price: 15, currency: "EUR", duration: "1 hour", image: "/cards/istanbul.webp", rating: 4.6, reviews: 45000 },
      { id: "bosphorus-cruise", name: "Bosphorus Cruise", category: "Tour", price: 25, currency: "EUR", duration: "2-3 hours", image: "/destinations-hero/istanbul/istanbul-hero-suleymaniye-mosque-bosphorus-ferry.webp.webp", rating: 4.6, reviews: 55000 },
      { id: "dolmabahce", name: "Dolmabahce Palace", category: "Historic Site", price: 20, currency: "EUR", duration: "2-3 hours", image: "/destinations-hero/istanbul/istanbul-hero-dolmabahce-palace-bosphorus.webp.webp", rating: 4.7, reviews: 38000 },
      { id: "turkish-bath", name: "Traditional Turkish Bath Experience", category: "Tour", price: 65, currency: "EUR", duration: "2 hours", image: "/cards/istanbul.webp", rating: 4.6, reviews: 22000 },
      { id: "galata-tower", name: "Galata Tower", category: "Observation Deck", price: 12, currency: "EUR", duration: "1 hour", image: "/destinations-hero/istanbul/istanbul-hero-galata-tower-seagull-aerial.webp.webp", rating: 4.5, reviews: 42000 },
      { id: "istanbul-food-tour", name: "Istanbul Street Food Tour", category: "Tour", price: 55, currency: "EUR", duration: "3-4 hours", image: "/cards/istanbul.webp", rating: 4.7, reviews: 18000 },
      { id: "whirling-dervishes", name: "Whirling Dervishes Ceremony", category: "Entertainment", price: 45, currency: "EUR", duration: "1.5 hours", image: "/cards/istanbul.webp", rating: 4.7, reviews: 25000 },
      { id: "princes-islands", name: "Princes Islands Day Trip", category: "Tour", price: 55, currency: "EUR", duration: "8-10 hours", image: "/cards/istanbul.webp", rating: 4.5, reviews: 28000 },
      { id: "cappadocia-day-trip", name: "Cappadocia Day Trip with Flight", category: "Tour", price: 350, currency: "EUR", duration: "14-16 hours", image: "/cards/istanbul.webp", rating: 4.8, reviews: 15000 }
    ],
    "las-vegas": [
      { id: "high-roller", name: "High Roller Observation Wheel", category: "Observation Deck", price: 37, currency: "USD", duration: "1 hour", image: "/cards/las-vegas.webp", rating: 4.5, reviews: 28000 },
      { id: "grand-canyon", name: "Grand Canyon Day Trip", category: "Tour", price: 129, currency: "USD", duration: "10-12 hours", image: "/cards/las-vegas.webp", rating: 4.8, reviews: 35000 },
      { id: "helicopter-tour", name: "Las Vegas Helicopter Tour", category: "Tour", price: 99, currency: "USD", duration: "15-30 min", image: "/cards/las-vegas.webp", rating: 4.7, reviews: 18000 },
      { id: "madame-tussauds-lv", name: "Madame Tussauds Las Vegas", category: "Museum", price: 35, currency: "USD", duration: "1.5-2 hours", image: "/cards/las-vegas.webp", rating: 4.4, reviews: 22000 },
      { id: "cirque-du-soleil", name: "Cirque du Soleil Show", category: "Entertainment", price: 125, currency: "USD", duration: "2 hours", image: "/cards/las-vegas.webp", rating: 4.8, reviews: 48000 },
      { id: "hoover-dam", name: "Hoover Dam Tour", category: "Tour", price: 55, currency: "USD", duration: "4-5 hours", image: "/cards/las-vegas.webp", rating: 4.6, reviews: 32000 },
      { id: "mob-museum", name: "The Mob Museum", category: "Museum", price: 35, currency: "USD", duration: "2-3 hours", image: "/cards/las-vegas.webp", rating: 4.6, reviews: 25000 },
      { id: "neon-museum", name: "Neon Museum", category: "Museum", price: 28, currency: "USD", duration: "1-2 hours", image: "/cards/las-vegas.webp", rating: 4.5, reviews: 18000 },
      { id: "stratosphere", name: "STRAT Tower Observation Deck", category: "Observation Deck", price: 25, currency: "USD", duration: "1-2 hours", image: "/cards/las-vegas.webp", rating: 4.4, reviews: 22000 },
      { id: "red-rock-canyon", name: "Red Rock Canyon Tour", category: "Tour", price: 65, currency: "USD", duration: "4-5 hours", image: "/cards/las-vegas.webp", rating: 4.7, reviews: 28000 },
      { id: "fremont-street", name: "Fremont Street Experience Tour", category: "Tour", price: 25, currency: "USD", duration: "2-3 hours", image: "/destinations-hero/lasvegas/las-vegas-hero-strip-welcome-sign-traffic.webp.webp", rating: 4.5, reviews: 35000 },
      { id: "vegas-food-tour", name: "Las Vegas Food Tour", category: "Tour", price: 95, currency: "USD", duration: "3-4 hours", image: "/cards/las-vegas.webp", rating: 4.6, reviews: 12000 },
      { id: "blue-man-group", name: "Blue Man Group", category: "Entertainment", price: 85, currency: "USD", duration: "1.5 hours", image: "/cards/las-vegas.webp", rating: 4.6, reviews: 32000 }
    ],
    "los-angeles": [
      { id: "universal-hollywood", name: "Universal Studios Hollywood", category: "Theme Park", price: 109, currency: "USD", duration: "8-10 hours", image: "/cards/los-angeles.webp", rating: 4.6, reviews: 85000 },
      { id: "hollywood-sign", name: "Hollywood Sign Hike", category: "Tour", price: 45, currency: "USD", duration: "3-4 hours", image: "/destinations-hero/losangeles/los-angeles-hero-hollywood-sign-hills.webp.webp", rating: 4.4, reviews: 15000 },
      { id: "griffith-observatory", name: "Griffith Observatory", category: "Observatory", price: 0, currency: "USD", duration: "2-3 hours", image: "/destinations-hero/losangeles/los-angeles-hero-griffith-observatory-night.webp.webp", rating: 4.7, reviews: 45000 },
      { id: "warner-bros-studio", name: "Warner Bros. Studio Tour", category: "Tour", price: 72, currency: "USD", duration: "3-4 hours", image: "/cards/los-angeles.webp", rating: 4.7, reviews: 55000 },
      { id: "hollywood-tour", name: "Hollywood Celebrity Homes Tour", category: "Tour", price: 55, currency: "USD", duration: "2-3 hours", image: "/cards/los-angeles.webp", rating: 4.5, reviews: 32000 },
      { id: "disney-california", name: "Disneyland California", category: "Theme Park", price: 164, currency: "USD", duration: "10-12 hours", image: "/cards/los-angeles.webp", rating: 4.8, reviews: 125000 },
      { id: "santa-monica-pier", name: "Santa Monica Pier & Beach", category: "Landmark", price: 0, currency: "USD", duration: "2-3 hours", image: "/destinations-hero/losangeles/los-angeles-hero-santa-monica-pier-sunset.webp.webp", rating: 4.5, reviews: 48000 },
      { id: "getty-center", name: "Getty Center", category: "Museum", price: 0, currency: "USD", duration: "3-4 hours", image: "/cards/los-angeles.webp", rating: 4.7, reviews: 42000 },
      { id: "lacma", name: "Los Angeles County Museum of Art", category: "Museum", price: 25, currency: "USD", duration: "2-3 hours", image: "/cards/los-angeles.webp", rating: 4.6, reviews: 28000 },
      { id: "la-brea-tar-pits", name: "La Brea Tar Pits & Museum", category: "Museum", price: 15, currency: "USD", duration: "2 hours", image: "/cards/los-angeles.webp", rating: 4.5, reviews: 22000 },
      { id: "walk-of-fame", name: "Hollywood Walk of Fame Tour", category: "Tour", price: 35, currency: "USD", duration: "2 hours", image: "/cards/los-angeles.webp", rating: 4.4, reviews: 38000 },
      { id: "paramount-studio", name: "Paramount Pictures Studio Tour", category: "Tour", price: 65, currency: "USD", duration: "2 hours", image: "/cards/los-angeles.webp", rating: 4.6, reviews: 18000 },
      { id: "la-food-tour", name: "LA Food & Culture Tour", category: "Tour", price: 79, currency: "USD", duration: "3-4 hours", image: "/cards/los-angeles.webp", rating: 4.6, reviews: 12000 }
    ],
    "miami": [
      { id: "everglades", name: "Everglades Airboat Tour", category: "Tour", price: 40, currency: "USD", duration: "3-4 hours", image: "/cards/miami.webp", rating: 4.6, reviews: 28000 },
      { id: "art-deco", name: "Art Deco Walking Tour", category: "Tour", price: 30, currency: "USD", duration: "2 hours", image: "/cards/miami.webp", rating: 4.5, reviews: 12000 },
      { id: "vizcaya", name: "Vizcaya Museum & Gardens", category: "Museum", price: 25, currency: "USD", duration: "2-3 hours", image: "/cards/miami.webp", rating: 4.7, reviews: 18000 },
      { id: "south-beach-tour", name: "South Beach Walking Tour", category: "Tour", price: 35, currency: "USD", duration: "2-3 hours", image: "/cards/miami.webp", rating: 4.5, reviews: 22000 },
      { id: "key-west-day-trip", name: "Key West Day Trip", category: "Tour", price: 65, currency: "USD", duration: "12-14 hours", image: "/cards/miami.webp", rating: 4.7, reviews: 35000 },
      { id: "miami-seaquarium", name: "Miami Seaquarium", category: "Aquarium", price: 50, currency: "USD", duration: "3-4 hours", image: "/cards/miami.webp", rating: 4.4, reviews: 25000 },
      { id: "wynwood-walls", name: "Wynwood Walls Art Tour", category: "Tour", price: 25, currency: "USD", duration: "1.5-2 hours", image: "/cards/miami.webp", rating: 4.6, reviews: 28000 },
      { id: "little-havana-tour", name: "Little Havana Food Tour", category: "Tour", price: 69, currency: "USD", duration: "2.5-3 hours", image: "/cards/miami.webp", rating: 4.7, reviews: 15000 },
      { id: "miami-boat-tour", name: "Miami Skyline Boat Tour", category: "Tour", price: 35, currency: "USD", duration: "1.5 hours", image: "/destinations-hero/miami/miami-hero-biscayne-bay-skyline-boat.webp.webp", rating: 4.5, reviews: 18000 },
      { id: "zoo-miami", name: "Zoo Miami", category: "Zoo", price: 25, currency: "USD", duration: "4-5 hours", image: "/cards/miami.webp", rating: 4.5, reviews: 22000 },
      { id: "frost-science", name: "Frost Science Museum", category: "Museum", price: 30, currency: "USD", duration: "2-3 hours", image: "/cards/miami.webp", rating: 4.6, reviews: 18000 },
      { id: "biscayne-snorkeling", name: "Biscayne National Park Snorkeling", category: "Tour", price: 75, currency: "USD", duration: "4-5 hours", image: "/cards/miami.webp", rating: 4.7, reviews: 12000 },
      { id: "miami-food-tour", name: "Miami Beach Food Tour", category: "Tour", price: 85, currency: "USD", duration: "3 hours", image: "/cards/miami.webp", rating: 4.6, reviews: 10000 }
    ]
  };
    ...
    try {
      ...
      if (!destInfo) {
        return res.status(400).json({ 
          error: "Invalid destination. Must be one of: " + Object.keys(tiqetsDestinations).join(", ") 
        });
      }
      ...
      if (enableAI) {
        try {
          ...
          fullAttraction.faqItems = [
            { question: `How much are tickets for ${attraction.name}?`, answer: `Tickets start from ${attraction.price} ${attraction.currency}. Check the booking page for the latest prices.` },
            { question: "What are the opening hours?", answer: "Please check the ticket options page for current opening hours." },
            { question: `How long should I spend at ${attraction.name}?`, answer: `Most visitors spend ${attraction.duration || "2-3 hours"} at this attraction.` }
          ];
  ...
  app.get("/api/intelligence/gaps", requireAuth, async (req: AuthRequest, res: Response) => {
    try {
      ...
      for (const content of publishedContent) {
        const gaps: Array<{ type: string; priority: "high" | "medium" | "low"; suggestion: string }> = [];
        ...
        if (totalText.length < 500) {
          gaps.push({
            type: "Thin Content",
            priority: "medium",
            suggestion: "Article has less than 500 characters. Consider expanding with more details"
          });
        }
      ...
      const highPriorityGaps = contentGaps.reduce((sum, c) => sum + c.gaps.filter(g => g.priority === "high").length, 0);
      ...
      for (const content of allContent) {
        if (content.type === "article" || content.type === "event") {
          ...
          if (title.includes("2024") || title.includes("2025") || 
              title.includes("festival") || title.includes("event") ||
              title.includes("exhibition") || title.includes("ramadan") ||
              title.includes("eid") || title.includes("new year")) {
  ...
  app.get("/api/intelligence/clusters/areas", requireAuth, async (req: AuthRequest, res: Response) => {
    try {
      const allContent = await storage.getContentsWithRelations({ status: "published" });
  ...
  app.get("/api/intelligence/internal-links", requireAuth, async (req: AuthRequest, res: Response) => {
    try {
      const links = await storage.getInternalLinks();
      const allContent = await storage.getContentsWithRelations({ status: "published" });
...
IMPORTANT: Include 5-8 internal links and 2-3 external links in your text sections.`,
  ...
  app.post("/api/users", requirePermission("canManageUsers"), checkReadOnlyMode, async (req, res) => {
    try {
      ...
      const passwordValidation = validatePasswordStrength(password, [username, firstName, lastName, email].filter(Boolean));
    ...
    try {
      ...
      if (password) {
        ...
        const passwordValidation = await validatePasswordChange(
          req.params.id, 
          password, 
          [existingUser?.username, existingUser?.email, existingUser?.firstName, existingUser?.lastName].filter(Boolean) as string[]
        );
        if (!passwordValidation.valid) {
          // Log failed password policy attempt
          logSecurityEventFromRequest(req, SecurityEventType.PASSWORD_CHANGE, {
            ...
            errorMessage: 'Password does not meet security requirements',
            details: { targetUserId: req.params.id, errorCount: passwordValidation.errors.length }
          });
          return res.status(400).json({ 
            error: "Password does not meet security requirements",
            code: 'WEAK_PASSWORD',
  ...
  app.patch("/api/admin/homepage/sections/:id", requirePermission("canEdit"), checkReadOnlyMode, async (req, res) => {
    try {
      const { id } = req.params;
      const locale = (req.query.locale as string) || "en";
  ...
  app.get("/api/admin/homepage/available-images", requirePermission("canEdit"), async (req, res) => {
    try {
  ...
  app.get("/api/admin/homepage/available-hero-images", requirePermission("canEdit"), async (req, res) => {
    try {
      const fs = await import("fs").then(m => m.promises);
  ...
  app.post("/api/admin/homepage/upload-image", requirePermission("canEdit"), checkReadOnlyMode, upload.single("file"), async (req, res) => {
    try {
      if (!req.file) {
        return res.status(400).json({ error: "No file uploaded" });
      }
  ...
  app.post("/api/admin/homepage/hero-slides", requirePermission("canEdit"), checkReadOnlyMode, async (req, res) => {
    try {
      const locale = (req.query.locale as string) || "en";
  ...
  app.patch("/api/admin/homepage/hero-slides/:id", requirePermission("canEdit"), checkReadOnlyMode, async (req, res) => {
    try {
      const { id } = req.params;
      const locale = (req.query.locale as string) || "en";
  ...
  app.delete("/api/admin/homepage/hero-slides/:id", requirePermission("canEdit"), checkReadOnlyMode, async (req, res) => {
    try {
      const { id } = req.params;
  ...
  app.post("/api/admin/homepage/cards", requirePermission("canEdit"), checkReadOnlyMode, async (req, res) => {
    try {
      const locale = (req.query.locale as string) || "en";
  ...
  app.patch("/api/admin/homepage/cards/:id", requirePermission("canEdit"), checkReadOnlyMode, async (req, res) => {
    try {
  ...
  app.delete("/api/admin/homepage/cards/:id", requirePermission("canEdit"), checkReadOnlyMode, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
  ...
  app.post("/api/admin/homepage/experience-categories", requirePermission("canEdit"), checkReadOnlyMode, async (req, res) => {
    try {
      const locale = (req.query.locale as string) || "en";
  ...
  app.patch("/api/admin/homepage/experience-categories/:id", requirePermission("canEdit"), checkReadOnlyMode, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const locale = (req.query.locale as string) || "en";
  ...
  app.delete("/api/admin/homepage/experience-categories/:id", requirePermission("canEdit"), checkReadOnlyMode, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
  ...
  app.post("/api/admin/homepage/destinations", requirePermission("canEdit"), checkReadOnlyMode, async (req, res) => {
    try {
      const locale = (req.query.locale as string) || "en";
  ...
  app.patch("/api/admin/homepage/destinations/:id", requirePermission("canEdit"), checkReadOnlyMode, async (req, res) => {
    try {
  ...
  app.post("/api/admin/homepage/destinations/analyze-image", requirePermission("canEdit"), checkReadOnlyMode, async (req, res) => {
    try {
  ...
  app.post("/api/admin/homepage/region-links", requirePermission("canEdit"), checkReadOnlyMode, async (req, res) => {
    try {
      const locale = (req.query.locale as string) || "en";
  ...
  app.patch("/api/admin/homepage/region-links/:id", requirePermission("canEdit"), checkReadOnlyMode, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const locale = (req.query.locale as string) || "en";
  ...
  app.delete("/api/admin/homepage/region-links/:id", requirePermission("canEdit"), checkReadOnlyMode, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
  ...
  app.patch("/api/admin/homepage/cta/:id", requirePermission("canEdit"), checkReadOnlyMode, async (req, res) => {
    try {
      const { id } = req.params;
      const locale = (req.query.locale as string) || "en";
  ...
  app.patch("/api/admin/homepage/seo-meta/:id", requirePermission("canEdit"), checkReadOnlyMode, async (req, res) => {
    try {
      const { id } = req.params;
      const locale = (req.query.locale as string) || "en";
  ...
  app.post("/api/admin/homepage/generate-translations", requirePermission("canEdit"), checkReadOnlyMode, async (req, res) => {
    try {
...
<body style="margin: 0; padding: 0; background-color: #f8f5fc; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;">
  ...
  app.post("/api/email-templates", requirePermission("canCreate"), checkReadOnlyMode, async (req, res) => {
    try {
      const user = req.user as any;
      const templateData = {
        ...req.body,
  ...
  app.patch("/api/email-templates/:id", requirePermission("canEdit"), checkReadOnlyMode, async (req, res) => {
    try {
      const { id } = req.params;
  ...
  app.delete("/api/email-templates/:id", requirePermission("canDelete"), checkReadOnlyMode, async (req, res) => {
    try {
      const { id } = req.params;
      const existing = await storage.getEmailTemplate(id);
      if (!existing) {
  ...
  app.get("/api/subscriber-segments/:id", requirePermission("canViewAnalytics"), async (req, res) => {
    try {
  ...
  app.post("/api/subscriber-segments", requirePermission("canCreate"), checkReadOnlyMode, async (req, res) => {
    try {
      const user = req.user as any;
  ...
  app.patch("/api/subscriber-segments/:id", requirePermission("canEdit"), checkReadOnlyMode, async (req, res) => {
    try {
      const { id } = req.params;
  ...
  app.delete("/api/subscriber-segments/:id", requirePermission("canDelete"), checkReadOnlyMode, async (req, res) => {
    try {
      const { id } = req.params;
      const existing = await storage.getSubscriberSegment(id);
      if (!existing) {
  ...
  app.get("/api/newsletter/bounce-stats", requirePermission("canViewAnalytics"), async (req, res) => {
    try {
      ...
      const bounceRate = totalSubscribers > 0 ? (totalBounced / totalSubscribers * 100).toFixed(2) : "0";
  ...
  app.post("/api/newsletter/send", requirePermission("canPublish"), checkReadOnlyMode, async (req, res) => {
    try {
      ...
      if (!campaignName || !subject || !htmlContent) {
        return res.status(400).json({ error: "Missing required fields: campaignName, subject, htmlContent" });
      }
      ...
      // Create campaign
      const campaign = await db.insert(newsletterCampaigns)
        .values({
          name: campaignName,
          subject,
  ...
  app.get("/api/translations/usage", requirePermission("canViewAnalytics"), async (req, res) => {
    try {
      const { getDeepLUsage, getDeepLSupportedLocales, getUnsupportedLocales } = await import("./services/translation-service");
  ...
  const { getBudgetStatus, getTodayUsageStats, isProcessingPaused, pauseProcessing, resumeProcessing } = await import("./travi/budget-manager");
  const { getRecentJobs, getRunningJobs } = await import("./travi/checkpoint-manager");
  const { processDestinationCategory, getRecentLogs } = await import("./travi/processing-orchestrator");
  ...
  app.get("/api/admin/travi/stats", requireAuth, requirePermission("canViewAnalytics"), async (req, res) => {
    try {
      const budgetStatus = await getBudgetStatus();
      const todayUsage = await getTodayUsageStats();
      const runningJobs = await getRunningJobs();
      
      const todayCost = todayUsage.reduce((sum, u) => sum + parseFloat(u.estimatedCost || '0'), 0);
      ...
      res.json({
        ...
        processingStatus: isProcessingPaused() ? 'paused' : (runningJobs.length > 0 ? 'running' : 'idle'),
  ...
  app.get("/api/admin/travi/jobs", requireAuth, requirePermission("canViewAnalytics"), async (req, res) => {
    try {
  ...
  app.post("/api/admin/travi/process", requireAuth, requirePermission("canEdit"), async (req, res) => {
    try {
      ...
      if (!validCategories.includes(category)) {
        return res.status(400).json({ error: "Invalid category. Must be: attraction, restaurant, or hotel" });
      }
  ...
  app.get("/api/admin/travi/logs", requireAuth, requirePermission("canViewAnalytics"), async (req, res) => {
    try {
      const limit = Math.min(parseInt(req.query.limit as string) || 20, 100);
  ...
  app.get("/api/admin/travi/export", requireAuth, requirePermission("canEdit"), async (req, res) => {
    try {
      ...
      if (format === "csv") {
        res.setHeader("Content-Type", "text/csv");
        res.setHeader("Content-Disposition", `attachment; filename="travi-export-${Date.now()}.csv"`);
        res.send(result.csv);
      } else {
        res.json(result);
      }
  ...
  app.get("/api/admin/travi/export/summary", requireAuth, requirePermission("canViewAnalytics"), async (req, res) => {
    try {
  ...
  app.get("/api/admin/travi/locations", requireAuth, requirePermission("canViewAll"), async (req, res) => {
    res.json({
      message: "Deprecated - use Tiqets API",
      locations: [],
      total: 0,
      ...
      migration: "This endpoint has been migrated to the Tiqets integration system. Use /api/admin/tiqets/* endpoints instead."
    });
  });

  // DEPRECATED: Migrated to Tiqets system
  // GET /api/admin/travi/locations/:id - Get single location with all related data
  app.get("/api/admin/travi/locations/:id", requireAuth, requirePermission("canViewAll"), async (req, res) => {
    res.json({
      message: "Deprecated - use Tiqets API",
      migration: "This endpoint has been migrated to the Tiqets integration system. Use /api/admin/tiqets/* endpoints instead."
    });
  ...
  app.patch("/api/admin/travi/locations/:id", requireAuth, requirePermission("canEdit"), async (req, res) => {
    res.json({
      message: "Deprecated - use Tiqets API",
      migration: "This endpoint has been migrated to the Tiqets integration system. Use /api/admin/tiqets/* endpoints instead."
    });
  ...
  app.get("/api/admin/travi/locations/:id/history", requireAuth, requirePermission("canViewAll"), async (req, res) => {
    res.json([]);
  });

  // DEPRECATED: Migrated to Tiqets system
  // DELETE /api/admin/travi/locations/:id - Delete location
  app.delete("/api/admin/travi/locations/:id", requireAuth, requirePermission("canDelete"), async (req, res) => {
    res.json({
      message: "Deprecated - use Tiqets API",
      migration: "This endpoint has been migrated to the Tiqets integration system."
    });
  ...
  app.post("/api/admin/travi/locations/bulk", requireAuth, requirePermission("canEdit"), async (req, res) => {
    res.json({
      message: "Deprecated - use Tiqets API",
      migration: "This endpoint has been migrated to the Tiqets integration system.",
      affected: 0
  ...
  app.get("/api/admin/travi/config", requireAuth, requirePermission("canViewAll"), async (req, res) => {
    try {
      const { DESTINATION_METADATA } = await import("./travi/destination-seeder");
      ...
      const apiKeys = {
        ...
        gemini: { configured: !!(process.env.GEMINI_API_KEY || process.env.AI_INTEGRATIONS_GOOGLE_AI_API_KEY) },
      ...
      const budgetConfig = await db.select()
        .from(traviConfig)
        .where(eq(traviConfig.configKey, 'budget_settings'))
        .limit(1);
      
      const persistedBudget = budgetConfig.length > 0 
        ? (budgetConfig[0].configValue as any)?.value 
        : null;

      const destinationConfigs = await db.select()
        .from(traviConfig)
  ...
  app.post("/api/admin/travi/config/test-api", requireAuth, requirePermission("canEdit"), async (req, res) => {
    try {
  ...
  app.patch("/api/admin/travi/config/budget", requireAuth, requirePermission("canEdit"), async (req, res) => {
    try {
  ...
  app.patch("/api/admin/travi/config/destinations/:id", requireAuth, requirePermission("canEdit"), async (req, res) => {
    try {
      const { id } = req.params;
  ...
  app.get("/api/admin/travi/districts", requireAuth, requirePermission("canViewAll"), async (req, res) => {
    try {
      const { city } = req.query;
      
      const query = city 
        ? db.select().from(traviDistricts).where(eq(traviDistricts.city, city as string)).orderBy(traviDistricts.displayOrder)
        : db.select().from(traviDistricts).orderBy(traviDistricts.city, traviDistricts.displayOrder);
  ...
  app.post("/api/admin/travi/districts/auto-assign", requireAuth, requirePermission("canEdit"), async (req, res) => {
    res.json({
      message: "Deprecated - use Tiqets API",
      migration: "This endpoint has been migrated to the Tiqets integration system.",
      success: true,
  ...
  app.post("/api/admin/travi/locations/bulk-process", requireAuth, requirePermission("canEdit"), async (req, res) => {
    res.json({
      message: "Deprecated - use Tiqets API",
      migration: "This endpoint has been migrated to the Tiqets integration system.",
      success: true,
  ...
  app.get("/api/admin/tiqets/search-city", requireAuth, async (req, res) => {
    try {
      ...
      const response = await fetch(`https://api.tiqets.com/v2/cities?search=${encodeURIComponent(name as string)}`, {
        headers: {
  ...
  app.get("/api/admin/tiqets/attractions", requireAuth, async (req, res) => {
    try {
      ...
      const [countResult] = await db.select({ count: sql<number>`count(*)` }).from(tiqetsAttractions);
  ...
  app.get("/api/admin/tiqets/dashboard-stats", requireAuth, async (req, res) => {
    try {
      ...
      const [attractionsResult] = await db.select({
        total: sql<number>`count(*)`,
  ...
  app.post("/api/admin/tiqets/generate-parallel", requireAuth, async (req, res) => {
    ...
    try {
      ...
      const potentialProviders = providerStatus
        ...
        .map(p => p.name) as Array<"anthropic" | "openai" | "gemini" | "openrouter" | "deepseek" | "perplexity">;
      ...
      const availableProviders: Array<"anthropic" | "openai" | "gemini" | "openrouter" | "deepseek" | "perplexity"> = [];
      ...
      for (const providerName of potentialProviders) {
        try {
          ...
          sendEvent({ type: "provider_failed", provider: providerName, error: err.message?.slice(0, 100) });
        }
      }
      ...
      const allAttractions = await db.select()
        .from(tiqetsAttractions)
        .where(eq(tiqetsAttractions.contentGenerationStatus, "pending"));
  ...
  app.post("/api/admin/tiqets/generate-octypo", requireAuth, async (req, res) => {
    ...
    res.json({ 
      ...
      concurrency, 
      batchSize,
      message: 'Generation running in background. Check /api/admin/tiqets/octypo-status for progress.'
    });
    ...
    setImmediate(async () => {
      ...
      try {
        ...
        while (remainingAttractions.length > 0) {
          ...
          const articlesPerMin = processed > 0 
            ? ((processed / ((Date.now() - batchStartTime) / 60000))).toFixed(1) 
            : '0';
          ...
          const etaHours = parseFloat(articlesPerMin) > 0 
  ...
  app.get("/api/admin/tiqets/generation-progress", async (req, res) => {
    try {
      ...
      const etaText = hoursRemaining !== null 
        ? hoursRemaining < 1 
  ...
  app.post("/api/admin/tiqets/generate-smart-queue", requireAuth, async (req, res) => {
    ...
    try {
      ...
      if (isQueueRunning()) {
        res.write(`data: ${JSON.stringify({ type: "error", message: "Queue is already running" })}\n\n`);
        res.end();
        return;
      }
...
Return as valid JSON.`,
          {
            maxTokens: 4000,
            temperature: 0.7,
            systemPrompt: "You are a professional travel content writer creating SEO-optimized attraction content."
          }
        );
  ...
  app.get("/api/admin/tiqets/health", requireAuth, async (req, res) => {
    try {
      const { getHealthMetrics, forceReleaseAllLocks } = await import('./services/tiqets-background-generator');
      ...
      res.json({
        ...
        recommendations: [
          ...(staleInProgress > 5 ? ["High number of stale in-progress items - watchdog should clean these up"] : []),
          ...(health.consecutiveErrors > 3 ? ["Multiple consecutive errors detected - check AI provider status"] : []),
          ...(failureStats.total > 10 ? ["Consider using /api/admin/tiqets/reset-failed to retry failed items"] : []),
  ...
  app.post("/api/admin/tiqets/retry-all-failed", requireAuth, async (req, res) => {
    try {
      const { resetFailedAttractions, isQueueRunning } = await import('./services/content-job-queue');
  ...
  app.post("/api/admin/content-quality/regenerate-all", requireAuth, async (req, res) => {
    try {
      ...
      const { regenerateAllAttractions, getRegenerationStats } = await import('./services/content-regeneration-job');
  ...
  app.get("/api/admin/api-keys/detected", requireAuth, async (req, res) => {
    try {
      ...
      if (process.env.AI_INTEGRATIONS_ANTHROPIC_API_KEY) detected.anthropic.push("AI_INTEGRATIONS_ANTHROPIC_API_KEY");
      if (process.env.ANTHROPIC_API_KEY) detected.anthropic.push("ANTHROPIC_API_KEY");
      for (let i = 1; i <= 20; i++) {
        if (process.env[`ANTHROPIC_API_KEY_${i}`]) detected.anthropic.push(`ANTHROPIC_API_KEY_${i}`);
      }
      ...
      if (process.env.AI_INTEGRATIONS_OPENAI_API_KEY) detected.openai.push("AI_INTEGRATIONS_OPENAI_API_KEY");
      ...
      for (let i = 1; i <= 20; i++) {
        if (process.env[`OPENROUTER_API_KEY_${i}`]) detected.openrouter.push(`OPENROUTER_API_KEY_${i}`);
      }

      if (process.env.AI_INTEGRATIONS_GEMINI_API_KEY) detected.gemini.push("AI_INTEGRATIONS_GEMINI_API_KEY");
  ...
  app.use("/api/admin/entity-quality", requireAuth, requirePermission("canEdit"), entityQualityRoutes);
  ...
  // PLATFORM SYSTEMS (Feature-flagged, default OFF)
  // ============================================================================
  app.use("/api/admin/readiness", requireAuth, requirePermission("canEdit"), platformReadinessRoutes);
  ...
  app.use("/api/ops/executive/report", requireAuth, requirePermission("canEdit"), executiveReportRoutes);
  ...
  app.get("/api/admin/live-chat/conversations", isAuthenticated, async (req: AuthRequest, res: Response) => {
    try {
  ...
  app.get("/api/admin/live-chat/conversations/:id", isAuthenticated, async (req: AuthRequest, res: Response) => {
    try {
      const { id } = req.params;
  ...
  app.post("/api/admin/live-chat/conversations/:id/messages", isAuthenticated, async (req: AuthRequest, res: Response) => {
    try {
      const { id } = req.params;
  ...
  app.patch("/api/admin/live-chat/conversations/:id", isAuthenticated, async (req: AuthRequest, res: Response) => {
    try {
      const { id } = req.params;
  ...
  app.post("/api/admin/live-chat/conversations/:id/read", isAuthenticated, async (req: AuthRequest, res: Response) => {
    try {
      const { id } = req.params;
  ...
  app.get("/api/admin/affiliate/status", requirePermission("canEdit"), async (_req, res) => {
    try {
      const { getAffiliateHookStatus, isAffiliateHooksEnabled } = await import("./monetization/affiliate-hooks");
  ...
  app.post("/api/admin/affiliate/validate", requirePermission("canEdit"), async (_req, res) => {
    try {
      const { getAffiliateHookStatus, getAffiliateMetrics } = await import("./monetization/affiliate-hooks");
      ...
      if (!noViolations) {
        warnings.push(`${metrics.forbiddenZoneViolationsBlocked} forbidden zone violation attempts have been blocked`);
      }
  ...
  app.get("/api/admin/affiliate/metrics", requirePermission("canEdit"), async (_req, res) => {
    try {
      const { getAffiliateMetrics, isAffiliateHooksEnabled } = await import("./monetization/affiliate-hooks");
      const metrics = getAffiliateMetrics();
  ...
  app.delete("/api/real-estate-pages/:pageKey", requirePermission("canDelete"), async (req, res) => {
    try {
      const { pageKey } = req.params;
      
      const [deleted] = await db.delete(realEstatePages)
        .where(eq(realEstatePages.pageKey, pageKey))
        .returning();
  ...
  app.post("/api/real-estate-pages/seed", requirePermission("canManageSettings"), async (req, res) => {
    try {
      const STATIC_PAGES = [
        // Guides
        ...
        { pageKey: "payment-calculator", category: "calculator", title: "Payment Plan Calculator" },
        { pageKey: "affordability-calculator", category: "calculator", title: "Affordability Calculator" },
        ...
        { pageKey: "rental-yield-calculator", category: "calculator", title: "Rental Yield Calculator" },
        ...
        { pageKey: "case-investor-flip", category: "case_study", title: "Investor Flip Strategy" },
        ...
        { pageKey: "off-plan-business-bay", category: "location", title: "Business Bay Off-Plan" },
        { pageKey: "off-plan-dubai-marina", category: "location", title: "Dubai Marina Off-Plan" },
        { pageKey: "off-plan-creek-harbour", category: "location", title: "Creek Harbour Off-Plan" },
        { pageKey: "off-plan-palm-jumeirah", category: "location", title: "Palm Jumeirah Off-Plan" },
  ...
  app.delete("/api/surveys/:surveyId/responses/:id", requirePermission("canDelete"), async (req, res) => {
    try {
  ...
  app.get("/api/gdpr/export/:userId", requireAuth, async (req, res) => {
    ...
    try {
      ...
      const userContent = await db
        .select({ id: contents.id, type: contents.type, title: contents.title, status: contents.status, createdAt: contents.createdAt })
        .from(contents)
  ...
  app.delete("/api/gdpr/delete/:userId", requireAuth, requirePermission("canManageUsers"), async (req, res) => {
    const { userId } = req.params;
  ...
  app.get("/api/admin/digest/status", requireAuth, requirePermission("canEdit"), async (req, res) => {
    try {
  ...
  app.post("/api/admin/digest/send-now", requireAuth, requirePermission("canManageUsers"), async (req, res) => {
    try {
      const { sendWeeklyDigest, isWeeklyDigestEnabled } = await import("./newsletter/weekly-digest");
      
      if (!isWeeklyDigestEnabled()) {
        return res.status(400).json({ error: "Weekly digest feature is disabled. Set ENABLE_WEEKLY_DIGEST=true to enable." });
      }
  ...
  app.post("/api/admin/digest/test", requireAuth, requirePermission("canManageSettings"), async (req, res) => {
    try {
  ...
  app.post("/api/admin/digest/dry-run", requireAuth, requirePermission("canEdit"), async (req, res) => {
    try {
  ...
  app.get("/api/admin/digest/stats", requireAuth, requirePermission("canEdit"), async (req, res) => {
    try {
  ...
  // אפס הפתעות. הכל נשלט. (Zero surprises. Everything controlled.)
  // ============================================================================
  ...
  // ============================================================================
  app.post("/api/admin/assets/sync-google-drive", requirePermission("canManageSettings"), async (req: Request, res: Response) => {
    const report: {
      success: boolean;
    ...
    try {
      ...
      if (!categoriesFolder || !categoriesFolder.id) {
        report.errors.push({ city: "", error: "Categories subfolder not found in Website folder" });
        report.summary.totalErrors = 1;
        return res.status(404).json(report);
      }
        ...
        try {
          ...
          for (const file of cityFiles) {
            ...
            if (!imageMimeTypes.includes(file.mimeType || "")) {
              report.skipped.push({ city: cityName, file: file.name, reason: `Unsupported mime type: ${file.mimeType}` });
              continue;
            }
          ...
          report.errors.push({ city: cityName, error: cityErr.message || "Failed to list city folder contents" });
        }
      }
