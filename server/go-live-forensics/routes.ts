...
router.post('/events', requireEnabled, (req: Request, res: Response) => {
  ...
  const validTypes: ForensicsEventType[] = [
    'decision', 'approval', 'override', 'state_change',
    'degradation', 'recovery', 'deployment', 'rollback', 'incident', 'manual',
  ];
  ...
  try {
    const event = recordEvent(type, severity, source, title, description, data || {}, { actor, correlationId });
