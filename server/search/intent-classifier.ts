...
async function getDynamicLocations(): Promise<string[]> {
  ...
  try {
    ...
    const districtContentsRows = await db
...
const OCCASION_PATTERNS: Record<string, RegExp> = {
  romantic: /romantic|date|couple|anniversary|honeymoon/i,
  family: /family|kids|children|child-friendly/i,
  business: /business|corporate|meeting|conference/i,
  luxury: /luxury|premium|vip|exclusive|high-end/i,
...
export const intentClassifier = {
  ...
  async extractEntities(query: string): Promise<ExtractedEntities> {
    ...
    // Extract locations
    const foundLocations = locations.filter(loc => query.toLowerCase().includes(loc.toLowerCase()));
  ...
  buildFilters(intent: IntentType, entities: ExtractedEntities): Intent["suggestedFilters"] {
    ...
    if (entities.priceRange) {
      filters.priceRange = [entities.priceRange.min || 0, entities.priceRange.max || 10000];
    }
