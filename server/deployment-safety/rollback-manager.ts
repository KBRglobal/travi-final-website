...
const logger = {
  info: (msg: string, data?: Record<string, unknown>) =>
    log.info(`[Rollback] ${msg}`, data),
  warn: (msg: string, data?: Record<string, unknown>) =>
    log.warn(`[Rollback] ${msg}`, data),
...
// Step executors
type StepExecutor = (step: RollbackStep, plan: RollbackPlan) => Promise<{ success: boolean; error?: string }>;
...
function createDefaultSteps(): RollbackStep[] {
  return [
    createStep('validate_target', 'Validate Rollback Target', 'Verify target version exists and is deployable', 1),
...
export function createRollbackPlan(
  ...
  if (rollbackPlans.size >= MAX_ROLLBACK_HISTORY) {
    const oldest = Array.from(rollbackPlans.entries())
      .filter(([_, p]) => p.status === 'completed' || p.status === 'failed' || p.status === 'cancelled')
...
function updateHistory(plan: RollbackPlan): void {
  ...
  const completedRollbacks = history.rollbacks.filter(r => r.status === 'completed' || r.status === 'failed');
  const successfulRollbacks = history.rollbacks.filter(r => r.status === 'completed');

  history.successRate = completedRollbacks.length > 0
    ? successfulRollbacks.length / completedRollbacks.length
    : 0;
  ...
  history.avgDurationMs = durations.length > 0
