...
export class ContentFactory {
  ...
  private async persistAndPublish(content: GeneratedContent, destination: string): Promise<string> {
    ...
    const [inserted] = await db.insert(contents).values({
      ...
      title: content.title,
      slug: content.slug,
      metaDescription: (content as any).metadata.description,
      primaryKeyword: (content as any).metadata.keywords[0] || null,
      secondaryKeywords: (content as any).metadata.keywords.slice(1),
