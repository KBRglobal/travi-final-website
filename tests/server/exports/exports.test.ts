...
import {
  exportContentsAsCSV,
  exportContentsAsJSON,
} from "../../../server/exports/content-exporter";
...
describe("Content Inventory Exporter", () => {
  ...
  describe("Content Export", () => {
    it("should export contents as CSV", async () => {
      const result = await exportContentsAsCSV({ format: "csv", includeHeaders: true });
    ...
    it("should export contents as CSV without headers", async () => {
      const result = await exportContentsAsCSV({ format: "csv", includeHeaders: false });

      expect(result.data).not.toContain("id,title,status");
    });

    it("should export contents as JSON", async () => {
      const result = await exportContentsAsJSON({ format: "json" });
    ...
    it("should respect limit option", async () => {
      const result = await exportContentsAsCSV({ format: "csv", limit: 10 });
  ...
  describe("Entity Export", () => {
    it("should export entities as CSV", async () => {
      const result = await exportEntitiesAsCSV("hotels", { format: "csv", includeHeaders: true });
    ...
    it("should export entities as JSON", async () => {
      const result = await exportEntitiesAsJSON("attractions", { format: "json" });
    ...
    it("should handle different entity types", async () => {
      const hotelResult = await exportEntitiesAsCSV("hotels", { format: "csv" });
      expect(hotelResult.filename).toContain("hotels");

      const attractionResult = await exportEntitiesAsCSV("attractions", { format: "csv" });
      expect(attractionResult.filename).toContain("attractions");

      const diningResult = await exportEntitiesAsCSV("dining", { format: "csv" });
  ...
  describe("CSV Escaping", () => {
    it("should generate valid CSV structure", async () => {
      const result = await exportContentsAsCSV({ format: "csv" });
  ...
  describe("Max Rows Enforcement", () => {
    it("should enforce max export rows", async () => {
      const maxRows = EXPORTS_CONFIG.maxExportRows;

      // Request more than max
      const result = await exportContentsAsCSV({ format: "csv", limit: maxRows + 1000 });
